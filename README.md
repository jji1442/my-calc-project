...
##### 2. 후속 패치된 아크(10월달 아크).
아크의 패치 중 가장 애매했던 '증오의 연쇄' 개선되었습니다.
...
위 스킬로 변경되었는데 새로운 빌드가 바로 떠올랐습니다. 바로, '증오의 연쇄'를 제외한 극딜 이후 '링 익스체인지'를 통해 '웨폰퍼프 링'으로 변경하여 '증오의 연쇄'와 함께 데미지를 넣는 구조. 과거 아크의 극딜과 유사한 구조합니다. 이 빌드가 가능한 이유는 '인피니티 스펠'의 사출량 증가로 인하여 '징표' 생성확률 증가가 없이도 '징표' 생성하는데 충분하며, '징표' 최대 생성 개수도 큰 차이 안나기 때문입니다.

하지만 현재 디렉터의 '이론과 실전의 괴리율을 줄여보자'라는 패치 기조를 보았을 때, 이 빌드는 높은 확률로 다음 패치 때 사용하지 못하도록 패치할 것으로 보입니다. 물론 9월 패치에는 이 기조와 다르게 처리하였지만, 이번 10월 패치에는 현재 기조와 어느정도 일치한 모습입니다. 이 빌드를 사용하면 의도와 다르게 사용하여 이에 따른 조정이 있을 것으로 보입니다. 예상하기론, 극딜 이후에 이어서 사용하지 못하도록 후속 패치로 '징표' 생성확률 증가하는 기능을 삭제할 것 같습니다.

'영원히 굶주린 짐승'의 충돌횟수 증가와 '끝없는 고통'의 참격횟수 증가로 인해, 기존 극딜하는 재미가 돌아왔습니다.

#### 분석 결론
패치한 내용을 보면, 급하게 수정한 결과로 보입니다.

아크의 극딜 지속시간을 30초로 통일하려고 했던 것으로 보이나, 새로운 빌드가 연구된 것으로 보아 100% 깔끔한 구조가 아닙니다.
