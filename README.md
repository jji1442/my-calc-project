...
이러한 긍정적인 변화와는 별개로, 스킬의 전반적인 운용 방식 변화로 인해 전투의 피로감이 굉장히 늘어났습니다.

1. 게이지 부족 현상
   - 이론상 최대치의 데미지를 뽑아내기 위해서, 변신 유지하다보면 게이지 고갈로 현상이 자주 보입니다. 이는 흐름이 뚝 끊기는 느낌을 들게 만듭니다.

2. '잠식 제어' 스킬 쿨타임 문제.
   - '잠식 제어': 스펙터 상태로 변신하는 스킬(쿨타임 3초).
   - 1번 문제로 인해 게이지 고갈이 일어나 변신을 풀게 됩니다.
   - 하지만 변신 중에 새긴 '징표'를 회수하고 싶어 다시 한 번 변신하게 되는데 이 스킬 쿨타임의 3초라서 바로 변신을 풀 수 없다는 점이 문제입니다.
   - 게이지 0이 될 경우, 강제로 레프 상태가 되며 일정 시간 동안 게이지를 채울 수가 없습니다.

3. 어센트 스킬 문제.
   - 아크는 어센트 스킬을 사용 시 '강제로 스펙터 변신'하게 됩니다.
   - 게이지 0이 되면 '강제로 레프'되는 점과 모순되지 않도록 무적 효과를 지닌 스킬에는 시전 중 게이지 소모되지 않게 설정하였습니다.
   - 게이지 부족 현상 때문에 레프 상태에서 이 스킬을 사용하게 되면, 이 스킬 시전 시간 만큼 극딜이 밀리는 현상이 생겼습니다.

이 점을 미루어 보아, "게이지 부족 현상 때문에 다 생긴 일이네? 그러면 게이지 늘려주면 되지 않아?" 라고 생각하셨겠지만, 무작정 게이지를 늘려버리면 허수아비전과 실전의 괴리감이 더 커집니다.
...
1. 어센트 스킬로 인한 게이지 회복량 수급 문제가 더 심해짐.
2. 게이지 부족 현상이 자주 발생.
3.  
