...
## 7. 게임사 패치에 대한 평가.
25.08.21. 직업 전투 경험 개선 목적으로 아크는 '링 익스체인지' 스킬과 '최종데미지 6% 증가' 패시브를 받았습니다.
'링 익스체인지'로 인한 코어 강화 효율 변화를 측정하고 이에 대한 평가를 해보려고 합니다.
이 '최종데미지 6% 증가' 패시브는 상시 증가하므로, 코어 강화 효율 측정에 영향이 미치지 않는다는 점을 안내드립니다.

'링 익스체인지'란?
특수 스킬 반지 2개를 자동으로 변경해주는 스킬로, 특수 스킬 반지를 변경하게 될 경우 변경된 특수 스킬 반지의 사용 쿨타임이 10초 부여됩니다.

기존 아크의 전투 방식에 대해 설명해 드리겠습니다.
기존의 아크는 웨폰퍼프 링으로 변경하지 않았습니다. 왜냐하면 지속시간이 50초인 인피니티 스펠(쿨타임 2분)의 효용가치(점유율 약 27%)가 매우 높고, 특수 스킬 반지를 변경하기 위해선 전투를 5초간 멈춰야하는 단점이 있었기 때문입니다. 그리고 특수 스킬 반지를 변경한다고 해도 그만한 높은 피해량이나 생존적인 이점 둘 다 없었기 때문에 변경하지 않았습니다.

'링 익스체인지'의 사용 방식에 대해 설명을 해드리겠습니다.
리스트레인트 링을 착용된 상태에서 쿨타임 2분 및 1분 버프(리스트레인트 링 포함)들을 전부 사용하고, 15초간 보스에게 공격을 가합니다.(리스트레인트 링 4레벨 기준 15초 지속)
이후, '링 익스체인지' 스킬 사용 시 리스트레인트 링에서 웨폰퍼프 링으로 변경되며 10초의 쿨타임이 생깁니다. 10초 뒤 웨폰퍼프 링을 사용하여 추가 공격을 가합니다.

'링 익스체인지'가 생김으로 인해 아크는 2분 버프 지속시간 중에 특수 스킬 반지 2개를 사용할 수 있게되어, 더욱 더 높은 딜량을 뽑아 낼 수 있게되어 테스트용 데이터를 재측정하려고 합니다.
