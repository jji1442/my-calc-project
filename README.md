...
<br>
시행착오의 기준은 가장 단순한 코어 구성을 가진 렌의 '선참VI' 코어를 기준으로 작성하였고,
마지막 어센트 스킬은 렌의 '창룡파천검 : 일매낙화 천비인적' 코어를 기준으로 작성되었습니다.<br>
<br>
예상 점유율의 경우, 사이트상 100억당 최종데미지 상승율에 제가 계산한 도출한 100억당 최종데미지 상승율의 역산으로 계산하였습니다.<br>
<br>
1. 제가 가지고 있는 데이터는 점유율과 스킬 %데미지 상승량이니, 이를 이용한 엑셀을 제작해 보았습니다.
<img src="trial_and_error_img/trial_and_error_1.PNG">
분석 및 결론: 코어별 최대 레벨이 30레벨, 레벨별 간극이 최대 30%나 벌어진다는 것을 확인하였고, 이에 대해 '보정 상수가 존재하나?'라고 판단했습니다.<br>

1-1. 기존 엑셀에 보정상수(1레벨당 0.01% 감소)를 추가해 보았습니다.<br>
<img src="trial_and_error_img/trial_and_error_1_1.PNG">
분석 및 결론: 보정상수를 넣어도 값이 일정하지 않다는 것을 확인하였고, "스킬 %데미지와 연관이 있나?"라는 가설을 세웠습니다.<br>

2. 스킬 %데미지 상승량(%p)을 점유율의 곱으로 최종데미지 상승율 산출해보았습니다.<br>
<img src="trial_and_error_img/trial_and_error_2.PNG">
분석 및 결론: 스킬 %데미지 상승율에 따른 값이 일정하지 않으므로 "1->2레벨 상승 시의 스킬 %데미지 상승율이 고정인가?"라는 가설을 세웠습니다.

3. 1->2레벨 상승 시의 스킬 %데미지 상승율이 고정으로 두고 최종데미지 상승율을 산출해보았습니다.<br>
분석 및 결론: 스킬 %데미지 상승율이 고정적이지 않으므로 "코어별 강화 순서가 다르면 효율이 달라지나?" 라는 가설을 세웠습니다.
<img src="trial_and_error_img/trial_and_error_3.PNG">

4. 스킬 2가지를 섞어 강화했을 때 효율이 어떻게 변화하는지 확인했습니다.
-> "강화 순서가 다르면, 강화 효율이 다르네?"라는 결론에 도달했습니다.
<img src="trial_and_error_img/trial_and_error_4.PNG">

하지만, 이 때까지 스킬을 연속적으로 강화하였을 때 강화효율이 좋은 스킬이면서 '선참VI'과 유사한 단순한 코어 구성은 없어,
한동안 시행착오를 중지하였고, 메이플스토리의 어센트 스킬 출시를 기다렸습니다.

이후 메이플스토리에 어센트 스킬이 추가되었고. 어센트 스킬은 쿨타임과 관계 없이 스킬을 최대 3번 연속으로 사용 가능하다는 점을 이용하여, 6분 안에 어센트 3번를 다 사용한다는 가정으로 효율 계산하였을 때 연속적으로 강화(2->9레벨까지)하는 것이 효율이 좋다는 것을 보고, 다시 효율을 측정해보았습니다.
-> 오차 범위 내라고 판단함.
...
분석 및 결론: 게임사에서 제공하는 전투분석의 스킬별 점유율을 사용한 방식이 아니라, 스킬별 데미지를 총합 데미지로 나누었다라고 가정하면, '-0.044% ~ 0.081%'의 범위는 오차범위 내라는 결론에 도달하였습니다.
