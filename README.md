...
### 제작 기한
- 예상 기간: 영업일 기준 15.8일.<br>
   (10일 + 4 * 15일 + 25일) / 6 = 15.8일.
- 시작일: 25.10.31.
- 기한일: 25.11.21.
- 완성일: 미완성.
...
2. 측정 범위는 모든 헥사 코어(6차 스킬 코어, 5차 강화 코어, 4차 이하  마스터리 코어)임.
3. 기본 스킬의 %데미지는 ‘쓸만한 컴뱃 오더스’가 사용 중인 상태를 기준으로 측정함.
  - ‘쓸만한 컴뱃 오더스’는 ‘일부 스킬만’ 최대 레벨이 30레벨에서 31레벨로 상승하는 버프 스킬임.
4. 분석의 범위를 명확히 설정하기 위해서 ‘다가오는 죽음’은 ‘인피니티 스펠’ 사용 시에만 발생하는 경우를 기준으로 강화 효율을 측정함.
5. 마스터리 코어 효율 계산에 영향을 미치는 ‘강화 코어인 인피니티 스펠의 지속시간 중 다가오는 죽음 %p 상승’은 19레벨 기준으로 측정함.
   - ‘인피니티 스펠’은 ~50초~ 30초간 ‘다가오는 죽음’이 3개 추가로 생성함. 이 스킬은 강화 코어의 효과를 받아 ‘다가오는 죽음’의 데미지를 133%p 추가로 상승하는 버프 스킬임.
6. 어센트 스킬, 오리진 스킬, '어비스 차지드라이브VI' 이 3가지의 방어율 무시, 보스 공격 시 데미지 증가, 데미지 증가 효과가 전부 다르게 측정함.
   - 어센트 스킬은 장비 : 모자, 장비 : 반지, 조건부로 발동하는 패시브 스킬 효과, 액티브 스킬 사용 효과(길드 스킬, 장비 명장, 아크 자체 버프 포함), 몬스터의 속성, 몬스터의 패턴, 디버프, 지속 시간이 30분 미만인 소비, 캐시 아이템이 적용되지 않음.
   - 오리진 스킬은 엔젤릭 버스터의 링크 스킬을 제외한 모든 링크 스킬, 5차 특수 코어인 일격필살은 스탯 증가량 * 가동률로 측정함.(아크 자체 버프 스킬 또한 제외)
   - '어비스 차지드라이브VI'는 모든 조건부 스킬들은 스탯 증가량 * 가동률로 측정함.
7. 방어율 무시(%) 효율 측정 기준은 방어율 380% 보스임.
8. 스킬 코어는 최소 1레벨을 기준으로 측정, 강화 코어는 최소 0레벨을 기준으로 측정, 마스터리 코어는 최소 1레벨을 기준으로 측정.(스킬 점유율이 있어야 효율이 측정 가능)
