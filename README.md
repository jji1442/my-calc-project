...
- 정의한 함수.
  - 1레벨 상승 시 강화효율 체크하는 함수.
    ...
    - 리스트에 마스터리 코어 1~30레벨 재화 소모량을 저장함.
    - 재화 소모량은 마스터리코어 1~30레벨 리스트에서 [현재 스킬레벨]의 데이터임.
    - 재화 효율은 100만 / (재화 시세 * 재화 소모량)임.
    - 스킬 배율 1레벨 상승 시 %데미지 상승량(%p)/현재 레벨 기준 %데미지임.
    - 1레벨 상승 시 최종데미지 상승율은 점유율 * 스킬 배율임.
    - 1레벨 상승 시 100억당 최종데미지 상승율은 재화 효율 * 점유율 * 스킬 배율임.
    - 리턴값은 1레벨 상승 시 최종데미지, 1레벨 상승 시 100억당 최종데미지임.
