...
사용횟수 보정치
= 매서풀VI 1레벨 사용횟수 / 매서풀VI 9레벨 사용횟수 * 100
= 168회 / 166.4회 * 100
= 100.962%

"보정된" 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지
= 사용횟수 보정치 * 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지
= 100.962% * 8.5103조
= 8.5921조

[두 레벨 간의 스킬 %데미지 차이 확인하기]

매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지 예상치 = 매서풀VI 1레벨 스킬 자체가 입힌 누적 데미지 * 스킬 %데미지 상승율
= 5.1859조 * 164.865%
= 8.5497조.
...
공격력 상승으로 인한 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지의 상승치
= 오차율 * 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지
= 0.496% * 8.5103조
= 0.0422조

앞서 계산한 수치(0.0422조)는 매서풀VI 스킬 '단독'으로 얻은 공격력으로 인한 데미지 상승치를 확인했습니다.

하지만, 매서풀VI의 공격력 상승 버프는 매서풀VI에만 적용되는 것이 아니라, 버프 지속시간 동안 사용하는 '모든 스킬'에 동일하게 적용됩니다. 매서풀VI를 제외한 나머지 스킬들이 얻은 데미지 상승량을 계산할 겁니다.

아래는 전체 데미지 상승량에서 매서풀VI 자체의 상승량을 제외하는 방식으로, 나머지 스킬에 적용된 공격력 버프의 상승치를 산출해보겠습니다.

매서풀VI 레벨 차이에 따른 전체 데미지 차이
= 매서풀VI 9레벨 전체 데미지 - 매서풀VI 1레벨 전체 데미지

매서풀VI 스킬 자체가 입힌 누적 데미지의 상승치
= 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지 - 매서풀VI 1레벨 스킬 자체가 입힌 누적 데미지

공격력 상승으로 인한 매서풀VI 스킬을 제외한 나머지 스킬로 입힌 누적 데미지의 상승치
= 매서풀VI 레벨 차이에 따른 전체 데미지 차이 - 매서풀VI 스킬 자체가 입힌 누적 데미지의 상승치
= 5.7148조 - (8.5103조 - 5.1859조)
= 2.3904조.

공격력 상승 전체 데미지의 비중
= 공격력 상승으로 인한 매서풀VI 스킬을 제외한 나머지 스킬로 입힌 누적 데미지의 상승치 / 매서풀VI 9레벨 전체 데미지 * 100
= 2.3904조 / 539.5919조 * 100
= 0.443%

매서풀VI를 제외한 나머지 스킬들의 버프 비중이 0.443%를 확인했습니다. 이제 여기서 앞단락에서 구해두었던, 공격력 상승분까지 더해주면, 비로소 이 버프가 캐릭터 전체 딜량에서 차지하는 종합 비중(0.451%)이 완성됩니다.
...
공격력 상승으로 인한 누적 데미지 비중
= (공격력 상승으로 인한 매서풀VI 스킬을 제외한 나머지 스킬로 입힌 누적 데미지의 상승치 + 공격력 상승으로 인한 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지의 상승치) / 매서풀VI 9레벨 전체 데미지 * 100
(2.3904조 + 0.0422조) / 539.5919조  * 100
= 0.451%

= 100 / (100 - 공격력 상승으로 인한 누적 데미지 비중)
= 1.00453
