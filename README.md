...
2. 패치 내용<br>
개선 항목과 변경 항목으로 나눠 설명하겠습니다.

<개선 항목>
1. 스킬 딜레이 감소 + '전투광란' 상시로 변경.
2. '기어다니는 공포' 키다운 중 HP 소모에서 MP 소모로 변경.
3. '끝없는 고통'의 쿨타임 60초에서 120초로 변경.
4. '영원히 굶주리는 짐승' 스킬 시퀀스에 추가 가능.

<변경 항목>
1. '인피니티 스펠' 지속시간을 50초에서 30초로 변경.
2. 신규 버프 스킬 '증오의 연쇄' 추가.
3. '영원히 굶주리는 짐승'의 총 %데미지 감소, 해당 스킬로 인한 '다가오는 죽음' 사출횟수 감소.
4. '끝없는 고통'의 총 %데미지 감소, 해당 스킬로 인한 '다가오는 죽음' 사출횟수 감소.
5. '거스트 차지드라이브VI', '어비스 차지드라이브VI' %데미지 소폭 상승.
6. '걷잡을 수 없는 혼돈VI', '채워지지 않는 굶주림VI' %데미지 대폭 상승.

3. 분석 보고서<br>
게임사에서 아크의 직업 전투 경험 개선하여 개선 이전에 아크의 구조와 문제점을 작성하고, 개선 이후 아크의 직업 전투 경험 개선이 어떻게 수정되었는지 확인해보려고 합니다.

<보고서 1편: 아크에 대한 설명><br>
<아크의 구조><br>
  아크는 변신을 통해 전투 형태가 전환되는 직업입니다. 
  변신 전을 레프 상태, 변신 후를 스펙터 상태라고 합니다.
  레프 상태에서는 공격 시 '적중 시 아크에게 버프를 부여하는 스펠'이 사출되며, 스펙터 상태에선 공격 시 '다가오는 죽음'이 사출됩니다.

  아크의 기본적인 전투 방식은 '극딜 버프'가 활성화된 동안은 스펙터 상태를 유지하고, 버프가 없을 때는 레프 상태로 전환하여 게이지를 회복하는 구조입니다.
  2분 전투 기준으로 60초 스펙터, 60초 레프의 비율을 유지하는 것이 핵심입니다.

  아크는 특정 스킬을 기반으로 다른 스킬을 연계하는 직업입니다. 이러한 스킬 연계 시스템은 크게 두 가지로 나뉩니다.
  레프 상태에선 '플레인 차지드라이브'을 기반으로 '거스트 차지드라이브', '스칼렛 차지드라이브', '어비스 차지드라이브', '멈출 수 없는 충동', '잊혀지지 않는 악몽' 스킬과 연계합니다.
  스펙터 상태에선 '잊혀지지 않는 흉몽'을 기반으로 '채워지지 않는 굶주림', '지워지지 않는 상처', '걷잡을 수 없는 혼돈', '멈출 수 없는 본능' 스킬과 연계합니다.

<'다가오는 죽음'에 대한 설명>
  '다가오는 죽음'은 아크의 핵심적인 컨셉입니다.
  스펙터 상태에서 적에게 공격 시 '다가오는 죽음'이 사출되며, 이 '다가오는 죽음'은 전체 딜량의 27%를 차지할 정도로 비중이 높습니다.
  많은 아크 유저들은 '다가오는 죽음'에 대한 높은 의존도 때문에, 스펙터 상태에서 사용하는 스킬들을 '다가오는 죽음'을 사출하기 위한 사전 작업이라 표현하곤 합니다.
  
<'돌아오는 증오'에 대한 설명>
  '돌아오는 증오'는 '다가오는 죽음'이 사출되어 적에게 적중 시 20% 확률로 '징표'가 생성되며, 이 '징표'를 회수하여 경로에 있는 적들을 공격하는 스킬입니다.
  이 '징표'는 최대치에 도달하더라도 기존 '징표'가 사라지기 전까지 새로 생성되지 않습니다. 또한, '징표'를 회수할 수 있는 거리에 제한이 있습니다.
  
<아크의 구조적 문제점과 개선 방안>
1. 연계형 스킬 단일로 사용 시 시전/후 딜레이 감소
  - 문제점: 연계형 스킬들을 단일로 사용 시 긴 시전/후 딜레이로 인해, 보스 패턴을 피하는데 어려움이 존재합니다.
  - 개선 방안: 이는 캐릭터의 유연한 전투를 방해하는 주요 원인으로, 연계형 스킬들을 단일로 사용 시 시전/후 딜레이를 감소시키는 개선이 필요합니다.
2. 상시 '전투광란'
  - '전투광란': 스킬을 연계할 때마다, 연계속도 1단계 상승(20초 지속, 최대 3중첩)
  - 문제점: 스킬 연계에 따라 연계 속도를 변화시켜 플레이의 일관성을 해칩니다. 이로 인해 스킬 연계 실패 시 캐릭터가 의도치 않게 이동하여 보스 패턴에 피격될 확률이 높아집니다.
  - 개선 방안: 연계 속도 효과를 최대 중첩 및 상시 적용되도록 변경하여 안정적인 플레이를 보장해야 합니다.
3. '기어다니는 공포' 시전 중 체력 감소 현상 삭제 및 공격 가능하게 변경.
  - '기어다니는 공포': 최대 5초간 키다운 가능한 스킬, 키다운 중 HP를 지속적으로 소모 및 무적 상태, 키다운 중 공격 불가능(쿨타임 60초)
  - 문제점: '기어다니는 공포'는 키다운 중 아무것도 하지 못할 뿐만 아니라, HP 소모가 불가능할 경우 무적 상태가 해제되는 구조적 문제를 가지고 있습니다.
  - 개선 방안: 다른 직업들과 유사하게 2초간 무적 상태와 동시에 공격이 가능하도록 변경하고, 소모 방식을 HP에서 MP로 전환하는 개선이 필요합니다.
...
7. '영원히 굶주리는 짐승' 사용 후 캐릭터 사망 시 짐승이 사라지지 않게 수정.
  - 문제점: '영원히 굶주리는 짐승'은 설치 후 캐릭터가 사망하면 사라져 딜 손실이 발생합니다.
  - 개선 방안: 캐릭터의 사망 여부와 관계없이 설치된 '영원히 굶주리는 짐승'이 사라지지 않도록 변경해야 합니다.
8. '끝없는 고통' 쿨타임 2분화.
  - 문제점: 끝없는 고통은 현재 쿨타임 1분 스킬로, 준극딜에 사용되지만 딜 상승량이 1.5%에도 미치지 못해 효율이 매우 낮습니다.
  - 개선 방안: 아크가 '극딜 직업'이라는 정체성을 고려할 때, 쿨타임을 2분으로 변경하여 메인 극딜 주기에 맞추는 것이 더 효과적입니다.
9. 극딜 버프 사용 시 체력 소모량 감소.
  - 문제점: 극딜 버프를 사용할 때 마다 최대 체력의 23% + 3000의 HP(개발자 아크 기준으로 최대 체력의 4%)를 소모합니다. 이는 캐릭터의 생존성을 크게 위협하고, 난이도를 불필요하게 높이는 원인이 됩니다.
  - 개선 방안: 안정적인 플레이를 위해 극딜 버프 사용 시 소모되는 HP양을 절반으로 줄이는 개선이 필요합니다.

<<게임사 패치에 대한 고찰>>

가장 먼저, 아크의 기본 구조가 바뀌었습니다.

기존 구조: 55초 스펙터, 55초 레프.
변경된 구조(고점 기준): 34초 스펙터, 21초 레프, 24초 스펙터, 31초 레프.
(게이지 부족 시, 레프로 바꿔야함)

전투 구조가 간단한 구조(극딜 구조)에서 다른 직업과 유사한 전투구조(극딜 - 준극딜 구조)로 바뀜으로 인해,
핵심 버프를 효율적으로 활용하지 못하는 상황으로 이어졌습니다.

아크의 핵심 버프.
1. 레프 오브 더 갓(아크 버프) 60초 지속.
2. 차지 앰플리피케이션(아크 버프) 60초 지속.
3. 매직 서킷 풀 드라이브(종족 공용) 60초 지속.
4. 그란디스 여신의 축복(종족 공용) 40초 지속.

'인피니티 스펠'의 지속시간은 30초로 변경으로 인해, 위의 핵심 버프틀의 지속시간와 맞지 않아 버프 효과를 온전히 활용하지 못하는 상황이 생겼습니다.

또한, '근원의 기억'을 극딜 사이클에 강제적으로 추가해야한다는 점이 문제점으로 작용되었습니다.

  - '근원의 기억': 18초간 게이지 소모하지 않으며, 시전 중 무적.
  기존 구조: 전투 중 유도리 있게(극딜 버프 사용 후 37초 내에 사용) 사용.
  변경된 구조: 극딜 버프 사용 후 16초 내에 사용해야하므로 극딜 사이클에 강제적으로 추가.

'극딜 - 준극딜 구조'로 변경할 예정이었다면, '근원의 기억'의 게이지 소모되지 않는 방식을 삭제하고 기본적인 게이지양을 늘리는 방식으로 변경했어야하며
'근원의 기억' 사용 중 게이지를 소모되지 않게 수정해야한다고 생각합니다.

전투경험개선이라는 명목으로 구조 개선을 했지만, 캐릭터의 안정성이 굉장히 하락했음을 알 수 있었습니다.

그리고, '인피니티 스펠'의 지속시간 50초에서 30초로 변경 및 '증오의 연쇄' 버프 스킬 추가에 대해 굉장히 부정적인 시각으로 바라보고 있습니다.

  이 결과가 예상컨데, "다른 직업도 극딜 압축해줬는데, 아크는 안해줄 수 없으니 '인피니티 스펠'의 지속시간을 30초로 변경(압축 아님)해주고.
  유저들이 '끝없는 고통VI'도 2분화 원하니 변경했더니 아크에게 남는 스킬은 '돌아오는 증오VI' 스킬이 하나 남으니,
  그 딜량을 올릴 수 있는 '증오의 연쇄' 버프를 추가하자."한 느낌이 듭니다.
...
이동형 보스가 아크의 딜 사이클을 5번 이상(약 4초 이상) 맞아줄 리가 없었습니다.
맞았다 한들, '다가오는 죽음'의 사출되는 속도도 느려 이동형 보스가 '무적 + 돌진'/'텔레포트'를 사용 시 '다가오는 죽음'이 증발되는 경우가 다반사였습니다.

증오의 연쇄 사용 시, '돌아오는 증오'의 쿨타임이 초기화되게 설계된 의도는 고점을 위한 설계인 것 같습니다.
게임사에서 아크를 변경한 의도대로 하면, 스펙터 유지시간이 최소 50초(극딜 30초, 준극딜 20초)가 됩니다.
여기서, 돌아오는 증오의 쿨타임 초기화 시스템을 넣었다는건 이 시간보다 더 많은 시간을 스펙터로 지내라는 말인데, 시스템적으로 불가능합니다. 110초 기준 스펙터 유지시간은 최대 55 ~ 60초입니다.
5 ~ 10초 안에 징표를 12개를 생성해서 '돌아오는 증오'의 쿨타임 초기화 시스템을 사용할 수 있는 경우는 '인피니티 스펠' 지속시간 중에만 사용이 가능합니다.
그러면 "그렇게 사용하면 되지 않냐?"라고 말씀하시겠지만, 준극딜이 밀리는 현상이 생깁니다.
하지만, 스킬 쿨타임 감소 4초 + 리스트레인트 링 5레벨(전투 스킬 반지)을 사용하면 이 현상들이 해결됩니다.
- 리스트레인트 4레벨: 지속시간 동안 및 영역 내에 있을 경우 공격력% 대폭 증가(15초 지속, 쿨타임 120초)
- 리스트레인트 5레벨: 지속시간 동안 및 영역 내에 있을 경우 공격력% 대폭 증가(20초 지속, 쿨타임 120초)
리스트레인트 4레벨 기준, 신규 극딜 사이클을 사용하기엔 빡빡한 구조(8초 안에 극딜 사이클 굴려야 함)
리스트레인트 5레벨 기준, 신규 극딜 사이클을 사용하는데 큰 지장 없는 구조(10초 안에 극딜 사이클 굴리면 됨)
쿨타임 밀림 현상 또한 메르세데스 유니온 효과로 스킬 쿨타임 6% 감소로 인해 쿨타임이 120초, 60초 스킬들이 각각 112.8초, 56.4초로 감소됩니다.
추가적으로 스킬 쿨타임 4초 감소 효과가 적용되는데, 각각 108.8초, 52.4초로 감소하므로 준극딜의 여유시간이 4초가 생깁니다.
신규 극딜 사이클로 인해 '증오의 연쇄' 쿨타임이 4초 가량 밀리는 것을 생각해보면, 고점을 위한 설계라고 봅니다.

극딜 중 '영원히 굶주리는 짐승'의 충돌횟수과 '끝없는 고통VI'의 참격횟수 조정으로 인해 '다가오는 죽음'의 사출횟수가 줄어 극딜의 재미가 사라졌습니다.
'다가오는 죽음'의 타격에 의한 소리, 이로 인한 보스의 체력이 감소되는 시각적 재미. 이 2가지를 놓쳤다고 볼 수 있습니다.

다음으로 '거스트 차지드라이브VI', '어비스 차지드라이브VI' %데미지 소폭 상승과 '걷잡을 수 없는 혼돈VI', '채워지지 않는 굶주림VI' %데미지 대폭 상승은
다가오는 죽음의 딜 점유율 낮아진 만큼 보정해준 느낌인데, 마스터리 코어의 강화효율 올려준 것은 좋습니다.
'걷잡을 수 없는 혼돈VI', '채워지지 않는 굶주림VI'의 시전 시간 보다 '지워지지 않는 상처VI', '멈출 수 없는 본능'의 시전 시간이 짧아 '다가오는 죽음'의 사출용으로 우선적으로 사용했지만, 이제는 쿨타임이 돌 때마다 '걷잡을 수 없는 혼돈VI', '채워지지 않는 굶주림VI'를 우선적으로 사용하게 변경되었습니다.
두 스킬 점유율이 기존 점유율에 비해 2.6 ~ 2.7배 상승된 것이 문제입니다.
이렇게 되면 '스킬 쿨타임 감소' 모자 + '확률에 따른 일부 스킬 쿨타임 초기화' 어빌리티를 강제적으로 변경해야 합니다.
스킬 쿨타임 감소는 기존에도 다다익쿨(스킬 쿨타임 감소가 높을수록 좋다)이라 문제는 없었지만 메인 어빌리티는 변경해야 합니다.
최근 아크 어빌리티를 종결 옵션(게임 시작한 지 4년 만에)을 뽑았습니다. 하지만 다시 바꿔야한다고 생각하니 이보다 더 불쾌할 수 없다고 생각합니다.

<<총평>>
점수는 5점 만점 1점 드립니다.
아크에 대한 지식이 없었던 사람이 편의성 개선과 극딜 압축 2가지 다 수용하려고 하다가 이 사단이 난 듯한 느낌이 듭니다.

생각 없이 패치한다고 생각든 이유가
1차적으로 오버 드라이브 단점 삭제하지 않은 것.
2차적으로 아크의 파티 버프를 조정하지 않은 것.
3차적으로 종족 공용 버프 패치하지 않은 것.

오버 드라이브는 28초 공격력 상승 그 이후 쿨타임 돌 때 까지 공격력 소폭 감소
인데, 다른 직업군은 패치를 해줬는데 해적 직업군만 패치해주지


이 패치는 악의적으로 패치한 느낌이 듭니다.

직업군 계열별 패치 상황
전사: 오라 웨폰 상시 유지, 바디 오브 스틸 피격 시 데미지% 상승 삭제.
마법사: 오버로드 마나 마나 소모량 감소, 에테리얼 폼 사용 시 공격 가능.
궁수: 직업별 가이디드 애로우 이펙트 변화.(단점이 없었음)
도적: 레디 투 다이 피격 %데미지 상승 삭제, 베놈 버스트 %데미지 대폭 상승.
해적: 패치 없음.

해적의 스킬 중 단점인 오버 드라이브의 스킬 쿨타임 중 공격력 소폭 감소. 이 부분은 왜 삭제하지 않는지 아직도 의문입니다.
지금 2번째 패치인데도 안들어온 것을 보면 악의적으로 패치를 안한 것으로 보입니다.
...
캐릭터의 전투 경험을 개선하는 가장 간단한 방법은 명확합니다.

1. 극딜 직업이다? 극딜 시간을 압축하고 딜링 과정의 족쇄를 풀어주는 것이 올바른 방향입니다.
2. 평딜 직업이다? 안정적인 딜량을 방해하는 요소를 제거하는 것이 중요합니다.
...
하지만, 게임사는 극딜에도 족쇄를 준극딜에는 더 큰 족쇄를 달아버리는 만행을 저질렀습니다.



모든 버프 스킬별로 쿨타임 달라 딜 편차가 심한 시절도 견뎌냈지만, 이번 아크의 패치는 직업 자체를 불쾌하게 변경한 것이므로 더 이상 이 직업을 할 수 없다고 판단했습니다.
다른 직업으로 강화효율을 계산하는 것으로 변경했습니다.
