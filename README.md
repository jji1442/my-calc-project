...
<br>
시행착오의 기준은 가장 단순한 코어 구성을 가진 렌의 '선참VI' 코어를 기준으로,
마지막 어센트 스킬은 렌의 '창룡파천검 : 일매낙화 천비인적' 코어를 기준으로 삼아 진행했습니다.<br>
<br>
...
분석 및 결론: 스킬 %데미지 상승율에 따른 값이 일정하지 않으므로, "스킬 %데미지 상승율은 1레벨에서 2레벨로 상승 시 상승율로 고정인가?"라는 가설을 세웠습니다.
...
분석 및 결론: "강화 순서가 다르면, 강화 효율이 다르네?"라는 결론에 도달했습니다.
<br>
하지만, 메이플스토리에 코어들이 추가되면서 하나의 코어를 연속적으로 강화(최소 5회 이상)했을 때 효율 좋은 구간이어야하며 '선참VI'과 유사한 단순한 코어를 찾기 어려워 한동안 시행착오를 중지하였습니다.<br>
<br>
이후 메이플스토리에 어센트 스킬이 추가되었고. 어센트 스킬은 쿨타임과 관계 없이 스킬을 한 보스에서 최대 3번을 연속으로 사용 가능하다는 점을 이용하여, 보스 클리어 타임이 6분을 기준으로 효율 계산하였을 때 연속적으로 강화(2 -> 9레벨)하는 것이 효율이 좋다는 환산주스탯의 정보를 보고 다시 효율을 측정해보았습니다.<br>
<br>
5. 어센트 스킬 기준으로 1번을 추가 테스트하였습니다.
