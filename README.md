...
3. 어센트 스킬 문제.
   ...
   - 이 현상으로 인해 BOSS '카링' 3페이즈에서 극딜 시간이 기존보다 더 밀립니다.
...
4. 생성된 '징표' 위치 고려.
   ...
   - 데미지 손해가 가장 적은 위치를 고려했을 때 / 안했을 때 데미지 편차가 굉장히 심합니다.
      ...
      - BOSS '최초의 대적자'의 1페이즈는 어센트 스킬 1번(1페이즈 체력의 약 4%) 만큼의 데미지 만큼 차이 납니다.

5. '근원의 기억' 강제 극딜 사이클 추가.
   - '근원의 기억': 18초간 게이지 소모하지 않으며, 시전 중 무적.
   - 무적기 하나 없는게 그렇게 큰 문제인가 싶으시겠지만, 저는 이 스킬의 부재로 인해 BOSS '최초의 대적자'에서 죽는 일이 빈번해졌습니다.
   - 기존에는 BOSS '최초의 대적자'에서 죽는 경우가 손에 꼽을 정도 입니다. 지인들이 죽는게 이해가 안될 정도로.

온 몸을 비틀며 고점을 바라보려고 노력해도 잘 되지 않으며, 이렇게 해야 기존 실전 데미지와 비슷하다는 점은 문제라고 생각합니다.

위 현상을 미루어 보아, "대부분 게이지 부족 현상 때문에 다 생긴 일이네? 그러면 게이지 늘려주면 되지 않아?" 라고 생각하시겠지만, 무작정 게이지를 늘려버리면 허수아비형 보스와 데미지 격차는 더욱 심해집니다. 스펙터 상태를 더 길게 유지가 가능하다는 건 그 시간만큼 연계가 가능합니다. 이동형 보스들은 연계 스킬을 잘 맞아주지 않는데 이번 패치로 연계 스킬의 %데미지가 커졌으므로 격차는 더 커질 것으로 예상됩니다.
