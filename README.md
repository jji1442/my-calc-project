...
위와 같이 개선되었는데 새로운 빌드가 바로 떠올랐습니다. 바로, 극딜 이후 웨폰퍼프 링과 함께 데미지를 넣는 구조. 과거의 아크가 했던 구조와 유사합니다.
이 빌드가 생각난 이유는 징표 생성하는데 인피니티 스펠의 사출량 증가로 충분하고, 징표 생성 개수도 큰 차이 안나기 때문입니다.

쿨밀리는 것 + 게이지 부족현상 고려하면 바로 사용해야하는 것이 맞습니다. 

위 패치는 '인피니티 스펠'을 유동적으로 사용할 수 있게 나눠준 느낌이 들지만, 이는 모순점이 존재합니다.
8월 밸런스 패치 때 추가된 링 익스체인지 스위칭 후 링의 쿨타임이 최소 10초 부여되게 설정되어 있었는데

장착한 스킬 반지와 예비 슬롯에 특수 스킬 반지 교체 사용 시 교체된 반지의 특수 스킬 및 링 액티베이션에 10초의 재사용 대기시간 적용

테스트 서버 기준으로 쿨타임이 5초였었습니다. 하지만 극딜 시간이 긴 캐릭터들이 수혜를 보자, 10초로 너프를 했던 것을 생각하면 이 또한 막힐 것으로 봅니다.
징표 생성확률 40%, 
