...
2. 패치 내용<br>
  ...
  &emsp;2. '기어다니는 공포' 키다운 중 HP 소모에서 MP 소모로 변경.<br>
  &emsp;3. '끝없는 고통'의 쿨타임 60초에서 120초로 변경.<br>
  &emsp;4. '영원히 굶주리는 짐승' 스킬 시퀀스에 추가 가능.<br>
  변경 항목<br>
  &emsp;1. '인피니티 스펠' 지속시간을 50초에서 30초로 변경.<br>
  &emsp;2. 신규 버프 스킬 '증오의 연쇄' 추가.<br>
  &emsp;3. '영원히 굶주리는 짐승'의 총 %데미지 감소, 해당 스킬로 인한 '다가오는 죽음' 사출횟수 감소.<br>
  &emsp;4. '끝없는 고통'의 총 %데미지 감소, 해당 스킬로 인한 '다가오는 죽음' 사출횟수 감소.<br>
  &emsp;5. '거스트 차지드라이브VI', '어비스 차지드라이브VI' %데미지 소폭 상승.<br>
  &emsp;6. '걷잡을 수 없는 혼돈VI', '채워지지 않는 굶주림VI' %데미지 대폭 상승.<br>
...
3. 전투경험 개선 후 아크 구조.<br>
  ...
  &emsp;&emsp;- '근원의 기억': 18초간 게이지 소모하지 않으며, 시전 중 무적.<br>
  ...
  &emsp;&emsp;하지만, 그 결과 실전 데미지는 처참해졌습니다. 이동형 보스가 아크의 연계 스킬을 5번 이상(약 4초) 맞아줄 리가 없었습니다. 맞았다 한들, ‘돌아오는 증오‘ 사용 시 이동형 보스가 돌진 또는 텔레포트를 사용하여 맞지 않는 경우가 다반사였습니다.<br>
  ...
  &emsp;&emsp;&emsp;;- 리스트레인트 링 4레벨: 지속시간 동안 및 영역 내에 있을 경우, 공격력% 대폭 증가.<br>
  &emsp;&emsp;&emsp;(15초 지속, 쿨타임 120초)<br>
  &emsp;&emsp;&emsp;- 리스트레인트 링 5레벨: 지속시간 동안 및 영역 내에 있을 경우, 공격력% 대폭 증가.<br>
  &emsp;&emsp;&emsp;(20초 지속, 쿨타임 120초)<br>
  &emsp;&emsp;&emsp;리스트레인트 링 4레벨 기준, 신규 극딜 사이클을 사용하기엔 빡빡한 구조.<br>
  &emsp;&emsp;&emsp;(8초 안에 극딜 사이클 굴려야 함)<br>
  &emsp;&emsp;&emsp;리스트레인트 링 5레벨 기준, 신규 극딜 사이클을 사용하는 데 큰 지장 없는 구조.<br>
  &emsp;&emsp;&emsp;(10초 안에 극딜 사이클 굴리면 됨)<br>
  &emsp;&emsp;&emsp;;쿨타임 밀림 현상 또한 메르세데스 유니온 효과로 스킬 쿨타임 6% 감소로 인해 쿨타임이 120초, 60초 스킬들이 각각 112.8초, 56.4초로 감소됩니다. 추가적으로 스킬 쿨타임 4초 감소 효과 모자가 적용되는데, 각각 108.8초, 52.4초로 감소하므로 준극딜의 여유시간이 4초가 생깁니다. 신규 극딜 사이클로 인해 '증오의 연쇄' 쿨타임이 4초 가량 밀리는 것을 생각해보면, 고점을 위한 설계라고 봅니다.<br>
  ...
  &emsp;&emsp;다음으로 '거스트 차지드라이브', '어비스 차지드라이브' %데미지 소폭 상승과 '걷잡을 수 없는 혼돈', '채워지지 않는 굶주림' %데미지 대폭 상승은 다가오는 죽음의 점유율 낮아진 만큼 보정해준 느낌인데, 마스터리 코어의 강화효율 올려준 것은 좋습니다. '걷잡을 수 없는 혼돈', '채워지지 않는 굶주림'의 시전시간 보다 '지워지지 않는 상처', '멈출 수 없는 본능'의 시전시간이 짧아 '다가오는 죽음'의 사출용으로 먼저 사용했지만, '걷잡을 수 없는 혼돈', '채워지지 않는 굶주림'이 먼저 사용하도록 변경되었습니다.<br>
...
4. 분석 결론<br>
  ...
  &emsp;&emsp;이는 아크의 전투경험을 불쾌하게 변경한 패치로 판단됩니다. 이로 인해 해당 직업의 핵심적인 재미가 훼손되었다고 판단하였고, 이 프로젝트의 논리적인 효율을 측정하기 위한 대상을 엔젤릭 버스터로 변경했습니다.<br>
...
| 전투 횟수 | 패치 전 55:55 빌드 | 패치 후 55:55 빌드 | 패치 후 58:52 빌드 |
|:---:|:---:|:---:|:---:|
...
| 평균 | 298조 3243억 | 279조 7229억 | 301조 5426억 |
  ...
  &emsp;&emsp;[>> 패치 전 55:55 빌드 분석 결과 보러가기](#unused_and_used_ring_exchange_img)<br>
  &emsp;&emsp;[>> 패치 후 55:55 빌드 분석 결과 보러가기](#unused_and_used_ring_exchange_img)<br>
  &emsp;&emsp;[>> 패치 후 58:52 빌드 분석 결과 보러가기](#unused_and_used_ring_exchange_img)<br>
