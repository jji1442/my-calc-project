...
2. 신규스킬 1분 증오의 연쇄.
3. 스펙터 연계스킬 중 '걷잡을 수 없는 혼돈VI', '채워지지 않는 굶주림VI' %데미지 대폭 상승.
4. 레프 연계스킬 중 '거스트 차지드라이브VI', '어비스 차지드라이브VI' %데미지 소폭 상승.
...
이러한 주요 버프틀의 지속시간과 인피니티 스펠의 지속시간(30초)와 맞지 않아, 모든 효과를 100% 받지만 활용을 50%도 못하는 상황도 생겼습니다.
...
'극딜 - 준극딜 구조'로 변경할 거였으면, '근원의 기억'의 게이지 소모되지 않는 방식에서 기본적인 게이지양을 늘리는 방식으로 전환했어야한다고 봅니다.
추가적으로 '근원의 기억'사용 중 게이지 소모되지 않게 수정해야한다고 생각합니다.

전투경험개선이라는 명목으로 구조 개선을 했지만, 캐릭터의 안정성이 굉장히 하락했음을 알 수 있었습니다.

위의 내용과 이어지는 내용으로 아래의 내용을 작성할거야.

그리고, '인피니티 스펠'의 지속시간 50초에서 30초로 변경 및 '증오의 연쇄 버프' 스킬 추가에 대해 굉장히 부정적인 시각으로 바라보고 있습니다.

이 결과가 예상컨데, "다른 직업도 극딜 압축해줬는데, 아크는 안해줄 수 없으니 '인피니티 스펠'의 지속시간을 30초로 변경해주고(압축 아니고, 나머지 지속시간을 준극딜로 옮기는 작업).
유저들이 '끝없는 고통'도 2분화 원하니 해줬더니 아크에게 남는 스킬은 '돌아오는 증오' 스킬이 하나 남아서,
...
맞았다 한들, 다가오는 죽음의 사출 속도도 느려 보스가 텔레포트를 사용 시 증발되는 경우가 다반사였습니다.(파티 플레이 한정)
