...
2. 강화효율 측정방식 변경으로 인한 처리속도 개선.
   ...
   - 설명: 재화 효율의 변화가 없는 구간에서는 계산을 생략하고, 재화 소모량이 크게 상승되는 레벨인 9레벨과 19레벨에서만 평균 효율을 측정하도록 로직을 변경하여 처리 속도를 개선.
...
3. 변수 사용량이 줄여 RAM 사용량 개선.
   ...
   - 설명: RAM 사용량을 줄이기 위해서 개선 및 추천 레벨 만큼 상승시킬 것을 가정.

4. 일부 계산 개선.
   - 기존: 5레벨 상승 시 평균값을 도출하여 사용.
   - 변경: 5, 6레벨 상승 시 총 상승량을 도출하여 사용.
   - 설명: 추천 레벨 만큼 강화를 한다는 가정이기 때문에 총 상승량을 기준으로 추천.
