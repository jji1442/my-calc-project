...
### 패치 내용
게임사에서 컨티뉴어스 링을 패시브 반지로 사용하게 만들었고, 액티브 반지로 리스트레인트 링만 사용할 수 있도록 설정하였습니다.
...
게임사에선 "컨티뉴어스 링을 처음 출시했을때만 해도 각각의 반지가 나름의 의미있는 선택지가 되길 바랬습니다. 하지만, 상위보스 리워드로 연마 기능까지 덧붙이면서 의도대로 되지 않았고, 이후 밸런스 패치를 하면서 소위 청기백기 현상이 발생해서 이 문제를 해결하고 싶었다."라고 설명했습니다.
...
이번 패치의 기조는 적절하다고 판단하나, 직업별 개성이 죽어버리는 상황이 생길 수 있으므로 직업별 개성에 맞게 밸런스 패치했으면 좋겠습니다.

추가적으로 스킬별 효율이 달라졌을거라고 판단되므로 헥사코어 초기화권을 지급해야 합니다.
...
12. 이벤트 버프에서 보스 공격 시 데미지 10% 증가가 없어지므로, 하이퍼 스탯에서 보스 공격 시 데미지 10% 제외하고 측정.
