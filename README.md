...
실전 데미지 부분에서 큰 체감을 한 부분이 '칼로스(보스)' 2페이즈입니다. 눈금 1칸을 파티원과 같이 오리진 스킬을 포함하여 극딜을 넣으면 50 ~ 60초 만에 다음 눈금이 열렸습니다. 그러나 개선 이후에는 준극딜까지 포함했음에도 딜이 부족한 상황이 발생했습니다. '칼로스(보스)'의 잦은 텔레포트로 인해 '돌아오는 증오' 스택도 쌓이지도 않고, 스킬 자체 적중율 또한 낮습니다. 이러한 현상은 실전 데미지 효율이 하락했음을 명백히 보여줍니다.
   - 눈금: 보스의 체력이 일정 수준 도달하면 체력이 깎이지 않고 보스 패턴이 나옴. 이 보스 패턴 파훼 시 다음 눈금 까지 체력을 감소시킬 수 있음. 파훼 실패 시 일정 체력 회복함.

'돌아오는 증오'의 기능 변화는 '세렌(보스)', '카링(보스)' 3페이즈에서는 좋았습니다. 맵 중앙에서 전투 시 '세렌'이 돌진 스킬을 사용하면 쌓아놓은 '징표'들을 맞출 방법이 없었습니다. 이로 인해 '세렌'을 유도하는 컨트롤(맵 바깥쪽으로 유인하는 행동)이 필수적이었습니다. 이 컨트롤을 했을 때/하지 않았을 때 데미지가 10% 이상 차이납니다. 패치 이후에는 보스를 유도할 필요가 없어져 입장 후 '세렌'의 돌진을 기다릴 필요가 없다는 점과 '세렌'을 맵 바깥쪽으로 유도할 필요가 없다는 점이 마음에 들었습니다. '카링' 3페이즈는 '블링크' 활용하여 최대 4명에게 적중할 수 있게 되었습니다.(기존 3명)
