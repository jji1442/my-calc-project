...
다음 패치에선 '증오의 연쇄'라는 스킬만 삭제하고, 헥사코어에서 '인피니티 스펠' 강화효과를 7%p에서 8%p로 올려주었으면 좋겠습니다.

'증오의 연쇄' 삭제해야 하는 이유는 스킬 자체의 개성이 없기 때문입니다. 극딜을 압축시키기 위해서 이 스킬을 냅두었다면, '증오의 연쇄' 버프인 징표 생성확률 100%는 '인피니티 스펠'의 사출량 증가 때문에 무의미하고 '돌아오는 증오' 징표 개수만 늘어나는데 이는 '돌아오는 증오'의 스킬 %데미지 올리는 것과 같은 효과입니다. 만약에 55초 극딜 빌드를 위해서 이 스킬을 냅두었다면, 기존 아크의 재미인 '인피니티 스펠'의 지속시간을 50초로 돌려주는게 낫습니다. 이와 같이, 스킬 자체가 다른 스킬로 대체가 가능하다면 삭제하는 것이 맞다고 봅니다.

그 중에서 '인피니티 스펠'의 강화효율 증가를 택한 이유는 전투경험개선 패치 이후로 '인피니티 스펠'의 효율이 크게 떨어졌기 때문에 이를 보완해주는 것이 맞다고 생각합니다.(9월 패치 기준으로 강화효율이 최소 30% 감소됨)
...
2. 현재 레벨 기준 스킬 %데미지 조정하는 함수 실행.
3. 강화 레벨 산출하는 함수 실행.
4. 강화효율 산출하는 함수 실행(%p 상승, 최종데미지 증가).
5. 코어별 일부 스탯 %p 상승 효율을 산출하는 함수 실행.
6. 100억당 최종데미지 상승율 산출하는 함수 실행.
7. 강화 추천하는 함수 실행. + if (함수 리턴값 == -1): break
8. 결과 출력하는 함수 실행.
9. 점유율 재조정하는 함수 실행.
10. 데미지 업데이트하는 함수 실행.
11. 위의 3 ~ 10를 반복.
...
100억당 최종데미지 상승율을 따로 산출하는 함수를 만들어야함. 스킬 %p 증가 + 스탯 %p 증가. 이 2가지가 합쳐져서 100억당 최종데미지 상승율을 산출해야하는 구조.
스킬 %p 증가 효율 산출하는 함수, 스킬 최종데미지 증가 효율을 산출하는 함수 -> 코어별 일부 스탯 %p 상승 효율을 산출하는 함수 -> 100억당 최종데미지 상승율을 산출하는 함수. 순서
최종데미지 상승율을 산출 -> 코어별 최종데미지 상승율에 합산됨. -> 최종데미지 상승율 * 재화 효율.

처음 함수에서 강화 레벨을 저장할 것.
