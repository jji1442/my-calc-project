...
3. '걷잡을 수 없는 혼돈VI', '채워지지 않는 굶주림VI' %데미지 대폭 상승.
4. '거스트 차지드라이브VI', '어비스 차지드라이브VI' %데미지 소폭 상승.
5. '영원히 굶주리는 짐승'의 총 %데미지 감소, 참격횟수 감소.
6. '끝없는 고통'의 총 %데미지 감소, 참격횟수 감소.
p.s. 참격횟수는 추가 공격횟수를 의미하며, 아크는 이 참격횟수에 따라 '다가오는 죽음'의 사출량이 증가함.

일단, 아크의 기본 구조가 바뀌었습니다.

110초 기준으로
기존 구조: 55초 스펙터, 55초 레프.
변경된 구조: 34초 스펙터, 21초 레프, 23초 스펙터, 22초 레프.
(게이지 부족 시 중간에 끊어줘야함.)
...
증오의 연쇄 사용 시, '돌아오는 증오'의 쿨타임이 초기화되게 설계된 의도를 "전혀" 알 수 없었습니다.
...
준극딜 버프와 '증오의 연쇄'의 쿨타임이 맞지 않는다는 점이 문제입니다.
