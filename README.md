...
크게 보면 2분 기준으로 60초 스펙터, 60초 레프 형식 입니다.
...
<제가 생각하는 아크의 문제점>
1. 헥사코어 효율 개선.
  - 5차 코어에선 유일하게 '인피니트 스펠'만 효율이 무난하여 아쉽다는 생각이 듭니다.
  - 연계 스킬들의 점유율이 낮아 헥사코어의 강화효율이 낮다는게 문제입니다.
2. 스킬 딜레이 개선.
  - 연계 스킬이다보니 스킬 자체 시전 딜레이, 시전 후 딜레이가 굉장히 길어 개선했으면 좋겠습니다.
3. '기어다니는 공포(키다운 무적기)' 시전 중 체력 감소 현상 삭제 및 공격 가능하게 변경.
  - 키다운 중 HP가 감소하는 리스크를 가진 대신 최대 5초간 무적이 가능하다는게 장점이 있지만, 스킬 키다운 중 공격을 못합니다.
  - 다른 일부 직업군들과 동일하게 시전 시 무적, 공격 가능하게 변경되었으면 합니다.
4. '차지 스펠 앰플리피케이션(레프 상태의 시전한 버프를 파티원에게도 제공하는 버프)' 시전 시 바로 파티원에게 버프 제공되게 변경.
  - '차지 스펠 앰플리피케이션' 시전 후 레프 상태에서 버프를 가진 스킬을 재시전해야 하는 과정이 불쾌해 바로 버프 지급 형식으로 변경되었으면 합니다.
5. 돌아오는 증오의 적중률이 낮아 적중한 대상에게 표식이 새겨지는 방식으로 변경.
...
8. '끝없는 고통' 2분화.
  - 쿨타임 1분 스킬인데, 준극딜에 사용해도 딜 상승량은 1.5%채 되지 않아 극딜 직업에 맞게 쿨타임 2분화했으면 좋겠습니다.
9. 스킬 사용 시 체력 소모량 감소.
  - 극딜할 때 마다 최대체력의 23% + 체력 3000(4%) 소모.
1. 스킬 딜레이 감소 + 상시 '전투광란'
...
<변경항목>
인피니티 스펠 지속시간을 30초로 압축.
신규스킬 1분 증오의 연쇄.
스펙터 연계스킬 중 혼돈VI, 굶주림VI %데미지 대폭 상승.
레프 연계스킬 중 거스트, 어비스 %데미지 소폭 상승.
영굶짐 총 %데미지 감소, 참격횟수 감소.
끝없는 고통 총 %데미지 감소, 참격횟수 감소.?
...
레프 연계스킬 중 거스트, 어비스 %데미지 소폭 상승.
다가오는 죽음의 딜 점유율 낮아진 만큼 보정해준 느낌인데, 마스터리 코어의 강화효율 올려준 것은 좋지만,
혼돈VI, 굶주림VI 이 2가지로 인해, "아크의 어빌리티를 스킬 사용 시 20% 확률로 쿨타임 초기화(일부 스킬 한정)을 사용해야하나?"라는 말 까지 나오고 있습니다.
최근 제 아크는 4년 만에 어빌리티를 종결했는데 다시 어빌리티를 맞춰야하는 상황이 왔습니다.
이 부분 또한 문제 있다고 봅니다.



추가적으로 8월 밸런스 패치한 주의 이벤트로는 어빌리티 50% 할인 해줬으면서
9월 밸런스 패치한 주의 이벤트로는 어빌리티 50% 할인 안해주는지도 굉장히 의문입니다.
게임사들이 캐릭터의 구조를 바꿔버렸으면 적어도 이에 맞는 이벤트를 해줘야한다고 생각합니다.
...
인피니티 스펠 지속시간 감소된 분 만큼 다가오는 죽음의 스킬 %데미지 상승량 대신,
연계 스킬들의 %데미지를 늘려주었습니다.

이로 인해 인피니티 스펠의 강화효율이 급격하게 낮아졌습니다.
...
2. 평딜 직업이다? 평딜하는 족쇄를 줄이면 됩니다.

아크를 예로 들자면,
아크는 극딜 압축이 안되어있는 직업군입니다.
쿨타임 2분 스킬이지만, 1분 동안 데미지를 넣는 직업군이죠.
평균적으로 20~30초 동안 극딜하는 일반적인 직업군들에 비해 다르긴 합니다.
하지만, 아크는 일부 직업군 다른 시스템이 있습니다.
바로 변신 시스템이죠.
변신 시스템의 특징은 변신 후에는 확실한 메리트, 변신 전에는 디메리트 어느정도 있어야한다고 봅니다.
변신 했을 때 강력한 이미지.
변신하기 전에는 기본적인 직업군 보다 약한 이미지.
아크는 이 구조를 정확하게 만들어져 있었습니다.
55초 스펙터(극딜) - 55초 레프
스킬 쓰는 재미라고 하죠?
'인피니티 스펠' 사용 시 사출량이 많아져 타격감이 좋습니다.
이 타격감 하나 때문에 아크를 한다가 과언이 아닐 정도였습니다.

하지만, 게임사에선 60초 준극딜을 쥐어주겠다고.
인피니티 스펠의 지속시간을 30초로 줄였습니다.
줄일 수 있죠.
