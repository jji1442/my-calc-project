...
'링 익스체인지'로 인한 코어 강화 효율 변화를 측정하고 패치에 대한 평가를 해보려고 합니다.<br>
...
기존의 아크는 1분 주기 버프만 사용할 때, 웨폰퍼프 링으로 변경하지 않았습니다. 왜냐하면 지속시간이 50초인 인피니티 스펠(2분 주기 버프)의 효용가치(점유율 약 27%)가 매우 높고, 특수 스킬 반지를 변경하기 위해선 전투를 5초간 멈춰야하는 단점이 있었기 때문입니다. 그리고 특수 스킬 반지를 변경한다고 해도 높은 피해량 또는 생존적인 이점 모두 얻기 어려웠기 때문에 웨폰퍼프 링으로 변경하지 않았습니다.<br>
<br>
'링 익스체인지'의 사용 방식.<br>
...
위를 위한 측정해봐야하는 종류(코어 강화 효율 변화를 보기 위해서)
1. 링 익스 체인지 '없이' 오리진을 포함한 6분 사이클.
1. 링 익스 체인지 '있이' 오리진을 포함한 6분 사이클.


아래를 위한 측정해봐야하는 종류(패치에 대한 평가하기 위해서)
1. 링 익스체인지 '없이' 오리진 포함한 2분 사이클 + 2분 사이클 (최종데미지 6% 증가를 제외하여 계산)
2. 최초의 대적자 링 익스 체인지 '있이' 오리진을 포함한 2분 사이클 + 2분 사이클.

<패치에 대한 평가>
25.08.21. 전투 경험 개선을 패치한 이유는 '최초의 대적자(이하 신규 보스)'에 의해 패치된 것으로 추정됩니다.

패치 이전, 제 아크는 허수아비형 보스에게 2분 기준 피해량 66조, 오리진 스킬 포함하여 피해량 81조를 기록했습니다.(증거 없습니다)
하지만 패치 이후, 신규 보스에서 측정된 딜량은 패치 이전 두 딜량의 90% 수준을 기록했습니다.

이러한 데이터를 미루어 보았을 때, '링 익스체인지' 스킬 추가 및 '최종데미지 6% 증가' 패시브 추가는 신규 보스의 실전 딜량과 허수아비형 보스 딜량의 괴리율을 줄이기 위한 의도였을 가능성이 높습니다.
아크는 평균적으로 허수아비형 보스 피해량의 85~90%를 뽑아냈지만, 이번의 패치가 없었다면 신규 보스에서는 70~75%로 딜량이 크게 하락했을 것으로 추정됩니다.
