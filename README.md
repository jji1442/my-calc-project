...
10월 22일 ~
10월 16일 테스트 서버 패치 내용을 보았습니다.
1. '아크'의 성의 없는 보완 패치.
2. 보스 수익의 너프폭과 너프구간.
3. '아즈모스 협곡(컨텐츠)'의 몬스터 체력 증가.
4. '와일드헌터(캐릭터)'의 리마스터.

아크에게 적용된 패치는 크게 보면 3가지입니다.
1. 게이지 회복속도 60% 증가.
2. '증오의 연쇄'의 지속시간 20초 -> 15초.
3. '증오의 연쇄' 사용 시 '돌아오는 증오' 쿨타임 초기화 기능 삭제.

아크를 짧은 시간 많은 데미지를 넣는 유형의 캐릭터로 만들 것이라면, '증오의 연쇄'의 지속시간 너프를 해선 안됐고,
지속적으로 데미지를 넣는 유형의 캐릭터로 만들 것이라면, '증오의 연쇄' 사용 시 '돌아오는 증오' 쿨타임 초기화 기능 삭제했으면 안됐습니다.

증오의 연쇄 지속시간을 너프한 것은 게이지 회복속도 증가로 인해 데미지가 높아질 것을 우려하여 너프를 먹인 것으로 보입니다.
하지만, 극딜 때 조차도 증오의 연쇄를 100% 활용하지 못한다는게 문제가 있다고 생각합니다.

기존: '돌아오는 증오'의 '징표' 스택을 15스택, 15스택 적중.
변경: '돌아오는 증오'의 '징표' 스택을 15스택, 12스택 적중.

'증오의 연쇄' 사용 시 '돌아오는 증오' 쿨타임 초기화 기능을 삭제해선 안됐던 이유는 단순 편의성에 의해서 입니다.
게이지 회복속도 60% 상승했다면, 돌아오는 증오를 활용하여 데미지를 넣을 겁니다.
돌아오는 증오는 증오의 연쇄 스킬 없이는 징표 자체가 잘 쌓이지 않을 뿐더러, 활용했다면 온몸 비틀어서 사용했을겁니다.
그러면 이에 대한 리턴이 있어야하는데, 다음 극딜에 사용하지 못할 수도 있다는 단점을 껴안아야 한다니 이건 문제가 있다고 생각합니다.
신직업인 렌은 극딜 버프에 망혼강림 초기화 같은 기능 잘 넣어줬으면서 기존 직업에는 왜 이렇게 처리했는지 의문입니다.
