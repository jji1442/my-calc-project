...
- [module] 강화 횟수를 산출하는 함수 기능 변경.
   ...
   - 부연 설명: 코어 레벨이 10 이라고 가정하면 기존에는 고정값은 1을 리턴하여 11, 12, 13, 14레벨의 효율을을 일일이 측정하였지만, 변경된 후에는 동적인 값인 4를 리턴하여 14레벨의 효율을 측정하도록 변경함. 비용적 이점 때문에 연속적으로 강화하는 것이 좋기 때문.
...
- [module] 강화 횟수를 산출하는 함수를 재귀함수화 및 최적화.
   ...
   - 최초 1회의 경우 모든 코어별 강화 횟수를 측정해야하므로 리스트를 매개변수로 전달. 강화된 코어는 강화 횟수를 재측정해야만 하기 때문에 코어만 매개변수로 전달. 리스트 내 코어가 있다는 점을 이용하여 함수를 재귀함수로 제작.
...
- [module] 강화 횟수를 산출하는 함수(for 스킬 코어) 추가.
   ...
   - 처리과정
      ...
      - 부가효과로 인한 데미지 상승율을 데미지 상승율 리스트에 합산 처리함.
   - 리턴값
      - 스킬 코어 리스트(리스트)
...
- [exception] 강화 횟수를 산출하는 함수(for 마스터리 코어) 추가.
    ...
    - 처리과정
         ...
         - 위 강화 순서로 인한 결과로 if문으로 생성함.
         - if문 조건에 따라 강화횟수가 산출됨.
   - 리턴값
      - 마스터리 코어 리스트(리스트)
================================================================================
</details>
