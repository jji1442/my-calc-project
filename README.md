...
#### 주관적인 평가
개인적으로 아크 패치는 굉장히 어렵다고 생각합니다. 전투경험개선 전 아크의 기본적인 실전 데미지가 110초 기준으로 첫 55초는 총 데미지의 90% 나머지 55초는 총 데미지의 10%가 나옵니다. 이 데미지를 분산 시키려면, 이번 10월달의 테스트 서버처럼(패치 보고서로 작성되지 않음) 스펙터 지속시간을 늘리는 패치를 할 수 밖에 없죠. 하지만 이 방식은 아크에게 치명적인 문제점을 야기합니다.

아크에게 60초 미만 스킬들은 연계가 가능한 스킬들과 '돌아오는 증오' 이 2가지 밖에 없기 때문에 데미지를 분산시킬 방법이 마땅히 없는 것도 문제입니다.
