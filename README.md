...
정식 업데이트 사항에는 작성되어 있지 않지만, '돌아오는 증오' 사용 시, 캐릭터를 따로 오는 기능을 롤백(스킬 시전 위치로 돌아오도록 변경)하였습니다.
이 변경은 아쉬움이 남았지만, 이 스킬 기능은 긴 극딜 시간 및 준극딜 생김으로 인한 긴 전투 방식 때문에 영향이 있었던 것이었고, 그 현상을 없앴기 때문에 롤백했다고 봅니다.

아크는 실전 데미지가 체감상 5 ~ 7%가 늘은 것 같습니다. 실전 데미지면에선 8월 패치로 돌아갔다고 생각합니다.

극딜 압축됐다는 점과 스펙터 연계 딜량이 늘었다는 점을 생각하면 실전에선 더 더욱 체감되었습니다.

실전 데미지 부분에서 큰 체감을 한 부분이 '칼로스(보스)' 2페이즈입니다. 눈금 1칸을 파티원과 같이 오리진 스킬을 포함하여 극딜을 넣으면, 30초 만에 다음 눈금이 열렸습니다. 물론 스펙적으로 성장한 부분이 있다는 점을 감안하더라도 체감이 될 정도로 강해졌습니다. 스펙터 연계 딜량이 상승함에 따라 칼로스 체력이 애매할 경우 준극딜 버프를 사용해서 넘겨도 된다는 점이 굉장히 좋았습니다.


그러나 개선 이후에는 준극딜까지 포함했음에도 데미지가 부족한 상황이 발생했습니다. 데미지가 부족한 이유는 '칼로스'의 잦은 텔레포트로 인해, '돌아오는 증오' 스택도 쌓이지도 않고 스킬 적중율 또한 낮기 때문이라고 생각합니다. 이러한 현상은 실전 데미지 효율이 하락했음을 명백히 보여줍니다.
...
- 강화 횟수 산출하는 함수 기능 변경.
 	- 기존: 임계점을 기반으로 고정값 반환함.
  	...
  	- 설명: 기존에는 임계점 도달 시 일정 레벨을 올리도록 함. 이 점의 문제점은 이 임계점이 아닐 경우 1레벨씩 강화효율을 측정하여 처리양이 많아지는 문제가 있었습니다. 이 문제를 동적인 값을 리턴하여 처리양을 줄였습니다.
  	- 부연 설명: 코어 레벨이 10 이라고 가정하면 기존에는 고정값은 1을 리턴하여 11, 12, 13, 14레벨의 효율을을 일일이 측정하였지만, 변경된 후에는 동적인 값인 4를 리턴하여 14레벨의 효율을 측정하도록 변경함.(비용적 이유로 인해 연속적으로 강화하는 것이 효율이 좋음)

- 강화 횟수 산출하는 함수(for 스킬 코어) 추가.
	- 매개변수
 		- 스킬 코어 리스트(리스트)
   		- 보스에게 적용되는 데미지(스탯) 리스트(리스트)
     	- 보스에게 적용되는 방어율 무시(스탯) 리스트(리스트)
   - 설명
   	- 스킬 코어의 강화 횟수를 산출하는 과정
   - 처리과정
     - 점유율 리스트, 스킬 %데미지로 인한 데미지 상승량 리스트 생성
     - 효율 측정.
...
- 강화 횟수 계산하는 함수 기능 확장.
	- 기존: 마스터리 코어 한정.
 	- 변경: 일부 코어(마스터리 코어, 강화 코어) 한정.
  	- 설명: 강화 횟수 측정하는 범위를 모든 코어로 변경하였습니다.
  		- enh_type이라는 변수를 통해, 스킬 계수 증가인지 스킬 계수 증폭인지 구병하였습니다.
  

- 직업별 초기 데이터를 가져오는 함수 추가.
	- 예외사항 처리를 위해서. 
...
- 각종 예외처리
	- 5차 강화 코어 중 스킬 계수 증폭(최종데미지 증가) 형식이 아닌 스킬 계수(스킬 %데미지 증가) 형식의 코어가 존재.
  		- 해결방안: 강화 종류를 나누는 방식을 채택.
...
예상되는 상황
코어를 추천사항 중에 만약에 보공, 방어율 무시가 증가한 코어가 강화됐다면 어떻게 처리할 것인가.
