...
#### 분석 방법
##### 분석 종류
1. 전투경험개선 후 아크(9월달 아크).
2. 후속 패치된 아크(10월달 아크).

#### 분석 결과
##### 1. 전투경험개선 후 아크(9월달 아크).
[>> 전투경험개선 후 아크 보러가기(제작 예정)](#)
...
이 변경으로 '증오의 연쇄'를 제외한 극딜 이후, '링 익스체인지'를 통해 '웨폰퍼프 링'으로 교체하여 '증오의 연쇄'와 함께 이어서 데미지를 넣는 새로운 빌드가 등장했습니다. 이는 과거 아크의 극딜 구조와 유사한 구조입니다. 이 빌드가 가능한 핵심 이유는 '인피니티 스펠'의 '다가오는 죽음' 사출량 증가로 인해 최대 징표까지 생성하는데 충분하기 때문입니다. 그러나 이 빌드는 링 익스체인지 기획의도(준극딜에 전투스킬반지를 편하게 변경하여 사용할 수 있도록)와 상충되어 후속 패치가 예상됩니다. 예상되는 조정 방안으로는 극딜 이후 연계 사용을 막기 위해 '징표 생성 확률 증가' 기능 삭제가 될 가능성이 높습니다.

'영원히 굶주린 짐승'의 충돌 횟수 증가와 '끝없는 고통'의 참격 횟수 증가는 '다가오는 죽음'의 사출량을 높여, 직업 고유 재미를 돌려놓는 긍정적 효과를 가져왔습니다.

#### 분석 결론
아크의 극딜 지속시간을 30초로 압축하려고 했던 것으로 보이나, 위와 같은 새로운 빌드로 인해 다시 원점(스펙터 55초 - 레프 55초 구조)으로 돌아왔습니다. 물론 상황에 따라 30초 안에 압축된 극딜도 가능하고, 만약에 허수아비형 보스를 상대한다면 긴 템포의 극딜이 가능한 구조가 되었습니다.

#### 주관적인 평가
개인적으로 아크 패치는 굉장히 어렵다고 생각합니다.

전투경험개선 전 아크의 기본적인 실전 데미지가 110초 기준으로 첫 55초는 총 데미지의 90%, 나머지 55초는 총 데미지의 10%가 나옵니다. 이 데미지를 분산 시키려면, 이번 10월 테스트 서버처럼(패치 보고서로 작성되지 않음) 스펙터 지속시간을 늘리는 패치를 할 수 밖에 없습니다. 테스트 서버의 패치에선 게이지 회복량 및 게이지 회복속도 증가를 주었습니다. 하지만 게이지 크기는 그대로니, 9월 패치 기준 스펙터 - 레프 - 스펙터 - 레프의 구조가 유지됩니다. 이 구조가 불쾌하다고 느끼는 이유는 연계 스킬들이 레프/스펙터 모드가 따로 존재하기 때문이라고 생각합니다. 변신한다고 스킬 쿨타임이 초기화되는 기능이 있는 것도 아니고, 변신에 선딜레이 및 후딜레이도 존재하고, 자주 변신하면 스킬도 헷갈리는 경우도 생기게 됩니다. 이 패치는 많은 반발로 인해 취소 되었습니다.

그러면 스킬 점유율 나누어 주어야하는데 아크에게 60초 미만 스킬들은 연계가 가능한 스킬들과 '돌아오는 증오' 이 2가지 밖에 없기 때문에 데미지를 분산시킬 방법이 마땅히 없습니다.
