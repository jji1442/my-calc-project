...
추가적으로 최초의 대적자의 '힘의 순환' 주기가 2분 15초 마다 이므로, 메인 패턴(검 소나기 패턴, 진짜 찾기 패턴)에는 공격하지 않는 것으로 시간에 의한 데미지 편차를 줄이도록 노력했습니다.
보스를 속박시키지 않고 '끝없는 고통' 스킬 사용 시 생존에 난해가 생겨....
최초의 대적자에서는 패턴 맞을 시 최종데미지 감소라는 단점이 존재하여 1분 버프와 함께 사용하는 '끝없는 고통' 스킬은 제외하기로 하였습니다.

 
측정해봐야하는 종류(패치에 대한 평가하기 위해서)
1. 링 익스체인지 '없이' 오리진 포함한 2분 사이클 + 2분 사이클 (최종데미지 6% 증가를 제외하여 계산) 4분 사이클
2. 최초의 대적자 링 익스 체인지 '있이' 오리진을 포함한 2분 사이클 + 2분 사이클. 4분 사이클
