...
- 강화 횟수를 산출하는 함수(for 스킬 코어) 추가.
   ...
   - 처리과정
      - 점유율 리스트, 스킬 %데미지로 인한 데미지 상승량 리스트 생성함.
      - 부가효과로 인한 데미지 상승율을 스킬 %데미지 리스트에 합산 처리함.

- [exception] 강화 횟수를 산출하는 함수(for 강화 코어) 추가.
   - 매개변수
      - 강화 코어 리스트(리스트)
   - 설명
      - 스킬 코어의 현재 레벨이 정해진 구간 내에 존재할 경우, 바로 a레벨로 가도록 산출하는 과정
   - 처리과정
