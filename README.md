...
<아크의 구조><br>
  ...
  110초 기준으로 55초간 스펙터 상태, 55초간 레프 상태를 유지하는 것이 핵심입니다.
...
8. '끝없는 고통' 쿨타임 2분화.
  - 문제점: 끝없는 고통은 현재 쿨타임 1분 스킬로, 준극딜에 사용되지만 데미지 상승량이 1.5%에도 미치지 못해 효율이 매우 낮습니다.
...
캐릭터의 전투 경험을 개선하는 방법은 간단합니다.
극딜형 직업의 경우 데미지를 압축하고, 평딜형 직업의 경우 데미지를 넣는 과정에서 방해하는 요소를 제거하면 됩니다.
아크의 경우, '인피니티 스펠'의 지속시간이 길지만 다른 직업이 사용하는 준극딜 버프를 극딜 버프 하나에 몰아서 사용한다고 생각하면, 극딜 압축은 필수적인 요소가 아닙니다.
아크는 데미지를 넣는 과정에서 방해하는 요소를 제거하는 것이 가장 합리적인 개선 방향이었습니다.
하지만 게임사는 이와 반대로, 아크의 극딜에는 조건형 스킬에 추가적인 제약을 더했고, 준극딜에는 조건형 스킬이 메인인 방식으로 변경하여 불합리한 구조를 만들었습니다.
이는 아크의 전투 경험을 불쾌하게 변경한 패치로 판단됩니다.

이로 인해 해당 직업의 핵심적인 재미가 훼손되었다고 판단하였고, 이 프로젝트의 심층적인 강화효율 측정 대상을 엔젤릭버스터로 변경했습니다.
...
테스트 종류
1. 55초 스펙터 / 55초 레프 구조.
2. 35초 스펙터 / 20초 레프 / 25초 스펙터 / 30초 레프 구조.
...
기존 빌드로 굴렸을 때, 장점 기존 빌드랑 동일함.
단점 증오의 연쇄를 굴릴 수 없음.

그 차이를 기준으로 데미지 차이를 보았을 때/
