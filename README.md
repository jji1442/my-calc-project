...
(위 이미지는 수정해야함)
위 결과 이미지에서 보시는 바와 같이, 이번 단계에서 새롭게 설계한 스킬 코어 및 강화 코어 데이터가 시스템에 정상적으로 추가되어 분석되고 있음을 확인할 수 있습니다.
...
오리진 스킬 코어는 9레벨, 나머지 코어는 30레벨로 설정하고  20레벨 부가 효과인 보스에게 적용되는 데미지 증가 배율 수치를 조정하여 스킬 추천 레벨이 어떠한 양상으로 달라지는지 아래의 표와 같이 확인하실 수 있습니다.
