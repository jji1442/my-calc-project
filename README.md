...
**<분석 결론>**<br>
  ...
  &emsp;&emsp;주관적인 평가)
  &emsp;&emsp;&emsp;스킬 사용하는 타격감과 우월한 유틸로 보스들을 유린시키는 재미에 이 2가지가 어우려져 아크를 내려놓지 못했습니다.
  &emsp;&emsp;&emsp;하지만, 이 중 높은 타격감 지속시간을 줄이고 일부 유틸리티 성능을 가져감으로써 아크에 대한 재미가 반감되어 플레이하고 싶다라는 생각이 들지 않습니다.
...
<br>
2. 강화효율 측정방식에 대한 일부 변경. + 처리속도 가속.
기존: 모든 구간에서 5레벨 상승 시 평균 최종데미지 상승율을 측정
변경: 9레벨, 19레벨에서만 5, 6레벨 상승 시 평균 최종데미지 상승율을 측정
설명: 9레벨, 19레벨 이외에는 재화 소모량에 따른 효율이 바뀌지 않으므로 특정 레벨에서 측정하게 변경하였습니다.
3. 스킬 %데미지 상승율, 재화 효율을 모듈화.
기존: 1, 5, 6레벨 상승에 따른 강화효율 측정.
변경: 통합하여 제작함.
설명: 스킬 %데미지(평균), 재화 효율(평균)을 따로 함수화하여, for문 돌리는 것이 효율적임을 생각해냈다.
4. 시제품용 데이터는 엔젤릭 버스터의 데이터를 기반으로 제작되었음을 알립니다.
...
### 시제품용 데이터는 최신화 됩니다. 각각의 제품 / 프로젝트의 기준 날짜를 확인해 주시길 바랍니다.
