...
### 기능 요구사항
#### 전제조건
1. 알파 프로젝트용 데이터는 25.11.01 개발자의 코어 강화 상태 및 허수아비형 보스 상대로 측정한 스킬별 점유율을 기준으로 측정함.
2. 기본 스킬의 %데미지는 ‘쓸만한 컴뱃 오더스’가 사용 중인 상태를 기준으로 측정함.
  - ‘쓸만한 컴뱃 오더스’는 ‘일부 스킬만’ 최대 레벨이 30레벨에서 31레벨로 상승하는 버프 스킬임.
3. 분석의 범위를 명확히 설정하기 위해서 ‘다가오는 죽음’은 ‘인피니티 스펠’ 사용 시에만 발생하는 경우를 기준으로 강화 효율을 측정함.
4. 마스터리 코어 효율 계산에 영향을 미치는 ‘강화 코어인 인피니티 스펠의 지속시간 중 다가오는 죽음 %p 상승’은 19레벨 기준으로 측정함.
   - ‘인피니티 스펠’은 50초간 ‘다가오는 죽음’이 3개 추가로 생성함. 이 스킬은 강화 코어의 효과를 받아 ‘다가오는 죽음’의 데미지를 133%p 추가로 상승하는 버프 스킬임.
...
### 구현 요구사항
#### 업데이트 사항
1. 코어별 강화 횟수, 강화 종류를 저장하는 변수 추가.
   - 기능: 강화 횟수 산출하는 함수 중복 사용 방지 장치 및 강화 로직 분기 수단.
   - 설명: 함수를 여러 번 실행하지 않도록 강화 횟수 측정하는 함수 횟수를 줄이고, 강화 종류는 강화 로직을 나 위한 장치임.

2. 강화 효율을 산출하는 함수에서 효율을 산출하는 함수로 변경.
   - 기존: 추천 레벨 달성 시 최종데미지 상승율, 100억당 최종데미지 상승율을 산출.
   - 변경: 추천 레벨 달성 시 최종데미지 상승율만 산출.
   - 설명: 정확한 효율을 측정하기 위해서 100억당 최종데미지 상승율을 산출하는 기능 분리.

3. 효율을 산출하는 함수 기능 확장 및 변수명 혼란 방지.
   - 기존: 스킬 %데미지(4차 이하 마스터리 코어, 6차 스킬 코어)
   - 변경: 스킬 계수(4차 이하 마스터리 코어, 6차 스킬 코어), 스킬 계수 증폭(5차 강화 코어)
   - 설명: 다양한 코어의 효율을 정확하게 측정하기 위해 함수 기능 확장. 기존 네이밍인 코어별 최종데미지 증가와 혼동이 오지 않도록 강화 코어의 최종데미지 증가는 스킬 계수 증폭으로 변경. 기존의 스킬 %데미지는 스킬 계수로 명칭을 통일.

코어별 사용될 스탯 수치를 저장하는 변수 추가.

6차 스킬 코어에는 상승되는 방어율 무시 변수 추가.

"상승했을 때만 호출되게 설정" 스킬에 부여된 방어율 무시 상승함에 따른 데미지 증가하는 수치 측정하는 함수.
기존 방어율 무시 효율에 비해서 얼마나 데미지가 더 들어갈 것인지 예측하는 함수인것이고.
기준점은 380% 보스 기준.

4. 부가 효과의 효율을 산출하는 함수 추가.
   - 기능: 해당 스킬에만 적용되는 스탯 상승하는 효과(6차 스킬 코어 패시브), 직업별 부가 효과의 효율을 산출.
   - 설명: 일정 레벨에 따라 해당 스킬 또는 전체에 영향을 미치는 스탯에 효율을 계산하기 위한 함수임.

5. 강화 효율 산출하는 함수 추가.
   - 기능: 추천 레벨 달성 시 100억당 최종데미지 상승율을 산출.
   - 설명: 추천 레벨 달성 시 최종데미지 상승율을 이용하여 추천 레벨 달성 시 100억당 최종데미지 상승율을 산출.

6. 코어 추천해주는 함수 기능 확장.
   - 기존: 4차 이하 마스터리 코어 한에서 효율 비교.
   - 변경: 효율 비교 대상을 모든 코어로 확장.

코어를 추천해주면서 만약에 보공, 방어율 무시가 증가한 코어가 강화됐다면 어떻게 처리할 것인가.


#### ※ 코드가 추가되는 부분만 작성하였습니다.
...
데미지, 보공 이 2가지는 합산 처리. 방무는 따로 처리.

기본 스탯
base_stats - total_dmg_up, defense_rate_ignore
...
##### 메인 함수
1. 테스트용 데이터를 자동으로 입력되는 함수 실행.
2. 강화 코어에서 %p를 사용하거나, 다른 속성 종류를 사용하는 경우를 위한 함수 실행.
3. 현재 레벨 기준 스킬 %데미지 조정하는 함수 실행.
4. 강화 레벨 산출하는 함수 실행.
5. 강화효율 산출하는 함수 실행(%p 상승, 최종데미지 증가).
6. 코어별 일부 스탯 %p 상승 효율을 산출하는 함수 실행.
7. 100억당 최종데미지 상승율 산출하는 함수 실행.
8. 강화 추천하는 함수 실행. + if (함수 리턴값 == -1): break
9. 결과 출력하는 함수 실행.
10. 점유율 재조정하는 함수 실행.
11. 데미지 업데이트하는 함수 실행.
12. 위의 3 ~ 10를 반복.
...
- 강화효율 산출하는 함수.
  ...
  - 처리사항
    ...
    - 추천 레벨 만큼 상승 시 최종데미지 상승율 = 점유율 * 스킬 데미지 상승율
    - 추천 레벨 만큼 상승 시 100억당 최종데미지 상승율 = 재화 효율 * 점유율 * 스킬 데미지 상승율
    - 마스터리 코어가 아니라면 아래를 실행함.
    - 강화 코어별 레벨에 따른 강화횟수를 산출함. 
    - 현재 레벨에 따라 1, 5, 6레벨 상승 시 강화효율을 측정함.
    - 스킬 데미지 상승율 = 스킬 %데미지 상승량(%p) / 현재 스킬 %데미지
    - 총 스킬 데미지 상승율 = 이 강화 코어를 5, 6레벨 연속으로 강화했을 경우, 스킬 데미지 상승율을 합산함.
    - 재화 소모량 = 강화 코어 1 ~ 30레벨 재화 소모량[현재 레벨]
    - 총 재화 소모량 = 강화 코어 1 ~ 30레벨 재화 소모량[현재 레벨 : 현재 레벨 + 강화횟수 - 1]
    - 재화 효율 = 100만 / (재화 시세 * 재화 소모량)
    - 추천 레벨 만큼 상승 시 최종데미지 상승율 = 점유율 * 스킬 데미지 상승율
    - 추천 레벨 만큼 상승 시 100억당 최종데미지 상승율 = 재화 효율 * 점유율 * 스킬 데미지 상승율
  - 리턴값
...
- 강화 추천하는 함수.
  - 매개변수
    - 마스터리 코어 리스트(리스트) 
  - 처리과정
    - 마스터리 코어별/강화 코어별로 추천 레벨 상승 시 100억당 최종데미지 상승율을 한 리스트에 담음.
    - 그 리스트에서 인덱스와 값으로 가장 효율 좋은 마스터리/강화 코어 인덱스를 유추함.
  - 처리사항
    - 최댓값이 0이면, -1를 리턴함.
      - 모든 스킬이 최대 레벨을 달성함.
    - 아니라면, 가장 효율 좋은 마스터리 코어 인덱스를 리턴함.
       - 가장 효율 좋은 마스터리/강화 코어 인덱스 = 최댓값인 인덱스
         - 최댓값인 인덱스 = 0 ~ 3이면, 마스터리 코어 인덱스.
            - 마스터리 코어, 최댓값인 인덱스를 리턴할 것.
         - 최댓값인 인덱스 = 4 ~ 7이면, 강화 코어 인덱스.
            - 강화 코어, 최댓값인 인덱스 - 3을 리턴할 것. 
  - 리턴값
    - 강화/마스터리 코어, 가장 효율 좋은 인덱스

- 결과를 출력하는 함수.
  - 매개변수
    - 강화/마스터리 코어, 가장 효율 좋은 인덱스
  - 처리과정
    - 강화할 스킬이 있다면 코어 종류, 코어 번호, 추천 레벨, 추천 레벨까지 상승 시 최종데미지 상승율, 추천 레벨까지 상승 시 100억당 최종데미지 상승율을 출력함.
  - 처리사항
    - 코어 번호 = 인덱스 + 1
  - 리턴값
    - 없음
