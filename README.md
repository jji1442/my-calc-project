...
6. 메르세데스 유니온 효과(스킬 쿨타임 6% 감소)를 적용하여 오리진 스킬(360초)의 실제 쿨타임은 338.4초임.
&emsp;측정 기준을 소수점 첫 번째 자리에서 반올림하여 5분38초(338초)로 설정.
&emsp;> 대부분의 유저들이 메르세데스 유니온 효과를 가지고 있지만, 스킬 쿨타임 감소 효과를 지닌 모자는 비교적 소수의 유저들이 사용하므로 제외하고 측정.
7. 어센트 스킬은 시간 내에 가장 딜로스 적은 구간에서 1회 사용.
8. 개발자 기준 모든 도핑 사용, 어센틱 포스 요구치 1, 보스 레벨 250, 방어율 380%, 체력 무한, 대형, 반감 기준으로 측정.
...
이로 인한 아크의 스킬별 점유율 변화는 어떻게 되었는지 확인해봅시다.

점유율 변화를 확인하기 위해 각 점유율을 높은 스킬을 기준으로 다음과 같이 선정하였습니다.
