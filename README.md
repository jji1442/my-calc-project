...
### 구조 2
- 설명
  - 직업별 스킬 정보는 데이터베이스에 저장되어 있으며, 사용자에게 필수 데이터(코어별 레벨, 해당 스킬 점유율, 재화 시세)만을 입력받아 다음 스킬 강화를 추천하는 구조.
- 이점<br>
  1.&nbsp;낮은 수동 입력.<br>
...
- 문제점<br>
  1.&nbsp;외부 API의 한계.<br>
  &emsp;- 넥슨 API에서 모든 스킬 정보를 주는지 명확하지 않습니다.<br>
  2.&nbsp;배포 및 접근성의 한계.<br>
...
- 구조를 변경하게된 이유
  - 넥슨 API의 잠재적 한계에 대비하여, 직접 웹사이트를 제작하여 스킬 정보를 수정할 수 있는 구조로 변경하였습니다.
...
- 전제조건<br>
  ...
  2.&nbsp;기본 스킬의 %데미지는 '쓸만한 컴뱃 오더스'가 켜져 있는 상태를 기준으로 작성함.
  3.&nbsp;'어비스 차지드라이브 스펠 VI'는 스킬 레벨에 따라 '대상에게 적중 시 보스 공격 시 데미지 증가'의 효과가 증가하지만, 시제외하고 강화효율을 측정함. 해당 효과는 복합적인 상호작용으로 인해 정확한 계산이 어려움.<br>
  4.&nbsp;분석의 범위를 명확히 설정하고, 특정 조건에서의 효율을 집중적으로 평가하기 위해서 '다가오는 죽음'은 '인피니트 스펠' 사용 시에만 발생하는 경우를 기준으로 강화 효율을 측정함.<br>
  5.&nbsp;마스터리 코어 효율 계산에 영향을 미치는 '강화 코어의 인피니트 스펠 사용 시 다가오는 죽음 %p 상승'은 19레벨 기준으로 계산함.<br>
