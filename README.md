...
<아크의 구조><br>
  ...
  아크는 특정 스킬을 기반으로 스킬을 연계하는 직업입니다. 이러한 스킬 연계 시스템은 크게 두 가지로 나뉩니다.
  레프 상태에선 '플레인'을 기반으로 '거스트', '스칼렛', '어비스', '충동', '악몽' 스킬과 연계합니다.
  스펙터 상태에선 '흉몽'을 기반으로 '상처', '흉몽', '굶주림', '본능' 스킬과 연계합니다.

<'다가오는 죽음'에 대한 설명>
  '다가오는 죽음'은 아크의 핵심적인 컨셉입니다.
  스펙터 상태에서 적에게 공격 시 '다가오는 죽음'이 사출되며, '다가오는 죽음'은 전체 딜량의 27%를 차지할 정도로 비중이 높습니다.
  많은 아크 유저들은 '다가오는 죽음'에 대한 높은 의존도 때문에, 다른 스킬들을 '다가오는 죽음'을 발사하기 위한 사전 작업으로 표현하곤 합니다.
  
<'돌아오는 증오'에 대한 설명>
  '돌아오는 증오'는 '다가오는 죽음'이 사출되어 적에게 적중했을 때 20% 확률로 '징표'가 생성되며, 이 '징표'를 회수하여 경로에 있는 적들을 공격하는 스킬입니다.
