...
&emsp;신규로 구현된 함수들 중에서 중요한 로직을 담당하는 함수인 calc_enh_cnt_for_skill_core_list()에 대한 설명 드리겠습니다.<br>
위 함수 세부 설계도에서는 연한 갈색의 테투리로 표기하였습니다.

&emsp;calc_enh_cnt_for_skill_core_list()라는 함수는 스킬 코어 리스트(오리진 스킬, 어센트 스킬)를 위한 강화 횟수 계산하는 함수입니다.<br>
따로 계산하는 이유는 스킬 코어에 10레벨 단위로 보스에게 적용되는 데미지 증가 또는 방어율 무시 증가인 부가 효과의 효율을 측정하여 먼저 강화할 가치가 있는지 확인하기 위해서입니다.
