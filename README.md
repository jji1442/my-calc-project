...
&emsp;부가 효과의 효율이 비정상적으로 높으면 비용적 측면의 결과값이 '19레벨 -> 20레벨' 구간에 수렴하게 됩니다.<br>
이 과정에서 데미지적 측면의 효율이 상대적으로 과소평가되어 강화 순서가 역전되는 현상이 발생합니다.<br>
이를 개선하고자 비용적 측면의 결과가 '19레벨 -> 20레벨'로 도출될 경우, 강화 단계의 논리적 선후 관계를 유지하기 위해 데미지적 측면의 결과값을 적용하도록 설정하였습니다.<br>
아래의 예시는 오리진 스킬 코어는 9레벨, 나머지 코어는 30레벨로 설정하고 20레벨 부가 효과인 보스에게 적용되는 데미지 증가의 배율을 3000%로 조정하여 부가 효과의 효율이 비정상적으로 높게 설정하였습니다.
...
&emsp;20레벨의 효율이 앞뒤 레벨에 비해 비정상적으로 높을 경우, 위 조건을 넣지 않았을 경우 비용적 측면에서 '19레벨 -> 20레벨'이 선택되어 20레벨의 달성 가치를 '단일 레벨'로만 판단하여 19레벨 이하 구간 효율에 의해 20레벨 추천 순위가 뒤로 밀려버리는 논리적 오류가 생깁니다. 이미지 2는 이러한 구간 효율의 함정을 보정하여, 유저가 최종적으로 20레벨이라는 목표에 합리적으로 도달할 수 있도록 강화 경로를 매끄럽게 연결한 결과입니다.
