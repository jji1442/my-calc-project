...
위 함수 세부 설계도는 프로그램의 제어 흐름을 나타내는 실선과 함수 간의 의존성을 의미하는 점선을 통해 전체 로직을 구조화했습니다. 이번 알파 프로젝트로 구현된 함수는 빨간색 원으로 강조하였습니다.

추가 구현된 함수들 중에서 중요한 로직을 담당하는 함수에 대한 설명드리겠습니다.

calc_enh_cnt_for_skill_core_list()라는 함수는 스킬 코어 리스트(오리진 스킬, 어센트 스킬)를 위한 강화 횟수 계산하는 함수입니다.
따로 계산하는 이유는 스킬 코어에 10레벨 단위로 보스에게 적용되는 데미지 증가 또는 방어율 무시 증가인 부가 효과의 효율을 측정하여 먼저 강화할 가치가 있는지 확인한 후, 강화횟수를 반환하는 방식입니다.

먼저 강화할 가치를 따지는 이유는 비용적 이점과 데미지적 이점. 이 2가지 중에서 어떠한 것이 가치가 높은지 비교하는 겁니다.
비용적 이점의 경우에는 10레벨 달성 시 급격하게 재화 소모량이 증가하고, 11레벨 부터는 비교적 적은 재화 소모량을 토대로 '9레벨 -> 14레벨'의 효율이 좋은가?
데미지적 이점으로 인해 먼저 강화할 이유가 있다면, 선제적으로 앞의 레벨과 효율을 비교하여 효율을 측정합니다.
예를 들자면, '8레벨 -> 10레벨'의 효율 vs. '8레벨 -> 9레벨'의 효율.
이 2가지의 수치를 비교하여 효율이 좋은 쪽을 가려내는 겁니다.
전자가 좋다면 이어서, '7레벨 -> 10레벨'의 효율 vs. '7레벨 -> 8레벨'의 효율을 측정하는 구조로 앞의 구간과의 효율을 비교하는 구조입니다.
후자가 좋다면, 데미지적 이점에서 최종적으로 '9레벨 -> 10레벨'의 효율이 좋다는 것을 의미하기 때문에 앞의 구간과의 효율을 비교하는 반복은 무의미하므로 값을 반환합니다.

데미지적 이점으로 인한 최종적인 결과와 비용적 이점의 비교를 통해 최종적인 스킬 레벨 강화 횟수를 산출하는겁니다.

자, 여기서 의문점이 하나 들겁니다. "데미지적 이점이 커서 '8레벨 -> 10레벨'의 효율이 좋다는 결과가 산출됐다면, '9레벨 -> 10레벨'의 효율에 비해 효율이 떨어지지 않나요?"
그 부가 효과가 재화 소모량에 비해 효율이 좋다면 효율이 떨어질 가능성이 있습니다.
다만 그 부가 효과가 효율이 좋다면, '8레벨 -> 9레벨'의 효율 보다 '8레벨 -> 10레벨'의 효율이 좋아야만 먼저 강화할 가치가 있습니다.
'8레벨 -> 9레벨'의 효율 보다 효율이 안좋다면, 이 스킬 %데미지 상승량의 비해 & 비용적 상승량에 비해 10레벨 까지 강화할 이유가 없다는 겁니다.

"이 효율이 떨어진 상태로 데미지적 이점과 비용적 이점의 비교를 하면 우선 순위에서 밀리는 것 아닌가요?"라는 질문을 하나 더 던지게 될 겁니다.
하지만 '8레벨 -> 9레벨'의 효율이 '10레벨 -> 11레벨'의 효율보다 항상 좋다는 점을 이해한다면 이 질문은 자연스럽게 해결됩니다.
스킬 레벨이 올라갈수록 현재 스킬 %데미지양이 늘어나고, 고정적인 스킬 %데미지 상승에 대한 이점은 점차 줄어들게 됩니다.
스킬 레벨이 올라갈수록 또한 현재 재화 소모량이 증가하며 재화 효율에 대한 이점도 점차 감소되므로, 비용적 이점에 비해 데미지적 이점에서 이전 레벨과의 비교는 유효함을 의미합니다.
...
그리고 산출된 추천 레벨들을 정렬합니다.
이 추천 레벨이 서로 겹치는 구간이 있을 가능성(부가 효과의 효율이 좋은 경우)이 있습니다.
