...
- 환산주스탯(=효율분석 사이트)의 특징<br>
  ...
  4.&nbsp;고정된 강화 순서 제공.<br>
  &emsp;- 헥사 코어 레벨에 따라 달라지는 강화 효율을 고려하지 않고 고정된 강화 순서만을 제공합니다.
...
- 문제점<br>
  ...
  &emsp;- 특히 스킬별 1회당 공격횟수 같은 세부 정보를 모두 기억하기 어려워 잘못된 정보 기입 시 잘못된 효율이 측정되는 경우가 발생합니다.<br>
...
### 제작 기한
  ...
  - 시작일: 25.08.14.
  - 기한일: 25.08.26.
  - 완성일: 25.09.03.(영업일 기준 14일)

### 기능 요구사항
- 전제조건<br>
  1.&nbsp;25.08.14 개발자 '아크의 마스터리 코어'를 기준으로 개발함.<br>
  2.&nbsp;다가오는 죽음은 인피니트 스펠 사용하지 않아도 생성되는 사출기이지만, 시제품이라 인피니트 스펠 사용 시 기준으로 강화효율을 측정함.<br>
  3.&nbsp;마스터리 코어 효율 계산에 영향을 미치는 '강화 코어의 인피니트 스펠 사용 시 다가오는 죽음 %p 상승'은 19레벨 기준으로 계산함.<br>
  4.&nbsp;기본 스킬의 %데미지는 '쓸만한 컴뱃 오더스'가 켜져 있는 상태를 기준으로 작성함.
...
- 간단 후기
문서화 하는 과정 및 코드 최적화 과정이 포함된 기간으로 영업일 기준 14일이 소요되었습니다. 문서화하는 작업이 전체 기간 중의 70~80%를 차지하였고. 어떠한 문서를 작성해야 할지 고민이 많이 되어 예상된 일정보다 지연되었습니다. 예상 평균 기간은 실제보다 짧게 산정했으나, 최악의 경우를 고려한 최대 기간 예측은 실제와 정확히 일치하여 일정 지연 리스크를 최소화할 수 있었다.
