...
1. 내용은 지속적으로 추가 및 보완될 예정입니다. 특히 게임 관련 용어의 경우, 가독성을 위해 별도의 설명 없이 사용되었습니다.<br>
2. 프로젝트의 완전한 이해를 돕기 위한 상세한 게임 시스템 및 용어 설명은 추후 보충될 예정이니, 이 점 참고하여 주시기 바랍니다.<br>
3. 모든 작업은 제품의 기능을 우선으로 개발되었습니다. 추후 개선 작업을 통해 최적화할 예정입니다.<br>
4. 프로젝트 단계별 완성일은 개발자가 그 당시 보고서가 완성되었다고 판단한 시점으로 작성되었습니다.<br>
5. 완성일 이후에 보고서 내용의 일관성을 위하여 일부 내용 추가 및 수정되었습니다.<br>
6. 단계별 사용된 테스트용 데이터는 최신화 됩니다. 각각의 제품 / 프로젝트의 기준 날짜를 확인해 주시길 바랍니다.
...
- 시작일: 25.10.31.
- 기한일: 25.12.26.
- 완성일: 26.02.23.(영업일 기준 77일)

### 기능 요구사항
#### 전제조건
1. 알파 버전용 데이터는 25.11.11 개발자의 코어 강화 상태 및 허수아비형 보스 상대로 측정한 스킬별 점유율을 기준으로 측정함.
...
위 함수 세부 설계도에서 연한 갈색의 박스로 표기하였습니다.

&emsp;calc_enh_cnt_for_skill_core_list()라는 함수는 스킬 코어 리스트(오리진 스킬, 어센트 스킬)에 대한 강화 횟수(강화 순서)를 계산하는 함수입니다.<br>
따로 계산하는 이유는 스킬 코어에 10레벨 단위로 보스에게 적용되는 데미지 증가 또는 방어율 무시 증가로 인한 데미지 증가가 강화 순서에 영향을 주는지 확인하기 위함입니다.

&emsp;이 함수의 기본 방식은 비용에 따른 데미지 효율의 평균값을 구해서 어떤 강화 순서가 가장 효율이 좋은지 확인하는 방식입니다.<br>
강화 순서 종류는 부가 효과의 가치가 높아서 먼저 강화하는 경우와 부가 효과의 가치가 낮아서 비용적 측면이 고려되는 경우.<br>
이 2가지며, 이 2가지를 비교하여 강화 순서를 도출하고 강화 횟수를 산출하는 방식입니다.

&emsp;먼저 강화하는 것이 이점이 큰 지 확인하는 방법은 부가 효과가 부여되는 레벨의 이전 레벨과 효율을 비교하여 우열을 가려내 결과를 도출합니다.<br>
예를 들자면, '8레벨 -> 10레벨'의 효율 vs. '8레벨 -> 9레벨'의 효율. 이 2가지의 수치를 비교하여 효율이 좋은 쪽을 가려냅니다.<br>
전자가 좋다면, 이어서 '7레벨 -> 10레벨'의 효율 vs. '7레벨 -> 8레벨'의 효율을 측정하는 구조로 앞의 구간과의 효율을 비교합니다.<br>
후자가 좋다면, '8레벨 -> 9레벨'의 효율 보다 좋지 않으니 데미지적 측면에서는 '9레벨 -> 10레벨'라는 결과를 내놓게 되는 구조입니다.

&emsp;비용적 측면 또한, 데미지적 측면과 비슷한 형식으로 부가 효과가 부여되는 레벨의 다음 레벨과 효율을 비교하여 우열을 가려내 결과를 도출합니다.<br>
예를 들자면, '9레벨 -> 11레벨'의 효율 vs. '10레벨 -> 11레벨'의 효율. 이 2가지의 수치를 비교하여 효율이 좋은 쪽을 가려냅니다.<br>
...
&emsp; 좌측 이미지는 비용적 측면에서 '19레벨 -> 20레벨'이 선택되어 20레벨의 달성 가치를 '단일 레벨'로만 판단하여 19레벨 이하 구간의 효율에 의해 20레벨 추천 순위가 뒤로 밀려버리는 논리적 오류가 생깁니다. 우측 이미지는 강화 단계의 논리적 선후 관계를 유지하여, 유저가 최종적으로 20레벨이라는 목표에 합리적으로 도달할 수 있도록 강화 경로를 매끄럽게 연결한 결과입니다.

&emsp;두 측면 중에 효율이 좋은 쪽 하나가 선택될 것이고. 이 과정을 20레벨 및 30레벨 기준으로 2번 더 반복합니다.

&emsp;그리고 산출된 강화 순서들을 정렬합니다.<br>
이 강화 순서가 서로 겹치는 구간이 있을 가능성(부가 효과의 효율이 좋은 경우)이 있습니다.<br>
그럴 경우에는 두 레벨 구간 간의 효율을 비교합니다.<br>
그 중 더 높은 효율을 기준으로 강화 순서를 정렬합니다.<br>
예를 들어, '19레벨 -> 25레벨'과 '24레벨 -> 30레벨'이 효율 좋다고 되었다고 가정하면 위 2가지의 효율을 비교하여<br>
전자가 좋다면, 최종적으로 강화 순서를 '19레벨 -> 25레벨 -> 30레벨'<br>
후자가 좋다면, '18레벨 -> 30레벨'의 효율 vs. '18레벨 -> 19레벨'의 효율. 이전 데미지적 이점의 과정을 반복합니다.<br>
이러다 보면 10레벨 강화 순서 구간과 겹치는 가능성이 존재할 수 있으니, 두 레벨 구간 간의 효율을 비교할 수 있도록 구성합니다.

&emsp;그리고, 각 강화 순서 구간 별로 if문을 만든 대신 레벨이 높은 구간 순으로 강화 횟수를 산출하도록 if문을 설계하여 겹치는 가능성에 의해 강화 순서가 변하지 않도록 설계하였습니다.

&emsp;다음은 이 함수에 대한 검증 절차입니다.

&emsp;오리진 스킬은 9레벨, 나머지 코어는 30레벨로 설정하고 20레벨 부가 효과인 보스에게 적용되는 데미지 증가의 배율 수치를 조정하여 오리진 스킬 추천 레벨이 어떠한 양상으로 달라지는지 아래의 표로 정리하였습니다.
...
&emsp;배율 20% 및 500% 에서는 부가 효과에 대한 이점보다 비용적 이점이 더 크게 작용하는 양상을 보였으며, 이로 인해 20레벨을 빠르게 달성하기보다는 강화를 후순위로 강화하는 것이 효율이 좋다라는 결과가 도출되었습니다. 배율 800% 에서는 부가 효과에 대한 이점과 비용적 이점이 동일한 구간에 도달하여, 모든 단계를 순차적으로 강화하는 방식이 최적의 선택으로 나타났습니다. 이후 배율 3000% 에서는 부가 효과에 대한 이점이 비용적 이점을 명확히 상회하며, 20레벨을 빠르게 달성하는 것이 가장 효율적인 결과를 보였습니다.
...
처음으로 강화를 시작하는 부분에 마스터리 코어는 연두색, 강화 코어는 노란색, 스킬코어는 주황색으로 표시하였습니다.
...
### 패치 내용
게임사에서 액티브 반지 2개를 바꾸어서 사용하는 방식과 패시브 반지 1개만 사용하는 방식에서 패시브/액티브 반지 각각 1가지씩 사용할 수 있도록 변경하였습니다.
...
##### 1. 패치 전/후 주력 반지의 변화.
[패치 전 주력 반지]<br>
리스트레인트 링(4레벨): 15초 동안 생성되는 영역 안에서 자신의 공격력이 80% 증가.<br>
(재사용 대기시간 120초)<br>
웨폰 퍼프 - S링(4레벨): 15초 동안 장착중인 주무기 공격력의 320%만큼 str증가<br>
(재사용 대기시간 120초)<br>
<br>
컨티뉴어스 링(4레벨): 공격 스킬 사용 시 8초간 보스 몬스터 공격 시 데미지 140% 증가, 공격력 10% 증가.<br>
(재사용 대기시간 12초)<br>

[패치 후 주력 반지]<br>
컨티뉴어스 링(4레벨): 공격 스킬 사용 시 "30초 동안" 보스 몬스터 공격 시 데미지 "36%" 증가, 공격력 10% 증가.<br>
(재사용 대기시간 "없음")<br>
리스트레인트 링(4레벨): 15초 동안 생성되는 영역 안에서 자신의 공격력이 "68%" 증가.<br>
(재사용 대기시간 120초)<br>

##### 2. 반지 개편에 대한 설명.
패치 전에는 극딜 직업군들은 리스트레인트 링과 웨폰퍼프 링을 2가지를 사용했었고, 평딜 직업군들은 컨티뉴어스 링을 사용했었습니다.

게임사에선 "컨티뉴어스 링을 처음 출시했을때만 해도 각각의 반지가 나름의 의미있는 선택지가 되길 바랬습니다. 하지만, 상위보스 리워드로 연마 기능까지 덧붙이면서 의도대로 되지 않았고, 이후 밸런스 패치를 하면서 소위 청기백기 현상이 발생해서 이 문제를 해결하고 싶었습니다."라고 설명했습니다.

- 청기백기 현상: 평딜 직업이었다가 극딜 직업으로 바뀌어서 반지를 새로 사야하는 상황.

컨티뉴어스 링의 출시 의도 자체는 좋았으나, 극딜 직업들은 반지 2개를 구매 및 연마해야 하고 평딜 직업은 반지 1개를 구매 및 연마해야 한다는 점에서 재화 사용량의 차이는 밸런스 패치를 어렵게 만드는 근본적인 요인이었습니다.

#### 분석 결론
이번 패치는 비용적 불균형을 해소하고, 밸런스 패치로 인한 유저들의 '어떤 반지를 쓸까?'에 대한 고민 및 추가적인 투자를 방지한다는 측면에서 적절한 개선 방향이라고 생각합니다.

#### 주관적인 평가
과거에는 180초 주기를 가진 극딜 직업들 조차도 긴 극딜 주기의 공백을 메워주어 가동률이 높은 컨티뉴어스 링을 사용하였지만, 전 직업이 120초 주기로 통일되고 극딜 시간이 압축되면서 리스트레인트 링과 웨폰퍼프 링을 컨티뉴어스 링이 따라잡기 힘든 구조가 되었습니다.

여기에 더불어, 컨텐츠에 대한 보상으로 보스 공격 시 데미지 증가를 지급하여 컨티뉴어스 링의 가치가 더 낮아지는 현상이 발생하여 유저들은 리스트레인트 링의 가치를 더욱 더 높게 평가하게 되었습니다.

이로 인해, 스킬별 효율이 달라졌을거라고 판단되므로 헥사코어 초기화권을 지급해야만 합니다.
...
8. 개발자가 보스전에서 사용하는 물약 설정. 필드는 어센틱 포스 요구치 1. 보스는 레벨 250, 방어율 380%, 체력 무한, 대형, 반감 기준으로 측정.
   - 길드 스킬의 방어율 무시 버프, 콜렉터의 영약, VIP 버프, MVP 슈퍼파워, 붕어빵 뿌리기, 챔피언의 가호 제외.
9. 총 3회 측정하고, 평균값 낸 수치를 작성.
10. 어빌리티는 보스 공격 시 데미지 증가 / 상태 이상에 걸린 적 공격 시 데미지 증가 / 버프 스킬 지속 시간 증가 기준으로 측정.
    - 재사용 어빌리티는 데미지 편차가 심하므로 사용하지 않음.
11. 이벤트 버프에서 보스 공격 시 데미지 10% 증가가 없어지므로, 하이퍼 스탯에서 보스 공격 시 데미지 10% 제외하고 측정.
    - 패치 전 하이퍼 스탯: 데미지 증가 10레벨, 보스 공격 시 데미지 증가 14레벨. (30% + 51% = 41%)
    - 패치 후 하이퍼 스탯: 데미지 증가 12레벨, 보스 공격 시 데미지 증가 15레벨. (36% + 55% = 51%)
...
데이터상으로 '근원의 기억' 및 '솔 헤카테: 스틱스'의 점유율이 하락한 것은 '웨폰퍼프 링' 미사용으로 인해 해당 스킬들의 고점이 낮게 책정된 결과입니다. 이에 따라 '잊혀지지 않는 흉몽VI'의 점유율이 소폭 상승한 것처럼 보이나, 이는 패치 전 데이터의 최소\~최대 편차 범위 내에 머무르는 수준입니다.
...
연계되는 스킬들의 데미지는 기존과 차이가 없음을 알 수 있습니다.

#### 분석 결론
패치 전에 비해 전체 데미지가 5.49% 상승하였고, '웨폰퍼프 링'의 효과가 집중되던 주요 스킬들의 비중은 상대적으로 낮아졌음을 알 수 있었습니다.
...
3. '매서풀VI' 0레벨 + 기존 사이클 vs. '매서풀VI' 1레벨 + 기존 사이클.
...
아래의 이미지는 '매서풀'(또는 '매서풀VI') 및 '인피니티 스펠'을 사용 후, '잊혀지지 않는 흉몽VI'를 10번 사용한 결과입니다.

1. '매서풀'
<img src="patch_analysis_report_26.04.16/mcfd_spec_change/mcfd.PNG">

2. '매서풀VI'
<img src="patch_analysis_report_26.04.16/mcfd_spec_change/mcfd_vi.PNG">
...
'매서풀VI' 0레벨 + 기존 사이클 보다 '매서풀VI' 1레벨 + 기존 사이클이 0.567% 낮은 현상이 발생하였습니다.

다음은 일부 스킬들의 평균 사용횟수를 추출하였습니다.
...
120초 마다 '다가오는 죽음'이 최대 36회 발동 되지 않으며, 360초 기준으로는 3번 사이클을 돌리기 때문에 36회 * 3번 = 108회.

'다가오는 죽음' 스킬 %데미지와 '매서풀' 스킬 %데미지를 합산하여 '매서풀VI'의 스킬 %데미지가 어느 레벨 대에서 기존 성능과 비슷해질지 확인해 보았습니다.

'매서풀VI'는 다음과 같이 수치가 상승합니다.

[공용코어로 출시된 '매직 서킷 풀드라이브 VI']<br>
MP 1000 소비, 60초 동안 데미지 45% 증가, 공격력 '10 + 3 * 스킬레벨', 마력 '2 + 3 * 스킬레벨' 증가.<br>
지속시간 동안 일정 범위 내 최대 10명의 적에게 마력 표식을 새김.<br>
마력 표식이 있는 적에게 자신의 직접 공격 스킬 적중 시 '272 + 24 * 스킬레벨'% 의 데미지로 6번 공격하는 마력 폭풍 발생.<br>
마력 폭풍 재발동 대기시간 1초.<br>
(재사용 대기시간 120초)

'다가오는 죽음' 수치 및 영향을 미치는 항목은 다음과 같습니다.

[4차 이하]: '다가오는 죽음' 데미지 450% * 2번.<br>
[6차]: '잊혀지지 않는 흉몽VI' 27레벨의 패시브 '다가오는 죽음' 데미지 54%p 증가.<br>
[6차]: '인피니티 스펠' 16레벨의 지속시간 중 '다가오는 죽음' 데미지 112%p 증가.<br>
[5차]: '다가오는 죽음' 최종데미지 120% 증가.

'매서풀'로 인한 누적 %데미지<br>
= '매서풀' 스킬 %데미지 * 사용횟수 + '다가오는 죽음' 스킬 %데미지 * 사용횟수<br>
= (1100 * 3) * 47.6 + ((450 + 54 + 112) * 2 * 2.2) * 121.4<br>
= 486,122.56(48만 6122.56%)

'매서풀VI' 1레벨 스킬 %데미지<br>
= '매서풀VI' 스킬 %데미지 * 사용횟수<br>
= (296 * 6) * 168<br>
= 298,368(29만 8368%)<br>
[추가 효과]: 60초 동안 공격력 13
...
'매서풀VI' 0레벨은 '매서풀VI' 8레벨 보다 누적 %데미지가 더 높다는 점을 알 수 있습니다.<br>
공격력 상승치에 따른 데미지 상승량을 고려하더라도 '매서풀VI' 6레벨에서야 비로소 기존 성능과 동일해질 것으로 예상됩니다.

#### 분석 결론
핵심 스킬을 최대 레벨(30레벨) 달성한 이후 나머지 효용감 없는 스킬들을 강화할 경우에, '매서풀VI' 10레벨 이상 강화하는 것을 권장합니다.

<details>
<summary> 분석 결과 확인하기(펼치기/닫기)</summary>

핵심 스킬을 최대 레벨(30레벨)로 달성하였을 경우, '매서풀VI'의 가치.

[4차 이하]: '다가오는 죽음' 데미지 450% * 2번.<br>
[6차]: '잊혀지지 않는 흉몽VI' 30레벨의 패시브 '다가오는 죽음' 데미지 60%p 증가.<br>
[6차]: '인피니티 스펠' 30레벨의 지속시간 중 '다가오는 죽음' 데미지 210%p 증가.<br>
[5차]: '다가오는 죽음' 최종데미지 120% 증가.

'매서풀'로 인한 누적 %데미지<br>
= '매서풀' 스킬 %데미지 * 사용횟수 + '다가오는 죽음' 스킬 %데미지 * 사용횟수<br>
= (1100 * 3) * 47.6 + ((450 + 60 + 210) * 2 * 2.2) * 121.4<br>
= 514,675.2(51만 4675.2%)

'매서풀VI' 10레벨 스킬 %데미지<br>
= '매서풀VI' 스킬 %데미지 * 사용횟수<br>
= (512 * 6) * 168 = 516,096(51만 6096%)<br>
[추가 효과]: 60초 동안 공격력 40
</details>

#### 주관적인 평가
주변인이 "그래서 '매서풀VI'을 몇 레벨까지 찍어야 효율이 나오냐?"라고 물어보신다면, 저는 환산주스탯 기준 '매서풀VI' 14레벨부터 강화하는 시점을 적극 추천합니다. 이 시점부터 투자비용 대비 스킬 %데미지의 손해를 최소화하는 동시에 레벨업에 따른 본격적인 딜 상승 이득을 기대할 수 있기 때문입니다.

#### 추가 테스트
'매서풀VI' 9레벨 기준으로 테스트하여 '매서풀VI'의 가치를 측정해보겠습니다.
...
'매서풀VI'의 가치를 구하기 위해선 다음의 과정을 거칩니다.<br>
1단계: '매서풀VI' 스킬 %데미지 상승분 구하기.<br>
2단계: '매서풀VI' 스킬 한정 영향을 미친 추가효과 상승분 구하기.<br>
3단계: '매서풀VI' 스킬을 제외한 나머지 스킬에 영향을 미친 추가효과 상승분 구하기.<br>
4단계: '매서풀VI' 스킬이 영향을 미친 추가효과 상승분 구하기.<br>
5단계: 정리.

##### 1단계: '매서풀VI' 스킬 %데미지 상승분 구하기.

'매서풀VI' 1레벨과 '매서풀VI' 9레벨 각각의 스킬 자체가 입힌 누적 데미지 및 사용횟수를 산출합니다.<br>
- 매서풀VI 1레벨 및 9레벨의 스킬 자체 누적 데미지(평균)와 사용 횟수(평균)<br>
...
실전 사용횟수 오차를 통제하기 위해 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지에 보정(0.962%)하여 두 레벨 간의 스킬 %데미지 차이(64.865%)를 적용하여 계산하면 다음과 같습니다.

사용횟수 보정치<br>
= 매서풀VI 1레벨 사용횟수 / 매서풀VI 9레벨 사용횟수<br>
= 168회 / 166.4회<br>
= 1.00962(-> 100.962%)

"보정된" 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지<br>
= 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지 * 사용횟수 보정치<br>
= 8조 5103억 * 100.962%<br>
= 8조 5921억

두 레벨 간 스킬 자체가 입힌 누적 데미지 예측치<br>
= 매서풀VI 1레벨 스킬 자체가 입힌 누적 데미지 * 1 + 스킬 %데미지 상승율<br>
= 5조 1859억 * 64.865%<br>
= 3조 3638억

<details>
<summary> 두 레벨 간의 스킬 %데미지 차이 확인하기(펼치기/닫기)</summary>

[공용코어로 출시된 '매직 서킷 풀드라이브 VI' 1레벨]<br>
MP 1000 소비, 60초 동안 데미지 45% 증가, 공격력 13, 마력 5 증가.<br>
지속시간 동안 일정 범위 내 최대 10명의 적에게 마력 표식을 새김.<br>
마력 표식이 있는 적에게 자신의 직접 공격 스킬 적중 시 296% 의 데미지로 6번 공격하는 마력 폭풍 발생.<br>
마력 폭풍 재발동 대기시간 1초.<br>
...
[공용코어로 출시된 '매직 서킷 풀드라이브 VI' 9레벨]<br>
MP 1000 소비, 60초 동안 데미지 45% 증가, 공격력 37, 마력 29 증가.<br>
지속시간 동안 일정 범위 내 최대 10명의 적에게 마력 표식을 새김.<br>
마력 표식이 있는 적에게 자신의 직접 공격 스킬 적중 시 488% 의 데미지로 6번 공격하는 마력 폭풍 발생.<br>
마력 폭풍 재발동 대기시간 1초.<br>
(재사용 대기시간 120초)

스킬 %데미지 상승율
= ('매서풀VI' 9레벨 스킬 %데미지 - '매서풀VI' 1레벨 스킬 %데미지) / '매서풀VI' 1레벨 스킬 %데미지
= (488 - 296) / 296
= 0.64865(-> 64.865%)
</details>

※ 분석 참고: 변수를 최소화하기 위해 단순 실측치가 아닌, 사용횟수 오차를 통제한 보정된 수치로 계산하였습니다.

두 레벨 간의 스킬 %데미지 차이를 계산하였을 때, "3조 3638억(0.630%)"의 데미지 차이를 만들어 냈습니다.

##### 2단계: '매서풀VI' 스킬 한정 영향을 미친 추가효과 상승분 구하기.

매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지 예측치<br>
= 매서풀VI 1레벨 스킬 자체가 입힌 누적 데미지 * (1 + 스킬 %데미지 상승율)<br>
= 5조 1859억 * (1 + 0.64865)<br>
= 8조 5497억

오차율<br>
= ("보정된" 실측치 - 예측치) / 예측치<br>
= (8조 5921억 - 8조 5497억) / 8조 5497억<br>
= 0.00496(-> 0.496%)<br>

해당 오차율(0.496%)은 표본 부족에 따른 통계적 오차를 내포할 수 있으나, 이론적으로 공격력 상승에 의한 데미지 상승치로 해석하는 것이 타당합니다.<br>
실측치에 오차율을 곱하여 공격력 상승으로 인한 매서풀VI 스킬 자체가 입힌 누적 데미지의 상승치를 산출할 수 있습니다.

공격력 상승으로 인한 '매서풀VI' 9레벨 스킬 자체가 입힌 누적 데미지의 상승치<br>
= 오차율 * '매서풀VI' 9레벨 스킬 자체가 입힌 누적 데미지(실측치)<br>
= 0.496% * 8조 5103억<br>
= 422억

앞서 계산한 수치(422억)는 '매서풀VI' 스킬 "단독"으로 얻은 공격력으로 인한 데미지 상승치를 확인했습니다.<br>
하지만, '매서풀VI'의 공격력 상승 버프는 '매서풀VI'에만 적용되는 것이 아니라, 버프 지속시간 동안 사용하는 "모든 스킬"에 동일하게 적용됩니다. '매서풀VI'를 제외한 나머지 스킬들이 얻은 데미지 상승량을 계산할 겁니다.

##### 3단계: '매서풀VI' 스킬을 제외한 나머지 스킬에 영향을 미친 추가효과 상승분 구하기.

전체 데미지 상승량에서 '매서풀VI' 자체의 상승량을 제외하는 방식으로, 나머지 스킬에 적용된 공격력 버프의 상승치를 산출해보겠습니다.

'매서풀VI' 레벨 차이에 따른 전체 데미지 차이<br>
= '매서풀VI' 9레벨 전체 데미지 - 매서풀VI 1레벨 전체 데미지<br>
= 539조 5919억 - 533조 8771억<br>
= 5조 7148억

'매서풀VI' 스킬 자체가 입힌 누적 데미지의 상승치<br>
= '매서풀VI' 9레벨 스킬 자체가 입힌 누적 데미지 - '매서풀VI' 1레벨 스킬 자체가 입힌 누적 데미지<br>
= 8조 5103억 - 5조 1859억<br>
= 3조 3244억

공격력 상승으로 인한 '매서풀VI' 스킬을 제외한 나머지 스킬로 입힌 누적 데미지의 상승치<br>
= '매서풀VI' 레벨 차이에 따른 전체 데미지 차이 - '매서풀VI' 스킬 자체가 입힌 누적 데미지의 상승치<br>
= 5조 7148억 - 3조 3244억<br>
= 2조 3904억

공격력 상승 전체 데미지의 비중<br>
= 공격력 상승으로 인한 '매서풀VI' 스킬을 제외한 나머지 스킬로 입힌 누적 데미지의 상승치 / '매서풀VI' 9레벨 전체 데미지<br>
= 2조 3904억 / 539조 5919억<br>
= 0.00443(-> 0.443%)

'매서풀VI'를 제외한 나머지 스킬들의 추가 효과의 가치, "2조 3904억(0.443%)"를 확인했습니다.

##### 4단계: '매서풀VI' 스킬이 영향을 미친 추가효과 상승분 구하기.

여기서 앞서 계산해두었던 '매서풀VI' 한정 공격력 상승분까지 더해주면, 비로소 이 추가효과가 캐릭터 전체 딜량에서 차지하는 종합 비중을 구할 수 있습니다.
...
추가 효과로 인한 누적 데미지 상승치<br>
= 공격력 상승으로 인한 '매서풀VI' 9레벨 스킬 자체가 입힌 누적 데미지의 상승치 + 공격력 상승으로 인한 '매서풀VI' 스킬을 제외한 나머지 스킬로 입힌 누적 데미지의 상승치<br>
= 422억 + 2조 3904억<br>
= 2조 4326억

추가 효과로 인한 전체 데미지 비중<br>
= 공격력 상승으로 인한 '매서풀VI' 스킬을 제외한 나머지 스킬로 입힌 누적 데미지의 상승치 / '매서풀VI' 9레벨 전체 데미지<br>
= 2조 4326억 / 539조 5919억<br>
= 0.00451(-> 0.451%)

하지만, 여기서 주의할 점이 하나 있습니다. 계산한 "0.451%"라는 숫자는 최종 결과물의 비중일 뿐, 우리가 알고 싶은 기존에 비해서 얼마나 쎄졌는지에 대한 상승율이 아닙니다.

버프를 받기 전의 데이터를 기준점으로 두고 역산하는 과정이 반드시 필요합니다.

추가 효과로 인한 전체 데미지 상승율<br>
= 100 / (100 - 공격력 상승으로 인한 누적 데미지 비중)<br>
= 1.00453

[공격력 상승 관련 파트 결론]
공격력 소비 아이템 또는 스킬 사용하지 않는 상황 및 공격력%를 제외한 공격력 3450 기준으로 '60초 동안 공격력 24 상승'은 '최종 데미지 0.453%과 유사'하다는 점을 알 수 있습니다.

※ 분석 참고: 공격력 상승량에 가동률(지속시간 / 재사용 대기시간)을 곱하게 되면, 5분 40초동안 동일한 데미지를 넣었다는 가정을 해버리기 때문에, 실전과 괴리율이 굉장히 큽니다.
...
= (296 * 6) * 168<br>
= 298,368(29만 8368%) -> <br>
[추가 효과]: 60초 동안 공격력 13 -> 평균 최종데미지 0.245% 상승하는 가치를 지님.

'매서풀VI' 9레벨 스킬 %데미지<br>
= '매서풀VI' 스킬 %데미지 * 사용횟수<br>
= (488 * 6) * 168<br>
= 491,904(49만 1904%) (점유율 1.577%, 데미지 상승율 1.189%)<br>
[추가 효과]: 60초 동안 공격력 37 -> 평균 최종데미지 0.698% 상승하는 가치를 지님.

1레벨 기준 레벨당 최종데미지 상승율을 구하기.
1레벨 기준 레벨당 최종데미지 상승율을 구하기.

※ 레벨별 평균을 낸 값으로 일부 오차가 있을 수 있습니다.
