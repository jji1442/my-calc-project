...
&emsp;- 하지만 그 이상적인 세팅과 다르거나 모든 사용자가 같은 전투방식을 사용하지 않기 때문에 데이터를 제출한 유저와 강화효율의 차이가 클 수 있습니다.
2. 반영되지 않는 요소들.<br>
...
- 문제점
  ...
  &emsp;- 특히 스킬 1회당 타수 같은 세부 정보를 모두 기억하기 어려워 잘못된 정보 기입 시, 다시 입력해야 하거나 잘못된 효율이 측정되는 경우가 발생합니다.
  3. 강화 스킬 간의 높은 의존성.<br>
...
- 메인 함수<br>
  ...
  2.&nbsp;마스터리 코어 중복 상승으로 인한 현재 레벨 기준 스킬 %데미지 조정하는 함수 실행.<br>
  3.&nbsp;마스터리 코어별 강화효율 산출하는 함수 실행.<br>
  4.&nbsp;강화 추천하는 함수 실행.<br>
...
- 정의할 함수 종류.
  - 사용자로부터 입력받는 함수.
    ...
    - 처리해야 할 사항
      ...
      - 마스터리 코어별 현재 레벨이 float 형식, string 형식이라면, 다시 입력.
        - type(변수명)을 활용.
  ...
  - 마스터리 코어 중복 상승으로 인한 현재 레벨 기준 스킬 %데미지 조정하는 함수.
    - 매개변수
      - 마스터리 코어 리스트(리스트)
    - 설명
      - 서로 다른 마스터리 코어가 같은 스킬의 %데미지를 상승하는 경우가 존재.
      - 그러한 경우에는 해당하는 마스터리 코어의 현재 레벨 기준 스킬 %데미지를 합산하여 각각 현재 레벨 기준 스킬 %데미지로 변경하는 과정.
  ...
  - 마스터리 코어별로 강화효율 산출하는 함수.
    ...
    - 설명
      ...
      - 1레벨 상승 시 최종데미지 상승율, 1레벨 상승 시 100억당 최종데미지 상승율, 5레벨 상승 시 평균 최종데미지 상승율, 5레벨 상승 시 100억당 평균 최종데미지 상승율을 산출.
    - 처리해야할 사항
  ...
  - 강화 추천하는 함수.
    - 매개변수
      - 마스터리 코어 리스트(리스트) 
    - 설명
      - 모든 최종데미지 상승율을 한 리스트 안에 넣고, 최댓값을 찾아 인덱스와 함께 리턴하는 과정.
    - 처리해야할 사항
      - 가장 효율 좋은 마스터리 코어 번호 = 리스트 인덱스 // 2 + 1
      - 최댓값이 0이면, 모든 스킬이 최대 레벨을 달성
        - 0, 0을 리턴함.
      - 최댓값 번호 % 2가 0이면, 인덱스가 0, 2, 4, 6이므로 1레벨 상승 시 100억당 최종데미지 상승율 관련 데이터가 존재함.
        - 가장 효율 좋은 마스터리 코어 번호, 1을 리턴함.
      - 아니라면, 1, 3, 5, 7이므로 5레벨 상승 시 100억당 평균 최종데미지 상승율 관련 데이터가 존재함.
        - 가장 효율 좋은 마스터리 코어 번호, 5을 리턴함.
    - 리턴값
  ...
  - 결과를 출력하는 함수.
    ...
    - 처리해야할 사항
      - 강화 횟수가 0이라면, 모든 스킬이 최대 레벨을 달성함.
        - False를 리턴함.
      - 아니라면, 강화할 스킬이 있으므로 스킬 이름, 추천 레벨, 최종데미지 상승율을 출력함.
        - True를 리턴함.
    - 리턴값
      - bool형.
