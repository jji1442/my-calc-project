...
-> '최대 레벨이 30레벨인데, 최대 간극이 30%나 벌어지네? 보정 상수가 존재하나?' 라는 생각을 했습니다.
<img src="trial_and_error_img/trial_and_error_1.PNG">

1-1. 최대 레벨이 30이라는 점과 최대 간극이 30%인 점을 이용하여 보정상수(1레벨당 0.01% 감소)를 추가해보았습니다.<br>
-> 보정상수를 넣어도 값이 일정하지 않네? 스킬 %데미지와 연관이 있나?
...
3. 1 -> 2레벨 상승 시의 스킬 %데미지 상승율이 고정으로 두고 최종데미지 상승율을 산출해보았습니다.
-> 
<img src="trial_and_error_img/trial_and_error_3.PNG">

4. 스킬 2가지를 섞어 강화했을 때 효율이 어떻게 변화하는지 확인했습니다.
<img src="trial_and_error_img/trial_and_error_4.PNG">

5. 어센트 스킬은 쿨타임과 관계 없이 스킬을 3번 연속으로 사용 가능하다는 점을 이용하여, 6분 안에 어센트 3번를 다 사용한다는 가정으로 효율 계산하였을 때 연속적으로 강화하는 것이 효율이 좋다는 것을 보고, 다시 효율을 측정해보았습니다.
-> 오차 범위 내라고 판단함.
-> 이 오차 범위 생긴 이유는 게임사에서 제공하는 전투분석의 소숫점 2번째 자리 수까지의 점유율을 사용한 것이 아니라, 입힌 피해량을 전체 피해량으로 나누어 측정하였기 때문이라고 판단함.
