...
3. '영원히 굶주리는 짐승'의 총 %데미지 감소, 참격횟수 감소.
  - 참격횟수는 추가 공격횟수를 의미하며, 아크는 이 참격횟수에 따라 '다가오는 죽음'의 사출량이 증가
4. '끝없는 고통'의 총 %데미지 감소, 참격횟수 감소.
...
전투 구조가 간단한 구조(극딜 구조)에서 다른 직업과 유사한 전투구조(극딜 - 준극딜 구조)로 바뀜으로 인해 아크의 핵심 버프를 효율적으로 활용하지 못하는 상황이 왔습니다.
아크의 핵심 버프.
...
이러한 주요 버프틀의 지속시간과 인피니티 스펠의 지속시간(30초)와 맞지 않아, 모든 효과를 100% 받지만 활용을 50%도 못하는 상황이 생겼습니다.
'인피니티 스펠'은 지속시간이 짧아져 '인피니티 스펠'의 강화효율 자체도 낮아지는 결과를 초래했습니다.

이 인피니티 스펠 변경으로 인해, '근원의 기억(18초간 게이지 소모하지 않음 + 시전 중 무적)'을 극딜 사이클에 추가해야한다는 점이 문제점으로 작용되었습니다.
...
'근원의 기억' 사용 중 게이지 소모되지 않게 수정해야한다고 생각합니다.
...
극딜 사이클 말한 김에 '영원히 굶주리는 짐승'과 '끝없는 고통VI'의 참격횟수 조정으로 인해 아크의 극딜 재미가 사라졌습니다.
참격횟수에 따라 '다가오는 죽음'의 사출량이 달라지는데,
사출량이 줄어 적중되는 소리가 적어져 대응되는 입히는 피해량을 보면 재밌었습니다.
하지만 그 
...
결론: 구조적 모순과 개선의 실패
이번 패치는 아크에 대한 깊이 있는 이해 없이 기존의 구조에 새로운 메커니즘을 억지로 끼워 맞춘 결과물로 보입니다.
전투 경험 개선이라는 명목으로 이루어졌음에도 불구하고, 캐릭터의 플레이에 더 많은 제약이 생겨 불쾌한 경험을 유발했습니다.

캐릭터의 전투 경험을 개선하는 가장 간단한 방법은 명확합니다.

극딜형 직업의 경우, 극딜 시간을 압축하고 딜링 과정의 족쇄를 풀어주는 것이 올바른 방향입니다.

지속 딜형 직업의 경우, 안정적인 딜량을 방해하는 요소를 제거하는 것이 중요합니다.

하지만 이번 아크 패치는 이러한 기본 원칙을 따르지 않았습니다. 스펙터 상태와 레프 상태가 혼합된 구조임에도 불구하고, 게임사는 극딜에 새로운 족쇄를, 준극딜에는 더욱 큰 족쇄를 추가하는 모순을 범했습니다.

결과적으로, 이 패치는 캐릭터의 핵심 구조와 안정성을 심각하게 훼손했습니다. 과거 쿨타임 불일치로 인한 딜 편차와 같은 문제를 감수하면서 플레이했지만, 이번 변경은 직업 자체를 근본적으로 불쾌하게 만들어 지속적인 플레이가 불가능하다고 판단됩니다. 이에 따라 더 이상 아크를 활용한 강화 효율 계산을 진행하기 어렵다고 결론 내렸습니다.
