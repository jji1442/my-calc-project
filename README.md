...
- 환산주스탯(=효율분석 사이트)의 특징.<br>
  ...
  2.&nbsp;반영되지 않는 요소들.<br>
  &emsp;- 재화 시세는 반영하나 사용하고 있는 '스킬 쿨타임 감소'나 '특수 반지 스킬' 같은 변수들을 반영하지 않으며 헥사코어 스킬 레벨에 따른 변화가 없는 헥사코어 강화 순서를 제공합니다.
...
- 개인 맞춤형 분석 도구의 특징.<br>
  1.&nbsp;개인 맞춤형 최적화.<br>
  &emsp;- 이 도구는 사용자가 현재 사용하고 있는 전투방식 기준으로 가장 효율적인 강화 순서를 찾아줍니다.
...
### 구조 1
- 설명
  - 강화 효율 측정에 필요한 모든 데이터(현재 레벨 기준 스킬 %데미지, 다음 레벨 기준 스킬 %데미지, 스킬 1회당 타수, 측정된 공격횟수 등)를 입력받아, 계산을 통해 효율을 도출하는 구조.
- 이점<br>
...
- 문제점<br>
  ...
  &emsp;- 특히 스킬 1회당 타수 같은 세부 정보를 모두 기억하기 어려워 잘못된 정보 기입 시, 다시 입력해야 하거나 잘못된 효율이 측정되는 경우가 발생합니다.<br>
...
- 이점<br>
  1.&nbsp;낮은 수동 입력.<br>
  &emsp;- 모든 스킬 정보를 직접 입력해야 하는 기존과는 달리, 사용자에게 필수적인 정보만을 요구합니다. 이로 인해 다시 입력해야 할 경우 번거로움이 줄어듭니다.<br>
...
- 이점<br>
  1.&nbsp;낮은 수동 입력.<br>
  &emsp;- 모든 스킬 정보를 직접 입력해야 하는 기존과는 달리, 사용자에게 필수적인 정보만을 요구합니다. 이로 인해 다시 입력해야할 경우 번거로움이 줄어듭니다.<br>
  2.&nbsp;강화 스킬 간의 높은 의존성 해소.<br>
...
- 문제점<br>
  ...
  2.&nbsp;배포 및 접근성의 한계.<br>
  &emsp;- 어플리케이션으로만 제공되는 방식은 사용자가 직접 다운로드하고 설치해야 하므로 접근성이 낮습니다.
- 구조를 변경하게된 이유
...
- 이점<br>
  1.&nbsp;낮은 수동 입력.<br>
  &emsp;- 모든 스킬 정보를 직접 입력해야 하는 기존과는 달리, 사용자에게 필수적인 정보만을 요구합니다. 이로 인해 다시 입력해야할 경우 번거로움이 줄어듭니다.
  2.&nbsp;강화 스킬 간의 높은 의존성 해소.<br>
  &emsp;- 직업별로 강화 스킬 효율 계산해줌으로써, 강화 스킬 간의 효율을 계산할 수 있습니다.
  3.&nbsp;배포 및 접근성이 용이함.<br>
...
- 정의할 함수 종류.
  - 사용자로부터 입력받는 함수.
    - 매개변수
      - 마스터리 코어 리스트(리스트)
    - 설명
      - 사용자로부터 마스터리 코어별 현재 레벨, 스킬별 점유율을 입력받아 마스터리 코어 리스트(리스트)에 저장하는 과정.
    - 처리과정
    ...
    - 처리해야 할 사항
      ...
      - 마스터리 코어별 현재 레벨이 float 형식 또는 string 형식이라면, 다시 입력.
      - 스킬별 점유율이 0 미만이라면, 다시 입력.
      - 스킬별 점유율이 string 형식이라면, 다시 입력.
      - 스킬별 점유율 합산이 100 초과하면, 다시 입력.
    - 리턴값
      - 마스터리 코어 리스트(리스트)
  ...
  - 현재 레벨 기준 스킬 %데미지 조정하는 함수.
    - 매개변수
      - 마스터리 코어 리스트(리스트)
    - 설명
      - 서로 다른 마스터리 코어가 같은 스킬 %데미지를 중복으로 상승시켜 현재 레벨 기준 스킬 %데미지를 조정하는 과정.
    - 처리과정
  ...
  - 강화 추천하는 함수.
    - 매개변수
      - 마스터리 코어 리스트(리스트) 
    - 설명
      - 모든 100억당 최종데미지 상승율을 한 리스트 안에 넣고, 최댓값을 찾아 인덱스와 함께 리턴하는 과정.
    - 처리과정
  ...
  - 결과를 출력하는 함수.
    - 매개변수
      - 마스터리 코어 번호, 강화 횟수.
    - 설명
      - 강화횟수에 따라 스킬 이름, 추천 레벨, 최종데미지 상승율을 출력함.
    - 공식
      - 강화 횟수가 0이라면, False를 리턴함.
        - 모든 스킬이 최대 레벨을 달성함.
      - 아니라면, True를 리턴함.
        - 강화할 스킬이 있으므로 스킬 이름, 추천 레벨, 최종데미지 상승율을 출력함.
    - 리턴값
      - bool형.
  ...
  - 점유율 조정하는 함수.
    ...
    - 설명
      - 마스터리 코어 번호에 따라 마스터리 코어 리스트 내에 있는 점유율을 갱신함.
    - 처리과정
...
만약에 모든 딜 상승량이 0이라면, 끝나게 설정.
  ...
  - 중복체크. 1 - 2,3,4 / 2 - 3,4 / 3 - 4.
    - 매개변수 = 마스터리 코어 리스트
    - for i in range(1, len(마스터리 코어 리스트) + 1):
      - for j in range(i, len(마스터리 코어 리스트) + 1):
        - for value1 in 마스터리 코어 리스트[i][스킬]:
          - for value2 in 마스터리 코어 리스트[j][스킬]:
            - if value1 == value2:
...
if common_keys:
    print(f"중복된 키가 있습니다: {common_keys}")
else:
    print("중복된 키가 없습니다.")
