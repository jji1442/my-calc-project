...
### 환산주스탯(=효율분석 사이트)의 특징.
- 데이터 기반의 양날의 검.<br>
  - 환산주스탯의 헥사코어 효율표는 상위권 유저의 전투 데이터를 기반으로 산출됩니다.
  - 그렇기 때문에 초보자에게 가장 보편적인 가이드라인을 제시합니다.
  - 하지만 그 이상적인 세팅과 다르거나 모든 사용자가 같은 전투방식을 사용하지 않기 때문에 데이터를 제출한 유저와 강화효율의 차이가 클 수 있습니다.
- 반영되지 않는 요소들.<br>
  - 재화 시세는 반영하나 사용하고 있는 '스킬 쿨타임 감소'나 '특수 반지 스킬' 같은 변수들을 반영하지 않으며 헥사코어 스킬 레벨에 따른 변화가 없는 헥사코어 강화 순서를 제공합니다.

&nbsp;&nbsp;이러한 한계를 개선하기 위해 이 도구는 사용자에게 데이터를 직접 받아 사용자에게 최적화된 강화효율을 제공하도록 개발되었습니다.

### 개인 맞춤형 분석 도구의 특징.<br>
- 개인 맞춤형 최적화.<br>
    - 이 도구는 사용자가 현재 사용하고 있는 전투방식 기준으로 가장 효율적인 강화 순서를 찾아줍니다.
