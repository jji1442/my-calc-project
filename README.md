...
2. 6분 주기 측정에 적합하지 않은 스킬인 '스파이더 인 미러', '크레스트 오브 더 솔라'(쿨타임 4분) 미사용.
3. 허수아비형 보스에선 효용 가치가 없는 '일리움 링크' 제외.
4. 서버렉 패치되었으므로, '일격필살' 사용.
5. 확률형 버프인 '컨택트 카라반' 제외.
6. 메르세데스 유니온 효과(200레벨과 250레벨의 평균값)를 적용하여 오리진 스킬(360초)의 실제 쿨타임은 340.2초.
   - 대부분의 유저들이 메르세데스 200레벨 유니온 효과(5%)를 가지고 있지만, 250레벨 유니온 효과(6%)는 비교적 소수의 유저들이 사용하므로 평균값(5.5%)으로 측정.
   - 스킬 쿨타임 감소 효과를 지닌 모자는 유효가 아닌 직업군도 존재하고, 사용하는 직업군들 조차도 사용하지 않거나 쿨타임 감소 수치가 다른 아이템을 사용하기 때문에 평균을 내기 어려움.
7. 특수한 경우가 아닌 보편적인 경우를 기준으로 측정.
   - ex) 개발자의 아크는 스킬 쿨타임 2초 감소인 모자를 착용 중.
   - 340.2초 기준으로 측정 시, 4회차 쿨타임 2분인 버프 및 스킬을 8초 가량 사용할 수 있으니 쿨타임 2분인 스킬은 3회로 제한.
8. 어센트 스킬은 시간 내에 가장 데미지 손해가 적은 구간에서 1회 사용.
9. 개발자가 보스전에서 사용하는 물약만 설정. 필드는 어센틱 포스 요구치 1. 보스는 레벨 250, 방어율 380%, 체력 무한, 대형, 반감 기준으로 측정.
   - VIP 버프, MVP 슈퍼파워, 붕어빵 뿌리기, 챔피언의 가호 제외.
11. 총 3회 측정하고, 평균값 낸 수치를 작성.
12. 자석펫 착용하여 측정.
13. 어빌리티는 보스 공격 시 데미지 증가 / 상태 이상에 걸린 적 공격 시 데미지 증가 / 버프 스킬 지속 시간 증가 기준으로 측정.
   - 재사용 어빌리티는 데미지 편차가 심하므로 사용하지 않음.
13. 이벤트 버프에서 보스 공격 시 데미지 10% 증가가 없어지므로, 하이퍼 스탯에서 보스 공격 시 데미지 10% 제외하고 측정.
   - 패치 전 하이퍼 스탯: 데미지 10레벨, 보스 공격 시 데미지 증가 14레벨. (30% + 51%)
   - 패치 후 하이퍼 스탯: 데미지 12레벨, 보스 공격 시 데미지 증가 15레벨. (36% + 55%)
   - 데미지와 보스 공격 시 데미지 증가는 합연산으로 처리되므로 이와 같이 처리함.
