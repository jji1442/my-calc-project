...
#### 분석 결론
핵심 스킬을 다 최대 레벨(30레벨) 달성한 이후 나머지 효용감 없는 스킬들을 강화할 경우에, '매서풀VI'를 강화하는 것을 권장합니다.<br>
'매서풀VI'를 강화하는 경우에도 10 ~ 11레벨 이상 달성할 수 있는 충분한 재화를 모아두고 스킬을 강화하는 것을 권장합니다.

[여기서 디테일문을 추가해서 %데미지 기준으로 최대 레벨에서 작성.]

전체적으로 스킬을 강화하였을 때 효율.
...
#### 주관적인 평가
가장 간단한 방식으로는 매서풀의 기본 사양인 다가오는 죽음을 발생하지 않도록 사양 변경 및 다가오는 죽음의 %데미지를 보정시키는 방식.<br>
다른 방식으로는 확실하게 매서풀VI %데미지를 높히는 방식.<br>
다만, 이 방식은 현시점 아크라는 직업이 밸류가 굉장히 높아서 불가능합니다. 그러므로, 보정 작업이 필연적이라고 생각합니다.<br>

>> 밸류가 어느정도인지 유저들이 측정한 결과.<br>
>> 유저들의 환산 10.3 챌린지 결과 이미지.<br>

헥사 초기화권을 준다면, '매서풀VI'는 절대로 찍지 않을 겁니다.<br>
기본적인 이유는 아크라는 직업 자체가 공격력이 너무 높아 효율이 박살난 직업이라서 이보다 더 높은 공격력을 가지고 스킬 %데미지를 포기할 수 없기 때문입니다.<br>
헥사코어를 안찍었던 아크 유저들 기준으로도 '매서풀VI' 1레벨의 스킬 %데미지가 마이너스기 때문에 강화할 이유 조차 없기 때문입니다.<br>
...
매서풀VI 가치를 구하기 위해선 다음의 3가지 과정을 거칩니다.
1단계. 매서풀VI 스킬 %데미지 상승분 구하기.
2단계. 매서풀VI 스킬 한정 공격력 영향 미치는 상승분 구하기.
...
실전 사용횟수 오차를 통제하기 위해 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지에 0.962%를 보정하여 '8.5921조'를 산출했습니다. 이를 바탕으로 두 레벨 간의 스킬 %데미지 차이(64.865%)를 적용하여 계산하면 다음과 같습니다.

사용횟수 보정치<br>
= 매서풀VI 1레벨 사용횟수 / 매서풀VI 9레벨 사용횟수 * 100<br>
= 168회 / 166.4회 * 100<br>
= 100.962%

"보정된" 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지<br>
= 사용횟수 보정치 * 매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지<br>
= 100.962% * 8.5103조<br>
= 8.5921조

[두 레벨 간의 스킬 %데미지 차이 확인하기]

매서풀VI 9레벨 스킬 자체가 입힌 누적 데미지 예상치<br>
...
해당 오차율(0.496%)은 표본 부족에 따른 통계적 오차를 내포할 수 있으나, 이론적으로 공격력 상승에 의한 데미지 상승치로 해석하는 것이 타당합니다.

[두 레벨 간의 공격력 상승치 확인하기]

이를 근거로, 실측치에 오차율을 곱하여 공격력 상승으로 인한 매서풀VI 스킬 자체가 입힌 누적 데미지의 상승치를 산출할 수 있습니다.

공격력 상승으로 인한 '매서풀VI' 9레벨 스킬 자체가 입힌 누적 데미지의 상승치<br>
= 오차율 * '매서풀VI' 9레벨 스킬 자체가 입힌 누적 데미지<br>
= 0.496% * 8.5103조<br>
= 0.0422조

앞서 계산한 수치(0.0422조)는 '매서풀VI' 스킬 "단독"으로 얻은 공격력으로 인한 데미지 상승치를 확인했습니다.

하지만, '매서풀VI'의 공격력 상승 버프는 '매서풀VI'에만 적용되는 것이 아니라, 버프 지속시간 동안 사용하는 "모든 스킬"에 동일하게 적용됩니다. '매서풀VI'를 제외한 나머지 스킬들이 얻은 데미지 상승량을 계산할 겁니다.

아래는 전체 데미지 상승량에서 '매서풀VI' 자체의 상승량을 제외하는 방식으로, 나머지 스킬에 적용된 공격력 버프의 상승치를 산출해보겠습니다.

'매서풀VI' 레벨 차이에 따른 전체 데미지 차이<br>
= '매서풀VI' 9레벨 전체 데미지 - 매서풀VI 1레벨 전체 데미지

'매서풀VI' 스킬 자체가 입힌 누적 데미지의 상승치<br>
= '매서풀VI' 9레벨 스킬 자체가 입힌 누적 데미지 - '매서풀VI' 1레벨 스킬 자체가 입힌 누적 데미지

공격력 상승으로 인한 '매서풀VI' 스킬을 제외한 나머지 스킬로 입힌 누적 데미지의 상승치<br>
= '매서풀VI' 레벨 차이에 따른 전체 데미지 차이 - '매서풀VI' 스킬 자체가 입힌 누적 데미지의 상승치<br>
= 5.7148조 - (8.5103조 - 5.1859조)<br>
= 2.3904조.

공격력 상승 전체 데미지의 비중<br>
= 공격력 상승으로 인한 '매서풀VI' 스킬을 제외한 나머지 스킬로 입힌 누적 데미지의 상승치 / '매서풀VI' 9레벨 전체 데미지 * 100<br>
= 2.3904조 / 539.5919조 * 100<br>
= 0.443%

'매서풀VI'를 제외한 나머지 스킬들의 버프 비중이 "0.443%"를 확인했습니다. 이제 여기서 앞단락에서 구해두었던, 공격력 상승분까지 더해주면, 비로소 이 버프가 캐릭터 전체 딜량에서 차지하는 종합 비중(0.451%)이 완성됩니다.

하지만, 여기서 주의할 점이 하나 있습니다. 우리가 계산한 "0.451%"라는 숫자는 최종 결과물의 비중일 뿐, 우리가 알고 싶은 스펙 상승율이 아닙니다.

버프를 받기 전의 데이터를 기준점으로 두고 역산하는 과정이 반드시 필요합니다.

공격력 상승으로 인한 누적 데미지 비중<br>
= (공격력 상승으로 인한 '매서풀VI' 스킬을 제외한 나머지 스킬로 입힌 누적 데미지의 상승치 + 공격력 상승으로 인한 '매서풀VI' 9레벨 스킬 자체가 입힌 누적 데미지의 상승치) / '매서풀VI' 9레벨 전체 데미지 * 100<br>
(2.3904조 + 0.0422조) / 539.5919조  * 100<br>
= 0.451%

= 100 / (100 - 공격력 상승으로 인한 누적 데미지 비중)<br>
= 1.00453

결과적으로 "0.453%"라는 결과를 얻음.
