...
게임 시스템 설명은 게임적 용어를 이해시키 위한 도구이므로, 추후 보완될 사항들이 존재합니다. 그러므로 위 문서는 정리된 항목은 아님을 밝힙니다.
게임적 용어를 사용하지 않고 설명을 할 순 있습니다. 하지만 위 문서의 내용이 매우 길어지게 되며 이는 가독성이 떨어지는 문제가 발생시키므로 이를 개선하기 위해 위와 같은 처리를 했음을 알립니다.
...
<br>
25.07.17, 메이플스토리에 어센트 스킬이 추가되었습니다. 어센트 스킬은 한 보스에서 '쿨타임과 상관없이 스킬을 최대 3번을 연속으로 사용 가능'합니다. 보스 클리어 타임이 6분(스킬 중 가장 긴 쿨타임이 6분인 점을 고려하여 효율 측정함)을 기준으로 효율 측정하였을 때 연속적으로(2레벨에서 9레벨까지) 강화하는 것이 효율이 좋다는 것을 확인하였고, 위 가설에 추가 테스트를 진행해 보았습니다.<br>
<br>
...
분석 및 결론: 계산 과정에서 발생한 '-0.044% ~ 0.081%'의 오차는 환산 주스탯의 소수점 처리 방식의 차이라는 결론에 도달하였습니다.
...
- 이점<br>
  ...
  3.&nbsp;높은 가시성.<br>
  &emsp;- 글로 제작된 파이썬 응용 프로그램에 비해 웹 사이트에 이미지(스킬 이미지, 강화 효율표)를 넣어 가시성을 높힐 수 있습니다.<br>
  4.&nbsp;스킬 정보 변경 가능.<br>
...
- 베타 프로젝트: 정밀 분석 - '논리적인 부분 내에서' 모든 스킬의 효율을 재검토 및 기능 추가.
  - 간편 모드(합리적인 부분) / 상세 모드(논리적인 부분) 기능.
- 릴리즈 프로젝트: 공식 출시 - 웹 사이트 및 데이터베이스를 이용하여 스크립트 재구축함. 웹 사이트용 이미지 추가.
...
25.09.09 기준 잊혀지지 않는 악몽VI/잊혀지지 않는 흉몽VI/다가오는 죽음의 코어가 기존 9레벨에서 15레벨로 변경되었습니다.
<br>
'특수 스킬 반지'란?<br>
사용 시, 일시적으로 스탯이 오릅니다. 쿨타임 2분

'링 익스체인지'란?<br>
특수 스킬 반지 2개를 자동으로 변경해주는 스킬로, 특수 스킬 반지를 변경하게 될 경우 변경된 특수 스킬 반지의 사용 쿨타임이 10초 부여됩니다.<br>
보편적으로 리스트레인트 링과 웨폰퍼프 링을 변경합니다.<br>

<br>
'링 익스체인지' 스킬이 추가되기 전, 아크의 전투 방식.<br>
아크는 리스트레인트 링 단일로 사용했었습니다.(전투 스킬 반지 3분 주기 시절 제외)
단일로 사용한 이유는 1분 주기 버프만 사용할 때, 웨폰퍼프 링으로 변경하지 않았습니다. 왜냐하면 지속시간이 50초인 인피니티 스펠(2분 주기 버프)의 효용가치(점유율 대략 27%)가 매우 높고, 특수 스킬 반지를 변경하기 위해선 전투를 5초간 멈춰야하는 단점이 있었기 때문입니다. 그리고 웨폰퍼프 링을 변경한다고 해도 높은 피해량 또는 생존적인 이점 없었기 때문에 변경하지 않았습니다.<br>
<br>
'링 익스체인지'의 사용 방식.<br>
...
측정해봐야하는 종류(코어 강화 효율 변화를 보기 위해서)
1. 허수아비형 보스 링 익스체인지 '없이' 오리진을 포함한 6분 사이클.
2. 허수아비형 링 익스체인지 '있이' 오리진을 포함한 6분 사이클.

<패치에 대한 평가><br>
전투 경험 개선을 패치한 이유는 '최초의 대적자(이하 신규 보스)'에 의해 패치된 것으로 추정됩니다.<br>
<br>
패치 이전, 제 아크는 허수아비형 보스에게 2분 주기 2회 기준 @@조를 기록했습니다.<br>
하지만 패치 이후, 신규 보스에서 '링 익스체인지' 사용 및 최종데미지 6% 상승으로 측정된 보스 입장 후 2분 주기 2번 딜량은 패치 이전 두 딜량의 90% 수준인 @@조를 기록했습니다.<br>
<br>
...
극딜 버프: 2분 주기 버프 + 1분 주기 버프
준극딜 버프: 1분 버프

보편적으로 극딜 버프 올린 후 보스를 속박시키기 때문에

2분 주기 기준으로 피해량 차이를 작성하려고 했으나, 보스 패턴 운에 치우치지 않은 평균적인 데이터를 얻기 위해서 6분 주기로 측정하였다.
추가적으로 신규 보스의 '힘의 순환' 주기가 2분 15초 마다 이므로, 메인 패턴(검 소나기 패턴, 본체 찾기 패턴)에는 공격하지 않는 것으로 시간에 의한 데미지 편차를 줄이도록 노력했습니다.
('힘의 순환' 1주기 당 공격 가능한 시간은 총 1분 59초 입니다.)
보스를 속박시키지 않고 '끝없는 고통' 스킬 사용 시 보스 패턴을 맞을 위험(패턴 적중 시 최종데미지 감소)이 있어, 1분 주기 버프와 함께 사용하는 '끝없는 고통' 스킬은 사용하지 않는 것으로 하였습니다.
(1분 주기 버프 때는 보스를 속박시킬 수 없습니다.)


'끝없는 고통' 스킬은 1분 주기 버프와 함께 사용됩니다. 보스를 속박하는 시간의 쿨타임이 100초이므로 1분 주기 버프가 활성화된 동안에는 보스를 속박할 수 없기 때문에, 패턴을 맞을 위험이 있어 해당 스킬은 사용하지 않았습니다.
