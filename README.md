...
#### 주의 사항1: 기본 스킬의 스킬 이름에 30레벨 기준 %데미지라고 작성하지 않은 이유는 '쓸만한 컴뱃 오더스'를 사용 시 '일부 스킬만' 최대 레벨인 30레벨에서 31레벨로 상승하기 때문에 최대 레벨이라고 작성함.
#### 주의 사항2: 5레벨 상승 시 측정하는 이유는 특정 구간에서 효율이 급격하게 감소했다가 효율이 다시 올라가는 구조이기 때문에 구간별 소모 재화 효율 중 가장 좋은 효율을 기준으로 평균을 내어 작성함.
...
- 강화효율 체크하는 함수
매개변수 종류는 점유율, 스킬배율, 현재 스킬 레벨임.
변수에 재화 시세를 550을 저장함.
구현에서는 성능 향상을 위해 550만 대신 550으로 사용함.
리스트에 마스터리 코어 1~30레벨 재화 소모량을 저장함.
재화 효율은 100억 기준으로 재화시세 * 소모 재화량임.
구현에서는 성능향상을 위해 100억 대신 100만을 사용함.
그러므로, 재화효율 공식은 100만 / (재화시세 * 재화 소모량) 임.
재화 소모량은 마스터리코어 1~30레벨 리스트에서 [현재 스킬레벨 - 1]의 데이터임.
최종 데미지는 점유율 * 스킬배율임.
100억당 최종데미지는 재화효율 * 점유율 * 스킬배율임.
