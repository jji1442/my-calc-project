...
5. 실전성 없으며, 돌아오는 증오VI의 징표 생성량 편차가 심한 '끝없는 고통VI' 스킬 미사용.
6. 메르세데스 유니온 효과(200레벨과 250레벨의 평균값)를 적용하여 오리진 스킬(360초)의 실제 쿨타임은 340.2초.<br>
...
7. 특수한 경우가 아닌 보편적인 경우를 기준으로 측정.<br>
...
9. 개발자가 사용하는 실제 도핑. 필드는 어센틱 포스 요구치 1. 보스는 레벨 250, 방어율 380%, 체력 무한, 대형, 반감 기준으로 측정.
10. 총 3회 측정하고, 평균값 낸 수치를 작성.
확률형 버프인 컨택트 카라반 스킬은 제외.
11. 자석펫 끼고함.
...
이를 통해, 높은 퍼센트 데미지를 가진 '돌아오는 증오VI'는 웨폰퍼프 링의 시너지로 인해 유일하게 유의미한 딜 점유율 상승을 보였습니다. 반면, 링 익스체인지의 직접적인 영향을 받지 않는 스킬들(근원의 기억, 끝없는 고통VI 등)은 전체 딜량이 늘어남에 따라 상대적으로 점유율이 감소하는 결과를 보였습니다.

주목할 점은 아크의 주력 연계 스킬인 '잊혀지지 않는 흉몽VI'는 점유율에 큰 변화가 없었으나, 또 다른 핵심 연계 스킬인 '플레인 차지드라이브VI'는 딜 점유율이 감소했다는 점입니다. 이 점을 미루아 보아, 스펙터 상태의 연계 스킬들은 점유율이 기존과 동일한 대신 레프 상태의 연계 스킬들은 점유율은 하락했을 것이라고 추측할 수 있습니다.

다음은 스펙터 상태의 연계 스킬 점유율과 레프 상태의 연계 스킬 점유율 입니다.
