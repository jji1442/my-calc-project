...
- 정의한 함수.
  - 강화효율 체크하는 함수(1렙 당).
    - 매개변수는 점유율, 스킬 배율, 현재 스킬레벨임.
    - 변수에 재화 시세를 550을 저장함.
    - 구현에서는 성능 향상을 위해 550만 대신 550으로 사용함.
    - 리스트에 마스터리 코어 1~30레벨 재화 소모량을 저장함.
    - 재화 소모량은 마스터리코어 1~30레벨 리스트에서 [현재 스킬레벨 - 1]의 데이터임.
    - 재화 효율은 100만 / (재화 시세 * 재화 소모량)임.
    - 최종데미지 상승율은 점유율 * 스킬 배율임.
    - 100억당 최종데미지 상승율은 재화 효율 * 점유율 * 스킬 배율임.
    - 리턴값은 최종데미지, 100억당 최종데미지임.
  ...
  - 강화효율 체크하는 함수(5렙 기준).
    ...
    - 재화 소모량은 마스터리코어 1~30레벨 리스트에서 [현재 스킬레벨 - 1]의 데이터임.
    - 재화 효율은 100만 / (재화 시세 * 재화 소모량)임.
    - 5레벨당 최종데미지 상승율은 점유율 * 스킬 배율임.
    - 5레벨당 100억당 최종데미지 상승율은 재화 효율 * 점유율 * 스킬 배율임.
    - 리턴값은 5레벨당 최종데미지, 5레벨 당 100억당 최종데미지임.
