...
매서풀 기준으로 평균 0.5% 점유율을 가졌고, 사용횟수 평균은 47.6회 이므로,
1,100 * 3 * 47.6 * 200 = 31,416,000(3141만 6000%)

[4차 이하]: 다가오는 죽음 데미지는 450% * 2번,
[6차]: 잊혀지지 않는 흉몽VI의 패시브 '다가오는 죽음' 데미지 54%p 증가. 
[6차]: 인피니티 스펠 지속시간 중 '다가오는 죽음' 데미지 112%p 증가.
[5차]: '다가오는 죽음' 최종데미지 120% 증가.

매서풀 총 %데미지 + 다가오는 죽음 총 %데미지.
1100 * 3 * 47.6 + ((450 + 54 + 112) * 2 * 121) * 2.2 = 329,529.2(32만 9529.2%)
329,529.2 / 31,416,000 * 100 = 1.0489%

매서풀VI 1레벨 %데미지.
296 * 6 * 168 = 298,368
60초 동안 공격력 13
298,368 / 31416000 * 100 = 0.9497%

%데미지 기준으로 0.0992% 낮은 상황이고,
직접 측정한 결과는 0.567% 낮은 상황이니.
60초 동안 공격력 13 = (1 - 0.567%) / (1 - 0.0992%) * 100 - 100 = 0.4293%

수정해도 그러는데...?
수치 오류가 있어서 수정해봤는데 근삿값이 안나오네..
...
다른 방식으로는 확실하게 매서풀VI %데미지를 높히는 방식.
다만, 이 방식은 현시점 아크라는 직업이 밸류가 굉장히 높아서 불가능합니다. 그러므로, 보정 작업이 필연적이라고 생각합니다.
