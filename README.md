...
- 강화 횟수를 산출하는 함수를 재귀함수화 및 최적화.
	...
	- 최초 1회의 경우 모든 코어별 강화 횟수를 측정해야하므로 리스트를 매개변수로 전달. 강화된 코어는 강화 횟수를 재측정해야만 하기 때문에 코어만 매개변수로 전달.
	- 리스트 내 코어가 있다는 점을 이용하여 함수를 재귀함수로 제작.
...
- 강화 횟수를 측정하는 함수
	- 매개변수: 코어(딕셔너리) 또는 코어 리스트(리스트)
	- 처리 사항: if문으로 리스트인지 확인하고, 맞다면 for문으로 강화횟수 측정하는 함수(리스트[i])를 실행. 아니라면, 정해진 조건에 따라 값을 코어별 강화횟수에 저장하고 매개변수(코어)를 리턴함.
	- 함수의 제일 하단에는 매개변수(리스트)를 리턴함.
