...
### 모든 작업은 MVP(Minimum Viable Product)를 기준으로 작성되었습니다.
...
- 레벨 관련 리스트를 통일화.
	- 기존: 1 ~ 30레벨 기준으로 리스트 저장.
	- 변경: 모든 리스트를 0 ~ 30레벨 기준으로 저장하도록 변경.
 	- 설명: 강화 코어의 스킬 계수 증폭을 저장하는 리스트가 0 ~ 30레벨을 저장하므로, 모든 리스트의 통일성을 위해 수정.
...
- 6차 스킬 코어별
	- 현재 적용 중인 보스 공격 시 데미지 증가, 방어율 무시 증가 수치를 저장하는 변수 추가.

- 캐릭터 스탯(딕셔너리)
	- 평균 스탯(딕셔너리, 마스터리 코어) <- 아크만 필요.
		- 보스에게 적용되는 총 데미지%
		- 방어율 무시%
	- 하이브리드 스탯(딕셔너리, 스킬 코어(오리진))
		- 보스에게 적용되는 총 데미지%
		- 방어율 무시%
	- 기본 스탯(딕서너리, 스킬 코어(어센트))
		- 보스에게 적용되는 총 데미지%
		- 방어율 무시%
...
### 서브 스크립트 구현
강화 코어에 사용되는 재화 소모량(리스트)
스킬 코어에 사용되는 재화 소모량(리스트)
강화 코어 레벨별 스킬 계수 증폭 리스트 0 ~ 30레벨(리스트)

### 강화 코어 레벨별 스킬 계수 증폭 리스트는 "0레벨"도 포함.
### 스킬 코어(오리진, 어센트 스킬)는 최소 1레벨을 기준으로 측정. -> 스킬 코어에 사용되는 재화 소모량[0] = 0
### 모든 강화 코어는 최소 0레벨을 기준으로 측정. -> 강화 코어에 사용되는 재화 소모량[0] = 75
