...
이 변경으로 '증오의 연쇄'를 제외한 극딜 이후, '링 익스체인지'를 통해 '웨폰퍼프 링'으로 교체하여 '증오의 연쇄'와 함께 추가적인 데미지를 넣는 새로운 빌드가 등장했습니다. 이는 과거 아크의 극딜 구조와 유사한 구조입니다. 이 빌드가 가능한 핵심 이유는 '인피니티 스펠'의 '다가오는 죽음' 사출량 증가로 인해 최대 징표까지 생성하는데 충분하기 때문입니다. 그러나 이 빌드는 링 익스체인지 기획의도(준극딜에 전투스킬반지를 편하게 변경하여 사용할 수 있도록)와 상충되어 후속 조정이 예상됩니다. 예상되는 조정 방안으로는 극딜 이후 연계 사용을 막기 위해 '징표 생성 확률 증가' 기능 삭제가 될 가능성이 높습니다.

'영원히 굶주린 짐승'의 충돌 횟수 증가와 '끝없는 고통'의 참격 횟수 증가는 '다가오는 죽음'의 사출량을 높여, 직업 고유 재미를 돌려놓는 긍정적 효과를 가져왔습니다.

#### 분석 결론
아크의 극딜 지속시간을 30초로 압축하려고 했던 것으로 보이나, 위와 같은 새로운 빌드로 인해 다시 원점(스펙터 55초 - 레프 55초 구조)으로 돌아왔습니다. 물론 상황에 따라 30초간 극딜도 가능하고, 만약에 허수아비형 보스를 상대한다면 긴 템포의 극딜이 가능한 구조가 되었습니다.

#### 주관적인 평가
아크라는 캐릭터는 패치하기 가장 어려운 캐릭에 속합니다.

9월달 패치 이전, 실전에선 극딜 주기인 110초 중 55초만에 딜량의 90 ~ 95%를 넣고, 나머지 55초를 노는 모습.



데미지가 한 곳으로 쏠려 있기 때문에 이 데미지를 분산 시켜 버리면, 구조적 문제 또한 개선 해야합니다. 하지만 그러한 문제는 해결할 생각은 없고,




보스 패턴 및 컷신 때문에 극딜 시간 날리는 문제도 있고 게이지도 넉넉한 편이 아니기 때문에 평딜 구간에서 생존을 위해 스펙터로 변신하게 되면 다음 극딜 시간이 줄어버리는 문제도 있어, 극딜 압축은 필연적이라고 생각합니다. 그래서 '인피니티 스펠' 지속시간 30초로 줄이는 패치 자체는 문제가 없다고 생각합니다. 하지만, 남은 지속시간의 데미지를 나눠줄 곳이 마땅하게 없다는 점이 문제입니다. 쿨타임 60초 스킬은 아크한테 안맞고,
