...
&emsp;이 함수의 기본 방식은 비용에 따른 데미지 효율의 평균값을 구해서 어떤 강화 순서가 가장 효율이 좋은지 확인하는 방식입니다. 여기서, 강화 순서 종류는 2가지로 나뉩니다.<br>
하나는 부가 효과의 가치가 높아서 먼저 강화하는 경우, 다른 하나는 부가 효과의 가치가 낮아서 비용적 측면이 고려되는 경우.<br>
이 2가지를 비교하여 강화 순서를 도출하고, 강화 횟수를 산출하는 방식입니다.

&emsp;먼저 강화하는 것이 이점이 큰 지 확인하는 방법은 부가 효과가 부여되는 레벨의 이전 레벨과 효율을 비교하여 우열을 가려내 결과를 도출합니다.<br>
예를 들자면, '8레벨 -> 10레벨'의 효율 vs. '8레벨 -> 9레벨'의 효율. 이 2가지의 수치를 비교하여 효율이 좋은 쪽을 가려내는 겁니다.<br>
...
&emsp;비용적 측면 또한, 데미지적 측면과 비슷한 형식으로 부가 효과가 부여되는 레벨의 다음 레벨과 효율을 비교하여 우열을 가려내 결과를 도출합니다.
예를 들자면, '9레벨 -> 11레벨'의 효율 vs. '10레벨 -> 11레벨'의 효율. 이 2가지의 수치를 비교하여 효율이 좋은 쪽을 가려내는 겁니다.<br>
...
이렇게만 처리한다면, 문제가 하나 발생합니다. "'9레벨 -> 10레벨'의 부가"
그런 경우가 딱 3가지의 경우의 수가 있습니다.
a. 데미지적 측면에서는 이점이 없는 경우.
b. 비용적 측면에서는 이점이 없는 경우.
c. 데미지적 측면 / 비용적 측면 둘 다 이점이 없는 경우.

각각
a. 비용적 측면으로 처리함.
b. 데미지적 측면으로 처리함.
c. '9레벨 -> 10레벨'라는 결과로 처리함.

부연 설명을 하자면,
데미지적 측면에서 이점이 없어 '9레벨 -> 10레벨'이라는 결과값이 나왔다면, 분명히 효율은 '10레벨 -> 11레벨' 보다 낮습니다.


두 결과값이 "9레벨 -> 10레벨"을 가리키지 않을 경우 서로의 답을 크로스하게

&emsp;데미지적 이점으로 인한 결과와 비용적 이점과의 비교를 통해 추천 레벨을 산출하는 겁니다.

&emsp;여기서 의문점이 하나 들겁니다. "데미지적 이점이 커서 '8레벨 -> 10레벨'의 효율이 좋다는 결과가 산출됐다면, '9레벨 -> 10레벨'의 효율에 비해 효율이 떨어지지 않나요?"<br>
그 부가 효과가 재화 소모량에 비해 효율이 좋다면 효율이 떨어질 가능성이 있습니다.<br>
다만 그 정도로 부가 효과의 효율이 좋다면, '8레벨 -> 9레벨'의 효율 보다 '8레벨 -> 10레벨'의 효율이 좋아야만 우선적으로 강화할 가치가 있습니다.<br>
'8레벨 -> 9레벨'의 효율 보다 효율이 안좋다면, 부가 효과로 인한 데미지 상승량의 비해 & 10레벨의 소모 비용 상승량에 비해 10레벨 까지 우선적으로 강화할 이유가 없다는 겁니다.

&emsp;"이 효율이 떨어진 상태로 데미지적 이점과 비용적 이점의 비교를 하면 우선 순위에서 밀리는 것 아닌가요?"라는 질문을 하나 더 던지게 될 겁니다.<br>
...
&emsp;다시 돌아와서, 데미지적 이점과 비용적 이점의 효율을 비교합니다.

&emsp;두 이점 중에 효율이 좋은 쪽 하나가 선택될 것이고. 이 과정을 20레벨 및 30레벨 기준으로 2번 더 반복합니다.

&emsp;그리고 산출된 추천 레벨들을 정렬합니다.<br>
이 추천 레벨이 서로 겹치는 구간이 있을 가능성(부가 효과의 효율이 좋은 경우)이 있습니다.<br>
그럴 경우에는 두 레벨 구간 간의 효율을 비교합니다.<br>
그 중 더 높은 효율을 기준으로 추천 레벨을 정렬합니다.<br>
예를 들어, '19레벨 -> 25레벨'과 '24레벨 -> 30레벨'이 효율 좋다고되었다고 가정하면 위 2가지의 효율을 비교하여<br>
...
&emsp;그리고, 각 추천 레벨 구간 별로 if문을 만든 대신 레벨이 높은 구간 순으로 강화 횟수를 산출하도록 if문을 설계하여 겹치는 가능성에 의해 강화 레벨 추천 순서가 변하지 않도록 설계하였습니다.


오리진 스킬은 9레벨, 나머지 코어는 30레벨로 설정하고 20레벨 부가 효과인 보스에게 적용되는 데미지 증가의 배율 수치를 조정하여 오리진 스킬 추천 레벨이 어떠한 양상으로 달라지는지 아래의 표로 정리하였습니다.
...
배율 20% 및 500% 에서는 부가 효과에 대한 이점보다 비용적 이점이 더 크게 작용하는 양상을 보였으며, 이로 인해 20레벨을 빠르게 달성하기보다는 강화를 후순위로 강화하는 것이 효율이 좋다라는 결과가 도출되었습니다.

반면 배율 800% 수준에 도달 시, 부가 효과에 대한 이점과 비용적 이점이 동일한 구간에 도달하여, 모든 단계를 순차적으로 강화하는 방식이 최적의 선택으로 나타났습니다.

이후 배율 3000% 에서는 부가 효과에 대한 이점이 비용적 이점을 명확히 상회하며, 20레벨을 빠르게 달성하는 것이 가장 효율적인 결과를 보였습니다.
...
### 알파 프로젝트 이미지
알파 프로젝트를 실행 시 출력되는 텍스트 양이 많아, 출력물 중 일부만 이미지를 첨부하여 정상적으로 실행됨을 보여드리겠습니다.
...
이번 단계의 중요한 로직에 대한 결과를 파란색 박스로 표기하였습니다.

스킬 코어1(이하 오리진 스킬)의 20레벨의 부가 효과의 효율이 낮아 20레벨을 거치지 않고 19레벨에서 바로 25레벨을 달성하는 것이 추천 되었습니다.(수정해야함)
