...
2. '영원히 굶주림 짐승' 롤백(총 %데미지 30% 상승, 충돌횟수 증가).
...
이 변경으로 '증오의 연쇄'를 제외한 극딜 이후, '링 익스체인지'를 통해 '웨폰퍼프 링'으로 교체하여 '증오의 연쇄'와 함께 추가적인 데미지를 넣는 새로운 빌드가 등장했습니다. 이는 과거 아크의 극딜 구조와 유사한 구조입니다. 이 빌드가 가능한 핵심 이유는 '인피니티 스펠'의 '다가오는 죽음' 사출량 증가로 인해 최대 징표까지 생성하는데 충분하기 때문입니다. 그러나 이 빌드는 링 익스체인지 기획의도(준극딜에 전투스킬반지를 편하게 변경할 수 있도록)와 상충되어 후속 조정이 예상됩니다. 예상되는 조정 방안으로는 극딜 이후 연계 사용을 막기 위해 '징표 생성 확률 증가' 기능 삭제가 될 가능성이 높습니다.

'영원히 굶주린 짐승'의 충돌 횟수 증가와 '끝없는 고통'의 참격 횟수 증가는 '다가오는 죽음'의 사출량을 높여, 직업 고유 재미를 돌려놓는 긍정적 효과를 가져왔습니다.

#### 분석 결론
아크의 극딜 지속시간을 30초로 압축하려고 했던 것으로 보이나, 위와 같은 새로운 빌드로 인해 다시 원점(스펙터 55초 - 레프 55초 구조)으로 돌아왔습니다.

#### 주관적인 평가
아크라는 캐릭터 기준으로는 패치하기 가장 애매한 캐릭인 것 같습니다.

9월달 패치 이전 아크는 준극딜이 사실상 없습니다. 대신 긴 극딜 버프가 존재했습니다. 실전에선 극딜 버프 지속시간 동안 전체 딜량의 90% 이상 차지할 정도라서 링 익스체인지로 인해 데미지를 몰아서 넣는 모습이 그리 좋은 모습이 아니라고 판단한 것 같습니다. 그러면, 캐릭터 구조를 전면 개편해야합니다.

하지만, 아크의 시너지 부터 압축시켜주긴 너무 좋은
...
3. 변수 사용량이 줄어 RAM 사용량 개선.
   ...
   - 변경: 레벨 상승 시 (평균) 최종데미지 상승율을 측정.
   - 설명: 필요할 경우에만 100억당 (평균) 최종데미지 상승율로 변환하여 사용.
