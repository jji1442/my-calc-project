...
<'다가오는 죽음'에 대한 설명>
  ...
  스펙터 상태에서 적에게 공격 시 '다가오는 죽음'이 사출되며, '다가오는 죽음'은 전체 딜량의 27%를 차지할 정도로 비중이 높습니다.
...
<'돌아오는 증오'에 대한 설명>
  ...
  이 '징표'는 최대치에 도달하더라도 기존 '징표'가 사라지기 전까지 새로 생성되지 않습니다. 또한, 징표를 회수할 수 있는 거리에 제한이 있습니다.
...
또한, '인피니티 스펠'의 보정하지 않아 강화효율이 낮아지는 결과를 초래했습니다.
...
유저들이 '끝없는 고통VI'도 2분화 원하니 변경했더니 아크에게 남는 스킬은 '돌아오는 증오VI' 스킬이 하나 남아,
...
이동형 보스가 아크의 딜 사이클을 5번 이상(약 4초 이상) 맞아줄 리가 없었고.
맞았다 한들, '다가오는 죽음'의 사출 속도도 느려 이동형 보스가 '무적 + 돌진'/'텔레포트'를 사용 시 '다가오는 죽음'이 증발되는 경우가 다반사였습니다.

증오의 연쇄 사용 시, '돌아오는 증오'의 쿨타임이 초기화되게 설계된 의도는 극한의 고점을 위한 설계인 것 같습니다.
게임사에서 아크를 변경한 의도대로 하면, 스펙터 유지시간이 최소 50초(극딜 30초, 준극딜 20초)가 됩니다.
여기서, 돌아오는 증오의 쿨타임 초기화 시스템을 넣었다는건 이 시간보다 더 많은 시간을 스펙터로 지내라는 말인데,
시스템적으로 불가능합니다. 110초 기준 스펙터 유지시간은 최대 55 ~ 60초입니다.
5~10초 안에 징표를 12개를 생성해서 '돌아오는 증오'의 쿨타임 초기화 시스템을 사용할 수 있는 경우는 '인피니티 스펠' 지속시간 중에만 사용이 가능합니다.
그러면 "그렇게 사용하면 되지 않냐?"라고 말씀하시겠지만, 준극딜 쿨타임이 밀리는 현상도 생각해야합니다.
하지만, 스킬 쿨타임 감소 4초 + 리스트레인트 링 5레벨(전투 스킬 반지)을 사용하면 이 현상들이 해결됩니다.
- 리스트레인트 4레벨: 지속시간 동안/영역 내에 있을 경우 공격력% 대폭 증가(15초 지속, 쿨타임 120초)
- 리스트레인트 5레벨: 지속시간 동안/영역 내에 있을 경우 공격력% 대폭 증가(20초 지속, 쿨타임 120초)
...
신규 극딜 사이클로 인해 '증오의 연쇄' 쿨타임이 4초 가량 밀리는 것을 생각해보면, 극한의 고점을 위한 설계라고 봅니다.

극딜 사이클 말한 김에 '영원히 굶주리는 짐승'의 충돌횟수과 '끝없는 고통VI'의 참격횟수 조정으로 인해 '다가오는 죽음'의 사출횟수가 줄어 극딜하는 재미가 사라졌습니다.
...
이렇게 되면 '스킬 쿨타임 감소' 모자 + '확률에 따른 일부 스킬 쿨타임 초기화' 어빌리티를 강제적으로 변경됩니다.
