...
환산주스탯에서 제공하는 100억당 최종데미지 상승율 수치는 '어떻게 제공될까?'라는 생각으로 시제품 개발 전에 해보았던 엑셀 작업입니다.<br>
항목별로 설명 드리겠습니다.<br>

<br>

성능 향상을 위해 재화 시세의 기본 입력 단위를 '만'으로 설정했습니다. 이로 인해 기준점인 100억은 '100만'으로 처리해야 합니다.<br>
재화효율은 100만 / (재화 시세 * 재화 소모량) 입니다.<br>
딜 상승량은 '(현재 레벨 기준 스킬 %데미지 + 스킬 %데미지 상승량(%p)) / 현재 레벨 기준 스킬 %데미지' 입니다.<br>
스킬 레벨업 시 새로운 점유율 계산 방식은 '현재 점유율 * 다음 스킬 배율 / 현재 스킬 배율 / (1 + 점유율 * (다음 스킬 배율 / 현재 스킬 배율 - 1) / 100)' 입니다.<br>
기존 점유율에 스킬 배율의 상승분을 곱하고, 그 상승분만큼 전체 점유율(100%)도 함께 증가하는 것을 반영하여 도출합니다.<br>
사이트상 최종데미지는 환산주스탯에서 제공하는 스킬 강화 시 최종 데미지 상승하는 수치입니다.<br>
레벨별 간극은 레벨 별로 예상 최종 데미지 상승율 수치와 사이트상 최종 데미지 상승율 수치의 간극을 의미하며, 계산 방식은 '예상 최종 데미지 상승율 / 사이트상 최종 데미지 상승율 - 1' 입니다.<br>
강화 비교는 현재 레벨과 이전 레벨의 최종 데미지 상승율 비교입니다. 계산 방식은 '현재 레벨 / 이전 레벨' 입니다.<br>

아래의 사진들은 '이러한 구조로 돌아가지 않을까?' 라는 생각으로 제작해본 엑셀 이미지 입니다.

내가 가지고 있는 데이터는 점유율과 딜 상승량이니, 이를 이용한 엑셀 표를 제작해 보자.

1. 점유율을 전체 딜량에서 딜 상승율만큼 보정하는 엑셀을 제작해보았습니다.
-> 최대 레벨이 30레벨인데, 최대 간극이 30%나 벌어지네? 설마... 보정 상수가 존재하나?
<img src="trial_and_error_img/trial_and_error_1.PNG">

1-1. 최대 레벨이 30이라는 점과 최대 간극이 30%인 점을 이용하여 1레벨당 0.01% 감소하는 로직을 추가해보았습니다.
   -> 보정상수를 집어넣어도 값이 튀네?
<img src="trial_and_error_img/trial_and_error_1_1.PNG">
2. 스킬 %데미지 상승량(%p)을 점유율의 곱으로 최종데미지 상승율 산출해보았습니다.
-> 스킬 %데미지와 연관이 있나?
<img src="trial_and_error_img/trial_and_error_2.PNG">
2-1. 점유율을 스킬 %데미지 배율로 나누어, 스킬 배율 1%당 최종데미지 상승량을 산출해보았습니다.
-> 점유율을 스킬 %데미지로 나누어도 그 값은 2번 결과랑 같네?
-> 고정적으로 처리할 수도 있지 않을까?
<img src="trial_and_error_img/trial_and_error_2_1.PNG">
5. 1 -> 2레벨 상승 시의 스킬 %데미지 상승율이 고정으로 두고 최종데미지 상승율을 산출해보았습니다.
-> 
<img src="trial_and_error_img/trial_and_error_3.PNG">
7. 스킬 2가지를 섞어 강화했을 때 효율이 어떻게 변화하는지 확인했습니다.
<img src="trial_and_error_img/trial_and_error_4.PNG">
8. 어센트 스킬은 쿨타임과 관계 없이 스킬을 3번 연속으로 사용 가능하다는 점을 이용하여,
   6분 안에 어센트 3번를 다 사용한다는 가정으로 효율 계산하였을 때 연속적으로 강화하는 것이 효율이 좋다는 것을 보고,
   다시 효율을 측정해보았습니다.
-> 오차 범위 내라고 판단함.
-> 이 오차 범위 생긴 이유는 게임사에서 제공하는 전투분석의 소숫점 2번째 자리 수까지의 점유율을 사용한 것이 아니라, 전체 데미지를 입힌 피해량으로 측정하였기 때문이라고 판단함.
...
베타 프로젝트(정밀 분석) - 논리적인 부분으로 모든 스킬의 효율을 재검토함. + 간편모드 / 상세 모드 추가.
릴리즈 프로젝트(공식 출시) - 웹사이트 및 데이터베이스를 이용하여 스크립트 재구축. 웹사이트 용 이미지 추가.


현재 레벨 기준으로 코어별로 최종데미지 상승율 보여줌.
코어 + 레벨 상승 시키면 다음 효율 보여줌.
상세보기 입력 시, 스킬별 최종데미지 상승율을 표기.
저장 입력 시, 데이터에 저장되고
데이터 확인 입력 시, 기존 데이터 정보 한눈에 보기 좋게 보여줌.
여기서 데이터 확인할 때 초기 값을 보여줄지, 상승량에 따른 점유율을 보여줄지. 이에 대해 고민해야함.
