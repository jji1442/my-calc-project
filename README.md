...
- 설명
예상 점유율의 경우, 환산주스탯에서 제공하는 100억당 최종 데미지 상승율을 제가 제작한 계산 방식을 역산하여 도출했습니다.<br>
"예상과 사이트가 같다면, 보정상수 값만 구하면 됨." 이 표현은 같은 레벨 기준 두 강화 비교가 동일하지만 최종데미지 상승율이 다르다면, 어떠한 보정상수를 받고 있음을 작성한겁니다.
...
<br>
- 전제조건<br>
  시행착오의 기준은 가장 단순한 코어 구성을 가진 렌의 '선참VI' 코어를 기준으로, 마지막 어센트 스킬은 렌의 '창룡파천검 : 일매낙화 천비인적' 코어를 기준으로 삼아 진행했습니다.<br>

1.&nbsp;제가 가지고 있는 데이터는 점유율과 스킬 %데미지 상승율이므로, 이를 이용한 엑셀을 제작해 보았습니다.
...
<br>
1-1.&nbsp;기존 엑셀에 보정 상수(1레벨당 0.01% 감소)를 추가해 보았습니다.<br>
<img src="trial_and_error_img/trial_and_error_1_1.PNG">
분석 및 결론: 보정 상수를 넣어도 값이 일정하지 않으므로, "스킬 %데미지와 연관이 있나?"라는 가설을 세웠습니다.<br>
<br>
2.&nbsp;고정 점유율 * 스킬 %데미지 상승율으로 최종데미지 상승율을 산출해보았습니다.<br>
<img src="trial_and_error_img/trial_and_error_2.PNG">
분석 및 결론: 스킬 %데미지 상승율에 따른 값이 일정하지 않으므로, "스킬 %데미지 상승율은 1레벨에서 2레벨로 상승 시 상승율로 고정인가?"라는 가설을 세웠습니다.<br>
<br>
3.&nbsp;스킬 %데미지 상승율이 1레벨에서 2레벨로 상승 시 상승율을 고정으로 두고, 최종데미지 상승율을 산출해보았습니다.<br>
<img src="trial_and_error_img/trial_and_error_3.PNG">
분석 및 결론: 스킬 %데미지 상승율이 고정적이지 않으므로 "코어별 강화 순서가 다르면 효율이 달라지나?" 라는 가설을 세웠습니다.<br>
<br>
4.&nbsp;스킬 2가지를 섞어 강화했을 때 효율이 어떻게 변화하는지 확인했습니다.
<img src="trial_and_error_img/trial_and_error_4.PNG">
분석 및 결론: "강화 순서가 다르면, 강화 효율이 다르네?"라는 결론에 도달했습니다.<br>
<br>
하지만, 메이플스토리에 코어들이 추가되면서 하나의 코어를 연속적으로 강화하는 것보다 다른 코어도 강화하는 것이 효율이 좋게 되어 측정에 난항을 겪게 되고, '선참VI'처럼 측정에 있어 단순한 코어를 찾기 또한 어려워 한동안 시행착오 검증을 중지하였습니다.<br>
<br>
아래 이미지의 빨간색 영역은 '선참VI' 코어입니다. 하나의 코어가 연속적으로 강화하였을 때의 효율적인 구조가 아님을 확인하실 수 있습니다.
<img src="trial_and_error_img/core_enh_eff.png">
> "어센트 스킬이 없었을 당시에 '선참VI'의 5레벨에서 8레벨까지 강화효율이 연속적으로 좋지 않았냐?"라고 하실 수 있지만, 이 4레벨 구간만으로 전체 코어 강화 로직을 일반화하기 어렵다고 판단했습니다.<br>
<br>
이후 메이플스토리에 어센트 스킬이 추가되었습니다. 어센트 스킬은 한 보스에서 '쿨타임과 관계 없이 스킬을 최대 3번을 연속으로 사용 가능'합니다. 보스 클리어 타임이 6분(스킬 중 가장 긴 쿨타임이 6분인 점을 고려하여 효율 측정함)을 기준으로 효율 측정하였을 때 연속적으로(2레벨에서 9레벨까지) 강화하는 것이 효율이 좋다는 것을 확인하였고, 위 가설에 추가 테스트를 진행해 보았습니다.<br>
<br>
...
5.&nbsp;어센트 스킬을 기준으로 가설 1번을 추가 테스트하였습니다.
