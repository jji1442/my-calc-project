...
5. 어센트 스킬, 오리진 스킬, '어비스 차지드라이브VI' 이 3가지의 방어율 무시, 보스 공격 시 데미지 증가, 데미지 증가 효과가 전부 다르게 측정함.
   ...
   - 오리진 스킬은 엔젤릭 버스터의 링크 스킬을 제외한 모든 링크 스킬, 5차 특수 코어인 일격필살은 스탯 증가량 * 가동률로 측정함.(아크 자체 버프는 제외)
   - '어비스 차지드라이브VI'는 모든 조건부/사용 스킬들을 스탯 증가량 * 가동률로 측정함.
6. 방어율 무시 측정 기준은 방어율 380% 보스 기준임.
7. 보스 클리어 타임은 15분 기준임.

#### 입력하는 데이터
없음 - 프로그램에 기입하여 결과를 출력하는 방식이 아닌 스크립트 내에 작성하여 결과를 출력하는 방식으로 변경됨.
