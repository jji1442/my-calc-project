...
- 정의할 함수 종류.
  - 사용자로부터 입력받는 함수.
    ...
    - 처리해야 할 사항
      ...
      - 마스터리 코어별 현재 레벨이 소수점 형식, 문자열 형식, 특수문자 형식이라면, 다시 입력
        - type(변수명)을 활용 
      - 스킬별 점유율이 0 이하라면, 다시 입력
      - 스킬별 점유율이 문자열 형식, 특수문자 형식이라면, 다시 입력
        - type(변수명)을 활용
      - 스킬별 점유율 합산이 100 초과하면, 다시 입력
    - 리턴값
      - 마스터리 코어 리스트(리스트)

  - 마스터리코어 중복 상승으로 인한 현재 레벨 기준 스킬 %데미지 조정하는 함수.
    - 매개변수
      - 마스터리 코어 리스트(리스트)
    - 설명
  ...
  - 마스터리 코어별로 강화효율 산출하는 함수.
    ...
    - 리턴값
      - 
1레벨 상승 시 최종데미지 상승율], 마스터리 코어 리스트[i][데미지 상승율][1레벨 상승 시 100억당 최종데미지 상승율] = 1레벨 상승 시 강화효율을 산출하는 함수(마스터리 코어[i])
