...
이 패치에 단점만 있지 않았습니다. 극딜 시간을 줄이고, 준극딜을 만들었으니 준극딜에 유효한 경우가 있을 겁니다. 예를 들어,

   1. 보스 체력이 애매하게 남은 보스를 처리하는 경우.
   2. 하위 보스의 페이즈 별로 처리하기 위하여 준극딜 사용하는 경우.
   3. 상위 보스들에선 극딜 사용 후 다음 페이즈로 넘어가는 과정(애니메이션 재생됨)에서 극딜 지속시간의 손해를 덜 볼 수 있게된 경우.

실전 데미지 부분에서 큰 체감을 한 부분이 '칼로스(보스)' 2페이즈입니다. 눈금 1칸을 파티원과 같이 오리진 스킬을 포함하여 극딜을 넣으면 50 ~ 60초 만에 다음 눈금이 열렸지만, 개선 이후에는 준극딜까지 넣어도 딜이 부족한 상황까지 생겼습니다. '칼로스(보스)'의 잦은 텔레포트로 인해 '돌아오는 증오' 스택도 쌓이지도 않고, 스킬 자체 적중율 또한 낮습니다. 이 말은 즉슨, 실전 데미지가 떨어졌다는 겁니다.
   - 눈금: 보스의 체력이 일정 수준 도달하면 체력이 깎이지 않고 보스 패턴이 나옴. 이 보스 패턴 파훼 시 다음 눈금 까지 체력을 감소시킬 수 있음. 파훼 실패 시 일정 체력 회복함.

'돌아오는 증오'의 기능 변화는 '세렌(보스)', '카링(보스)' 3페이즈에서는 좋았습니다. 맵 중앙에서 전투 시 '세렌'이 돌진 스킬을 사용하면 쌓아놓은 '징표'들을 맞출 방법이 없었습니다. 이로 인해 '세렌'을 유도하는 컨트롤(맵 바깥쪽으로 유인하는 행동)이 필수적이었습니다. 이 컨트롤을 했을 때/하지 않았을 때 데미지가 10% 이상 차이납니다. 패치 이후에는 보스를 유도할 필요가 없어져 입장 후 '세렌'의 돌진을 기다릴 필요가 없다는 점과 '세렌'을 맵 바깥쪽으로 유도할 필요가 없다는 점이 마음에 들었습니다. '카링' 3페이즈는 공용 이동 스킬인 '블링크' 사용하여 최대 4명에게 적중할 수 있게 되었습니다.(기존에는 최대 3명)
   - 블링크: 바닥에서 사용 시, 랜덤한 위치로 이동.

스킬의 운영 방식 변화는 긍정적인 효과가 있음에도 전투 피로감을 극대화했습니다.
...
4. 생성된 '징표' 위치 고려.
   ...
   - 데미지 손해가 가장 적은 위치를 고려했을 때 / 안했을 때 데미지 편차가 굉장히 심합니다.
      ...
      - 비합리적 패턴이 많은 구간(2페이즈의 체력 50% 미만)에서의 데미지 편차가 아니라, 초중반 구간(1페이즈 및 2페이즈 체력 75% 이상)에서 데미지 편차가 큽니다.
...
5. '근원의 기억' 강제 극딜 사이클 추가.
   ...
   - 기존에는 '최초의 대적자'에서 죽는 경우가 손에 꼽을 정도 입니다. 지인들이 '최초의 대적자'에서 죽는 경우가 이해가 안될 정도로.

온 몸을 비틀며 고점을 바라보려고 노력해도 잘 되지 않으며, 이렇게 해야 기존 이동형 보스들의 데미지와 비슷하다는 점은 문제라고 생각합니다.

위 현상을 미루어 보아, "대부분 게이지 부족 현상 때문에 다 생긴 일이네? 그러면 게이지 늘려주면 되지 않아?" 라고 생각하시겠지만, 무작정 게이지를 늘려버리면 허수아비형 보스와 데미지 격차는 더욱 심해집니다. 스펙터 상태를 더 길게 유지가 가능하다는 건 그 시간만큼 스펙터 스킬들의 점유율이 더 높아집니다.스킬연계가 가능합니다. 이동형 보스들은 연계 스킬을 잘 맞아주지 않는데 이번 패치로 연계 스킬의 %데미지가 커졌으므로 격차는 더 커질 것으로 예상됩니다.


   - 이 패치의 헤드라인은 "허수아비형 보스의 데미지와 실전 보스의 데미지 편차를 줄여보자."라는 취지로 시작했는데 위 방향으로 처리하는 것이 옳지 않다고 생각합니다.

제가 보기엔 데미지 편차를 줄이려면, 보스 패턴 너프가 맞다고 봅니다.

하지만 무작정 너프를 해버리면, 보스 잡는 재미가 없을 것이라고 판단됩니다.
과한 부분은 쳐내고, 시전 애니메이션과 다른 부분을 공격하는 현상을 수정해야한다고 생각합니다.

칼로스의 
세렌 칼로스 카링 대적자 검은 마법사
