...
3. 하이퍼 / 4차 이하 스킬의 %데미지는 '쓸만한 컴뱃 오더스'가 켜져 있는 상태를 기준으로 작성함.
...
1. 메인 함수 제작.
- 저장되어 있는 데이터.
모든 데이터는 float/int 형식으로 입력된 데이터를 딕셔너리 형태로 바꿔줘야함.
마스터리 코어별 "스킬별 점유율" - 딕셔너리 내 딕셔너리.
마스터리 코어별 "스킬별 공격횟수" - 딕셔너리 내 딕셔너리.
마스터리 코어별 "스킬 레벨" - 리스트.

- 필요한 딕셔너리 종류.
<외부 파일>
 기본 스킬
  하이퍼 / 4차 이하 스킬별 최대 레벨 기준 %데미지
