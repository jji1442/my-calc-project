...
### 구조 1
<설명><br>
&emsp;강화 효율 측정에 필요한 모든 데이터(현재 스킬 퍼센트 데미지, 다음 스킬 퍼센트 데미지, 스킬 1회당 타수, 측정된 공격횟수 등)를 입력받아, 계산을 통해 효율을 도출하는 구조.
- 이점<br>
...
### 구조 2
<설명><br>
&emsp;직업별 스킬 정보는 데이터베이스에 저장되어 있으며, 일부 데이터(해당 스킬 점유율, 현재 스킬레벨, 재화 시세)를 입력받아 효율을 계산하는 구조.

<이점><br>
1. 낮은 수동 입력.<br>
&emsp;모든 스킬 정보를 직접 입력해야 하는 기존과는 달리, 사용자에게 필수적인 정보만을 요구합니다. 이로 인해 다시 입력해야 할 경우 번거로움이 줄어듭니다.
...
- 메인 함수
  ...
  - 마스터리코어별 레벨 입력받기(그림 참조)
  - 마스터리코어별 스킬 순서대로 점유율 입력받기(그림 참조)
