...
3. 기본 스킬의 %데미지는 ‘쓸만한 컴뱃 오더스’가 사용 중인 상태를 기준으로 측정함.
  - ‘쓸만한 컴뱃 오더스’는 ‘일부 스킬만’ 최대 레벨이 30레벨에서 31레벨로 상승하는 버프 스킬임.
4. 분석의 범위를 명확히 설정하기 위해서 ‘다가오는 죽음’은 ‘인피니티 스펠’ 사용 시에만 발생하는 경우를 기준으로 강화 효율을 측정함.
5. 어센트 스킬, 오리진 스킬, '어비스 차지드라이브VI' 이 3가지의 방어율 무시, 보스 공격 시 데미지 증가, 데미지 증가 효과가 전부 다르게 측정함.
   - '어비스 차지드라이브VI'는 모든 조건부 버프들은 스탯 증가량 * 가동률로 측정함.
   - 오리진 스킬은 엔젤릭 버스터의 링크 스킬 및 아크 자체 버프 스킬은 스탯 증가량 * 가동률이 아닌 
   - 어센트 스킬은 장비 : 모자, 장비 : 반지, 조건부로 발동하는 패시브 스킬 효과, 액티브 스킬 사용 효과(길드 스킬, 장비 명장, 아크 자체 버프 포함), 몬스터의 속성, 몬스터의 패턴, 디버프, 지속 시간이 30분 미만인 소비, 캐시 아이템이 적용되지 
   - 
