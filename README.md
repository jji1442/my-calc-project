...
3. 새로운 스킬 기준으로 기존 사이클.

#### 분석 결과
##### 1. 공용 코어의 스킬 사양 변경.
[매직 서킷 풀드라이브(30레벨, 이하 기존 스킬)]
60초 동안 데미지 45% 증가.
공격 스킬 사용 시 4초마다 MP를 150 추가 소모하고 최대 6명의 적을 1100%의 데미지로 3번 공격하는 마력 폭풍 발생.
(재사용 대기시간 120초)

[공용코어로 출시된 "매직 서킷 풀드라이브 VI"(1레벨, 이하 신규 스킬)]
※ 라이브 서버 기준 스킬 설명입니다.
...
기존 스킬: 캐릭터 주변에 마력 폭풍 발생
신규 스킬: 적에게 마력 표식을 새기고, 적에게 스킬 적중 시 마력 폭풍 발생.

이 메커니즘이 변경되면서, 기존 스킬에 마력 폭풍과 연동되어 사출되던 '다가오는 죽음'이 사출되지 않은 현상이 발생하였습니다.

>> 공용코어 0레벨인 경우의 이미지 / 공용코어 1레벨인 경우의 이미지

※ 위 이미지는 테스트 서버에서 촬영된 결과이며, 라이브 서버에선 수치가 조정되었으나 메커니즘 상의 결함은 동일하게 재현됨을 안내드립니다.

위 이미지는 매직 서킷 풀드라이브(또는 VI) 및 인피니티 스펠을 사용 후, 잊혀지지 않는 흉몽VI를 10번 사용한 결과입니다.

'다가오는 죽음' 사용 횟수를 보시면,
기존 스킬 사용 시 52회로, (잊혀지지 않는 흉몽VI 10회 + 새어 나오는 흉몽 1회 + 매직 서킷 풀드라이브 2회) * 인피니티 스펠 4회.
신규 스킬 사용 시 44회로, (잊혀지지 않는 흉몽 VI 10회 + 새어 나오는 흉몽 1회) * 인피니티 스펠 4회.

메커니즘의 변화로 인해, '다가오는 죽음' 사용횟수가 줄었음을 알 수 있습니다.

##### 2. 기존 스킬 기준으로 기존 사이클 vs. 신규 사이클.
그동안 저는 기존 스킬(매직 서킷 풀드라이브)이 스탯 버프 및 일정 간격 마다 공격하는 형식만 가지고 있는 버프 스킬인 줄 알고 기존 사이클을 유지하였습니다. '다가오는 죽음'이 발생한다는 사실을 깨닫고, 다른 사람들이 사용하는 사이클을 따라 해보았습니다.
