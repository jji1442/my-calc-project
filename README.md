...
calc_enh_cnt_for_skill_core_list()

calc_enh_cnt_for_skill_core_list()는 스킬 코어 리스트를 위한 강화 횟수 계산하는 함수입니다.
따로 계산하는 이유는 스킬 코어에 10레벨 단위로 보스에게 적용되는 데미지 증가 또는 방어율 무시 증가인 부가 효과가 있습니다.
이 부가 효과의 효율을 측정하기 위한 함수로, 모든 레벨 구간에서의 스킬 %데미지로 인한 데미지 상승율 및 부가 효과로 인한 데미지 상승율을 합산하여 산출합니다.
이 산출된 데미지 상승율 리스트를 가지고, 먼저 강화해야하는지 검증하는 함수입니다.

10레벨 단위만 비교적 높은 비용을 요구하기 때문에, 그 10레벨 이후 레벨의 비용적 효율을 기준으로 효율이 좋은 시점을
