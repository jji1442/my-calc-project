...
기본적인 구조.
1. 스킬 %데미지 상승. (4차 마스터리 코어, 오리진 스킬, 어센트 스킬)
2. 스킬 최종데미지 % 상승.(5차 강화 코어)
3. 해당 스킬에만 적용되는 스탯% 상승하는 효과.(오리진 스킬, 어센트 스킬)

직업별 부가효과.
1. 아크에게만 적용되는 구조.(어비스 차지드라이브VI의 보스 공격 시 데미지 1% 증가)
   - 1레벨 31%, 1레벨 이후 6레벨당 +1%
   - 1, 7, 13, 19, 25 레벨

위 구조를 합쳐서 효율을 산출하는 함수.(일정 레벨 달성 시 최종데미지 상승율을 산출)
이 효율 산출한 것을 가지고 재화효율을 추가하여 강화효율을 산출하는 함수.(일정 레벨 달성 시 100억당 최종데미지 상승율을 산출)
